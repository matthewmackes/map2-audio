...
| Tracktion Engine | Fetched at build time via CMake FetchContent when `-DMAP2_DAW_MODE=ON` is set | See `juce-engine/cmake/Map2Tracktion.cmake` (introduced under T2503 Set 2). Tracktion Engine is available under GPLv3 or a commercial license from Tracktion Software Corporation (https://www.tracktion.com/develop/tracktion-engine). Embedding it under MAP2's AGPLv3 license is permitted per AGPLv3 §13 / GPLv3 §13 (one-way compatibility from GPLv3 into AGPLv3); the combined work distributes under AGPLv3. See `docs/architecture/LICENSE_COMPATIBILITY.md` for the full audit. The flag defaults OFF; the dependency is not fetched in stock builds. |
