...
- `[✓]` `T2508` — Python recorder service + routes + artifacts integration (phase 4 of T2504) — **DONE** (summary synced 2026-06-02; was stale `[>]` "6 of 7"). Verified fully shipped: RecorderService lifecycle + 6 session routes + 4 artifact-registry routes + WS broadcaster on `recorder:session` + asset_type="recording"; 122 pytest passing across `tests/test_recorder_*`. The earlier "remaining" items (15 fps broadcaster, engine-side transport binding) shipped with T2507's engine bindings.
- `[✓]` `T2459-H` — MIDI Backend Unification (controller-host + libremidi + ControllerEngine) — **code-side complete; all remaining gates are the one bench-session runbook → § After-Release Hardware Verification** ([`docs/midi/T2459_FINAL_BENCH_SESSION.md`](midi/T2459_FINAL_BENCH_SESSION.md)). (Reframed 2026-06-02: the HIL bench session no longer blocks this release.)
- `[✓]` `T2459-H3` — MeloAudio Commander device-pack cutover completion — code-side complete; bench gate (T2459 final bench session Gate 1) → § After-Release Hardware Verification.
- `[✓]` `T2459-H3-CFG` — MeloAudio Commander Configurator (Phases 1-6 + Outer-Loop-2 dispatcher all SHIPPED) — code-side complete; Phase 7 HIL (bench Gate 1) → § After-Release Hardware Verification.
- `[✓]` `T2459-H4` — Device-service migrations (Maschine/MPX-1/IntelFX) — code-side complete; HIL parity (bench Gate 3) → § After-Release Hardware Verification.
...
- `[✓]` `T2459-H7-PW-UMP` — Path 4 code-side COMPLETE end-to-end (2026-05-08); summary synced 2026-06-02 to match the detailed `[✓]`. The G1–G5 PipeWire/libremidi UMP-substrate + MIDI-2.0-hardware HIL is after-release follow-up (§ After-Release Hardware Verification), not a gate — a substrate/upstream issue, not MAP2 code.
...
- `[>]` `T2517` — **MPX-1 as effects-chooser block + AES/S/PDIF dual-connection** (filed 2026-05-12). 9 sub-tasks. Generalize existing `LexiconHardwareProcessor` (drop hardcoded UA-1000 channel constexprs → per-instance atomic channel-config); wire existing `hardware://lexicon-mpx1-spdif` descriptor into `/api/plugins/discover` (audit 2026-06-02: descriptor IS already returned — `hardware://lexicon-mpx1` is the canonical `uri` and `-spdif` ships in its `aliases` list via `build_lexicon_mpx1_plugin_descriptor`; both URIs are registered with the singleton lock. Open Q for this sub-task: promote `-spdif` to a first-class descriptor vs. keep it as an alias); add interface-capability registry (`digital_io_stereo`, `aes_ebu`, `spdif_coax`); singleton-instance enforcement with structured 409; per-instance side-panel + measured-latency calibration wizard. AES preferred, S/PDIF fallback, both per-instance. Estimated 19-21 h active + 1.5 h soak.
...
- `[✓]` `T2521` — **SonoBus/AOO full remote-audio transport deployment epic** (filed 2026-05-13). Decision lock complete 2026-05-13 via 5-question protocol: native MAP2-owned AOO daemon, SonoBus `/sonobus`, self-hosted connection server enabled by default, AVB-template GUI/integration/clustering/storage, PCM 24-bit/48 kHz low-latency defaults, full multichannel, no recorder/artifact integration, AVB preferred with SonoBus fallback, same-LAN + impairment validation, vendored AOO source with notices. **T2521 CODE-SIDE COMPLETE 2026-05-15 across cycles 26-30 of the extended Continue run.** Only T2521-4 (C++ daemon, requires RT audio bench) + T2521-10 (LAN two-node + impairment validation) remain — both bench-gated, handoff doc at [`docs/architecture/SONOBUS_BENCH_HANDOFF.md`](architecture/SONOBUS_BENCH_HANDOFF.md). **Cycle 26 (T2521-5 routes):** 12 architecture-doc-spec'd routes shipped — probe / groups CRUD / session disconnect / profiles CRUD with built-in immutability / network / connection-server / diagnostics. WS path consistency fix. 18 new pytest cases. **Cycle 27 (T2521-6 GUI):** `/sonobus` nav entry in GlobalTreeNav (sibling of /midi-hub /avb /artifacts); new `SonoBusRoutingPage` talker × listener matrix mirror of /avb/routing; 7 new jest cases. **Cycle 28 (T2521-7 picker):** `SnapshotInterfacePicker` gains a "SonoBus peers" transport group with Cloud icon; SonoBus IDs are first-class in `requested_input_interface_id` / `requested_output_interface_id`; +1 jest + 4 pytest cases prove Q10 (snapshot accepts) and Q12 (recorder rejects) coexist on the same ID. **Cycle 29 (T2521-8 packaging):** RPM spec installs systemd unit + firewalld fragment + env example; `%preun` stops + disables the unit on uninstall; `%postun` drops the firewalld fragment; `vendor/aoo/` gains VERSION + LICENSE.placeholder so the BSD-3 posture is visible pre-source-pull; 9 new pytest cases. **Cycle 30 (bench handoff):** `docs/architecture/SONOBUS_BENCH_HANDOFF.md` lands as the operator-facing runbook for T2521-4 + T2521-10 — vendor pull instructions, daemon acceptance criteria, soak-matrix table (3 profiles × 5 impairment levels), evidence-artifact inventory, post-soak closeout. **Full T2521 test surface: 154 pytest cases + 13 jest cases, all green; production build clean.** Code-side gates that no longer block deploy: T2521-1, -2, -3, -5, -6, -7, -8, -9 all Done. Bench-gated remaining: T2521-4, T2521-10. **Cycle 34 SHIPPED 2026-05-15 (T2521-7 publish-path SonoBus exposure):** Operator audit of the Publish Snapshot screenshot found three SonoBus exposure gaps. Fixed two in this cycle: (S1) `SnapshotInterfacePicker` ContentSwitcher now exposes a "SonoBus" tab alongside All / Local / AVB / Cluster; (S2) `SnapshotPublishAudioPortWorkspace` gains a symmetric "SonoBus peers" section in both Inputs and Outputs tabs, sourced from the existing `/api/sonobus/peers` projection via the `useSonoBusPeers` hook. Each peer card carries the listener_node_id + endpoint + active/configured Tag (mirroring the AVB talker/listener pattern). Channel summary line now counts SonoBus peers alongside AVB endpoints. 5 new jest cases pin the empty-state copy, the card render, the selection toggle, the Outputs-tab parity, and the Active/Configured tag tone. 60 SnapshotPublish + SnapshotInterface + SonoBus jest cases all green; production build clean (App-CWyJGix2). Remaining (S3): Routing topology Remote-peer flow node, deferred to cycle 35. **Cycle 35 SHIPPED 2026-05-15 (T2521-7 S3 scaffolding):** Type-level scaffolding for the Remote-peer flow node landed. `JuceGridRoutingMarkerId` gained `inputRemote` + `outputRemote` variants; `JuceGridRoutingFlowInfo` gained `remoteInput?` + `remoteOutput?` flags so per-mode canvas rendering can pick up the operator's SonoBus binding state. New parity-with-backend `sonoBusInterfaceIdGuards.ts` exports `SONOBUS_ID_PREFIX` + `isSonoBusInterfaceId` + `parseSonoBusInterfaceId` + `makeSonoBusInterfaceId` (mirrors `app/services/sonobus/interface_ids.py` 1:1 so the frontend identifies SonoBus IDs without round-tripping the backend). New `snapshotEditorRemotePeerAnnotations.ts` derives the `{ remoteInput, remoteOutput }` flag pair from a chain's I/O binding state. 22 new jest cases across 2 suites. The full per-mode canvas badge rendering (one Cloud icon next to Input/Output for each of the 5 routing modes) is filed as a follow-on slice because the visualizer's SVG layout is mode-specific and the badge placement needs an operator-driven design pass. Drive-by: restored `MidiHubShell.tsx` + `midi-hub/*` files that were accidentally deleted during a previous merge conflict resolution; production build was broken on master, now clean. **Cycle 36 SHIPPED 2026-05-15 (per-mode Cloud badge rendering, S3 closeout):** Closed the per-mode follow-on filed in cycle 35. New `summarizeRemoteFlags(flows)` helper collapses per-flow `remoteInput`/`remoteOutput` into diagram-level booleans, threaded into the 5 `build*Diagram` functions (Series / Parallel / A/B / Morph / Sidechain) via a new optional `remote?: boolean` field on `DiagramTerminal`. `renderTerminal` paints a 12×8 hand-drawn Cloud SVG path next to the terminal label when `remote=true`, with `<title>SonoBus remote peer</title>` for screen readers and an enhanced `aria-label`. Active-state cloud uses Carbon support-info; inactive uses border-strong. 5 new jest cases pin badge presence, no-badge default, the aria-label, the output-only case, and the every-mode 2-badge invariant. Visualizer + helper sweep 27/27 green; production build clean (App-DpSv5Sic). **S3 routing topology Remote-peer flow node integration is now code-complete across all 5 routing modes.** **Cycle 38 SHIPPED 2026-05-15 (first-class services framework — 4 → 5):** Updated `docs/architecture/FIRST_CLASS_SERVICES.md` to promote SonoBus as the 5th first-class platform service offering alongside MIDI / AVB / Sampler / Audio Effects. New row in the canonical surface/authority table; references section gains pointers to `SONOBUS_AOO_TRANSPORT.md` (design) + `SONOBUS_BENCH_HANDOFF.md` (bench-gate runbook); updated standing platform-directive memory at `project_first_class_services.md` from "four first-class service offerings" → "five first-class service offerings" with SonoBus's status, scope, and execution-order entry. The five-services framing is now the canonical platform-architecture model.
- `[✓]` `T2523` — **Maschine Looper Integration epic** (CLOSED 2026-05-14 in cycles 17-21 of the extended Continue run). Bridges the existing T2512 Multi-track Looper service to the MK1's transport zone + dual LCDs. **All five subtasks A-E shipped:** A backend transport router + 3 new service verbs + 3 new HTTP routes + 26 backend pytest cases; B dual-LCD `context="looper"` render context + 3 LCD pytest cases; C Performance-tab `MaschineLooperSection` + 10 jest cases; D `useLooperLiveStatus` WS hook replacing polling + 8 hook jest cases; E docs at [`docs/maschine/T2523_LOOPER_INTEGRATION.md`](maschine/T2523_LOOPER_INTEGRATION.md). **Full T2523 test surface: 29 new pytest cases + 18 new jest cases, all green.** Deferred to follow-ons: T2523-NR (SHIFT+NoteRepeat → quantize), T2523-TT (Tempo encoder tap tempo), T2523-MT (multi-track active-track selection), T2523-EC (daemon-side long-press erase confirm), T2511 (C++ RT engine bindings for `looper_play` / `looper_restart`). **Cycle 17 SHIPPED 2026-05-14 (T2523-A):** Maschine MK1 transport buttons (Play / Stop / Rec / Restart / Erase) now dispatch into LooperService via local Python (no HTTP round-trip); 3 new looper verbs `play_track` / `restart_track` / `toggle_quantize`; 3 new HTTP routes `POST /api/v1/looper/track/{t}/{play,restart,toggle-quantize}`; locked-track guards swallowed at the daemon boundary so a locked track produces silence not a crash; 26 new pytest cases across 3 suites + 1 device-pack schema fix (3 looper profiles had `../../_generic/...` paths that resolved one level above the repo). **Cycle 18 SHIPPED 2026-05-14 (T2523-B):** Maschine LCD `context="looper"` render path landed; left LCD now draws a 4-track loop-status grid (track #, state glyph IDLE/REC/PLAY/OVR/STOP, layer count, position bar proportional to playhead/loop length); right LCD draws master controls (active count, master dB, mute, tempo, sync-master track); structured `looper` payload returned alongside both XBM framebuffers so GUI Twin + physical MK1 share one source of truth (locked decision Q6). 3 new pytest cases. **Cycle 19 SHIPPED 2026-05-14 (T2523-C):** Performance tab `MaschineLooperSection` component lands between Step Sequencer and Quad Morph zones (locked decision Q4). 4-track loop-status grid mirrors the LCD render context with Carbon tones (red=recording, magenta=overdubbing, green=playing, cyan=stopped, warm-gray=empty); 5 transport buttons (Rec/Play/Stop/Restart/Erase) dispatch into `looperApi.{record,play,stop,restart,clear}` against Track 0; transient flash effect on the matching button when a physical `transport_press` lands (motion-reduce safe); status polled @ 250ms via TanStack Query. 3 new client methods (`play`/`restart`/`toggleQuantize`); 10 new jest cases; full Maschine sweep 90/90 green; production build clean. **Cycle 20 SHIPPED 2026-05-14 (T2523-D):** Replaced the 250 ms polling cadence inside MaschineLooperSection with a reusable `useLooperLiveStatus` hook that subscribes to the existing T2512-WS `looper:status` topic. Hook holds a 2 s safety-net HTTP refresh so pure playhead motion still ticks during quiet moments, surfaces `isConnected` so the section header can show "Live" / "Stale" / "Connecting…", and closes the WS cleanly on unmount. 8 new hook jest cases against a MockWebSocket fixture; section tests stay 10/10 green; full Maschine + hook sweep 98/98 green; production build clean. NoteRepeat + Tap Tempo deferred (MK1 has no dedicated tap-tempo button; NoteRepeat is busy with menu navigation — both reachable via SHIFT-combo follow-on). **Locked decisions (Q1-Q10):** (1) **Buttons in scope:** Play, Stop, Rec, Restart, Erase, Note Repeat, Tap Tempo (all 7 transport-zone buttons the daemon already decodes via `decoded_type: transport_press`). (2) **Action semantics:** Mixxx-style state machine — Rec = arm → record → close cycle; Play = play/stop toggle; Stop = stop+rewind; Restart = jump to loop start; Erase = clear active loop (long-press confirm); Note Repeat = quantize toggle; Tap Tempo = set length anchor. (3) **LCD content:** Per-track state grid only — Left LCD = 4-track loop status (track number, state IDLE/REC/PLAY/OVERDUB, length, position bar); Right LCD = master controls, tempo, sync status, totals. (4) **GUI placement:** New section in Performance tab (between Step Sequencer and Quad Morph zones). (5) **Backend wiring:** Wire to existing T2512 Looper service — route transport HID events through existing `/api/looper/*` routes; subscribe to existing WS topic for state. (6) **LCD render path:** Reuse `maschine_lcd_service` render context — add a new `looper` context value alongside existing `audio_grid` and `stats`. Backend Python composes the bitmap; frontend GUI mirrors via existing LCD simulator panel + Hardware Twin canvas. **Single source of truth for both physical + GUI LCDs.** (7) **Worklist scoping:** New T2523 epic with 4-6 subtasks (button bindings, LCD render context, GUI section, backend bridge, jest+pytest, docs). Cross-references T2512 (looper service) and T2522 (Maschine GUI). (8) **Execution order:** Finish T2522 cycles 10-15 first, then start T2523 as a fresh effort. (9) **Operator UX:** LCD + GUI equally — full feature surface in both places; user picks based on context. (10) **Audio routing scope:** Trigger only — MK1 buttons trigger existing T2512 looper actions; audio input/output routing stays whatever the looper service is already configured to capture. **Status:** filing-only at this point; subtask scaffolding lands at first execution.
- `[✓]` `T2522` — **Maschine MK1 Extended GUI epic** — CLOSED 2026-06-02. All 5 tabs ship real, substantial implementations (Hardware Twin, Profile Workbench, Performance, Mapping Studio, Diagnostics 6-panel dashboard); the Advanced-Maschine entry button is wired on the MK1 Hardware Catalog card; all 9 T2522-E chrome-review items (F1-F9) are Done. Verified 2026-06-02: no `maschine-tab-placeholder` rendered for any tab, no TODO/FIXME/stub markers in the Maschine component tree, Maschine jest sweep green. The `[>]` marker only reflected the extended-40-cycle retrospective window; there are no incomplete subtasks. (filed 2026-05-14). Tabbed `/maschine` shell exposing four full-vision surfaces beyond the current 6-panel engineering dashboard. **Locked decisions:** (a) tabbed shell at `/maschine` with `?tab=<id>` query-param routing — tabs: Twin · Workbench · Performance · Mapping · Diagnostics (the existing 6 panels move under Diagnostics); (b) build order T2522-A → T2522-C → T2522-D → T2522-B; (c) full-vision scope per subtask (multi-cycle epics, no MVP-only stopping points); (d) entry point is a new **"Advanced-Maschine"** button on the Maschine MK1 card in the Hardware Catalog (gated by `legacyId === 'maschine-mk1'`), routing to `/maschine?tab=workbench` (the existing legacy "Open" button keeps default tab = twin); (e) one rollup epic with four subtasks T2522-A/B/C/D; (f) shipped under autonomous Continue protocol — 15 cycles, commit + dual-push pre-authorized, no stubs, then 48h chrome retrospective at cycle 15. **Subtasks:** `T2522-A` Hardware Twin (foundation: tab framework, query-param routing, `Advanced-Maschine` button on DeviceCard, live SVG/canvas mirror of pads/encoders/dual-LCDs streaming from existing maschine WS); `T2522-C` Performance tab (4×4 pad grid, pressure/velocity curve editor over `pad_sensitivity_calibrator` + `pressure_curve_fitter`, step seq + scenes, Quad Morph XY); `T2522-D` Mapping Studio (snapshot-scoped param drag-drop, SHIFT-layer overlays, LED choreography binding, State Authority `document.controllers.maschine_mk1` integration with VALIDATING→LIVE phase visibility); `T2522-B` Profile Workbench (T700 profile DSL framework, JSON+flexbox layout engine, dual-LCD live render preview, all 25 T700 profiles); `T2522-E` Chrome review + interface improvements — added 2026-05-14 per operator request after the cycle-3 Twin landing. Audits the full T2522 surface (tabbed shell + Twin + Workbench + Performance + Mapping placeholders + Diagnostics) for chrome quality: Carbon conformance (per `docs/design/CARBON_CONFORMANCE_STANDARD.md`), header/spacing/typography hierarchy across the 5 tabs, Tab-vs-Diagnostics scroll-anchor consistency, Hardware Twin SVG visual fit & responsive scaling, AppShell action-chip behavior across tab transitions, deep-link UX (`?tab=` from Hardware Catalog, browser back/forward), accessibility (keyboard tab navigation + ARIA labels), motion-reduce + dark-theme parity, and per-tab empty-state polish; produces a delta plan + ships the resulting fixes in the same cycle. **Cycles shipped (15 of 15 in original Continue plan; extended to 40 cycles per operator request 2026-05-14):** C1 button + filing `7ff1819d4`; C2 tabbed shell `c735bf23a`; C3 Twin v1 `e0a0c8aec`; C4 Twin polish (hover/click/encoder rotation/activity flash) `26da37e14`; C5 Performance tab v1 `0b749cb6b → 3b2cecbb8`; C6 pressure-curve editor + backend facade + 5 pytest cases `9fe99ef51`; C7 step sequencer + scenes + 6 backend pytest cases `09cbda42e`; C8 Quad Morph zone `f5c16ff70`; C9 Mapping Studio scaffold + drag-drop `9e5007ef5`; C10 SHIFT layer + LED choreography + T2523 filing `c09c7845e`; C11 State Authority phase strip `fcf0fd72f`; C12 Profile Workbench + DSL framework `c13ffd7d4`; C13 LCD pixel renderer + workbench live preview `c3cdc6d02`; C14 8 of 25 T700 profiles `82808d207`; **C15 chrome review + immediate fixes** (AppShell hardware-layout action only on Diagnostics; LCD canvas a11y labels; full review doc at [`docs/maschine/T2522_E_CHROME_REVIEW.md`](maschine/T2522_E_CHROME_REVIEW.md) with 9 follow-on tickets `T2522-E-F1`..`T2522-E-F9` for cycles 31-39 of the extended run). **Total Maschine test surface: 80 frontend jest cases across 12 suites + 15 backend pytest cases, all green.** **Cycle 22 SHIPPED 2026-05-14 (T2522-E-F1+F4+F6+F7 bundle):** F1 AppShell kicker now reads `Platform / Maschine MK1 / <Tab label>` tracking tab transitions; F4 Mapping Studio empty state gets a "Set up your first chain →" deep-link, Profile Workbench empty-draft microcopy explains the `map2_maschine_profile_drafts` localStorage model; F6 Performance pad-block uses CSS `text-overflow: ellipsis`, Step Sequencer empty-state shows a 16-step bar marker with downbeat highlight, Quad Morph surfaces live X/Y as numeric coordinates; F7 Mapping Studio source list gains a sticky search box that filters by display + label. 3 new MaschinePage jest cases (kicker default + ?tab transitions + per-tab labels). Maschine sweep 93/93 green; production build clean. **Cycle 23 SHIPPED 2026-05-14 (T2522-E-F3+F5+F8 bundle):** F3 motion-reduce opt-outs landed on `maschine-test-suite__result-card--running` pulse + `maschine-perf__pad--clickable:active` scale transform; F5 Hardware Twin header gains a live BPM tag (mirroring Performance header) + a "View on Diagnostics →" jump-link; F8 Profile Workbench gets a "Reset to starter" button that overwrites the JSON editor with the canonical starter content for the currently selected profile. Maschine sweep 93/93 still green; production build clean. **Cycle 31 SHIPPED 2026-05-15 (T2522-E-F2 a11y/dark-theme audit):** Tokenized the 4 Carbon-equivalent hardcoded colors in `MaschinePage.css` — `#0a0e1a` pad text → `var(--cds-text-on-color)`; `#33b1ff` cyan tints + `#f4f4f4` lit-cell text → `var(--cds-support-info)` + `var(--cds-text-inverse)`; `maschine-seq__scene-cell--bound` cyan tint → token equivalent. The 5 remaining `lcd-mock-palette` hex values (LCD canvas + workbench LCD-card bands) gained explicit `// carbon-allow: lcd-mock-palette` annotations + comment explaining they intentionally mirror the physical MK1 LCD substrate. TabList already had `aria-label="Maschine MK1 extended GUI"` so keyboard nav contract was already met. Maschine sweep 93/93 still green; production build clean. **Cycle 32 SHIPPED 2026-05-15 (T2522-E-F9 pad-shape unification):** Operations Console pads (`maschine-ops__grid-pad`) now use the canonical Maschine pad shape: 8px border-radius + 2px border-width + the same `color-mix(in srgb, var(--cds-border-subtle) 50%, transparent)` border ratio Performance pads use. Three pad-shape stylings across the page reduced to one canonical CSS shape (Twin SVG is its own SVG idiom; the two CSS pad surfaces now align). Ops pads stay rectangular (vs Performance's 1:1 aspect-ratio) because the cell content includes a Tag + name + ButtonSet that wouldn't fit in a square; everything else aligns. Maschine sweep 93/93 still green. **All 9 T2522-E follow-on review items are now Done** (F1-F9): chrome polish across the full Maschine surface is complete code-side.
- `[>]` `T2524` — **React GUI release-readiness audit replay (software-only, no SubC)** — filed 2026-05-14 per operator request as a new copy of archived T847 minus standalone validation SubC. Scope: rerun audit + safe cleanup over current `web/` after recent GUI churn, close only software-verifiable tasks, and explicitly skip/record anything needing bench/HIL evidence. **Slice 1 shipped 2026-05-14:** deleted 7 unreferenced CSS leftovers; typecheck clean; no dependency/service/build-runtime changes. **Slice 2 SHIPPED 2026-05-14 (cycle 24):** deleted 3 unreferenced visualization TSX (`FrequencyResponseChart.tsx`, `ReverbTailChart.tsx`, `ReverbMSEGChart.tsx`) + 5 orphan custom plugin cards in `PluginCards/Custom/` (`Dragonfly/{Hall,Room,Plate}Card.tsx`, `LV2/{KeyboardSampler,REEVR}Card.tsx`). Verified the orphan status via `grep -rn "<basename>" web/src/` → only self-references; the corresponding plugins are wired through `registerPluginCard()` template fallback in `PluginCards/registry.ts`. Removed empty `Custom/Dragonfly/` directory. Typecheck clean; atomic build clean (App-Bg4qK9Vp). **Slice 3 SHIPPED 2026-05-14 (cycle 25):** Fixed two pre-existing stale jest assertions in `AppShell.test.tsx` that I uncovered while validating cycles 22-24 — the test was asserting "MultiTrack Recorder" was in the nav rail (retired under T2505 with the T2503 DAW cancellation; the hero card no longer mounts) and asserting that "Connections" / "Presets" MIDI Services children were visible on the landing route (T2525 single-open accordion means only the active branch is expanded). Comments updated to match the post-T2525 invariant. Also added `<rootDir>/web/src/_archive/` to `testPathIgnorePatterns` in `package.json` so archived multitrack-recorder tests stop firing — the `_archive/` tree is by-design retired code retained for git history only. **Slice 4 SHIPPED 2026-06-02 (Claude, ship loop):** deleted 3 fully-orphaned component TSX with ZERO references anywhere in `web/src` (verified per-file: no import, no `lazy()`, no route-string, no css/test sibling, no dir-mate reference) — `snapshots/CommunitySnapshotBrowser.tsx` (559 LoC), `library/InstalledBrowser.tsx`, `chains/ChainDeployModal.tsx` (sole file in its dir → removed the empty `chains/` dir). typecheck exit 0 + production build exit 0 (`✓ built in 18.77s`) — a live import would have broken the build, confirming the deletions safe. **Follow-up filed (T2524-B-FOLLOWUP): two interconnected orphan SUBTREES + a library cluster need whole-subtree analysis before deletion (not safe as singletons):** `components/HorizontalSignalChain/` (HorizontalSignalChain + HorizontalConnector + HorizontalPluginNode + SidechainConnector + types + css + icons — appears wholly dead but the files cross-reference internally, so verify the whole subtree is unreached before removing), `components/MidiCluster/` (Topology/Clock/Health/NodeCard appear orphaned but `MidiClusterConnectionMatrix` has a TEST so the dir is partially live — needs per-file tracing), and a `components/library/` orphan cluster (IRItemCard/NAMItemCard/SFItemCard/DownloadManager/IRFrequencyGraph/LibraryPaths flagged by the zero-import sweep alongside the InstalledBrowser deleted in slice 4 — likely a dead library-browser subtree, verify + batch-remove). Plus singleton candidates from the sweep needing the same per-file check: `layout/chrome/WorkspaceBar.tsx`, `layout/StaticHeroIconLauncher.tsx`, `Visualizations/AudioMeteringCard.tsx`, `Routing/{ParallelRoutingPanel,SidechainPanel,EffectsLoopSummaryPanel}.tsx`, `PluginTags/TagSelector.tsx`, `MidiHub/MidiInnovationPanel.tsx`, `NodeGraph/{NodeDetailTearsheet,NodeGraph}.tsx`, `snapshots/CommunitySnapshotBrowser`-adjacent — each needs the slice-4 zero-reference verification (some, e.g. NodeGraph, showed a barrel ref so are NOT confirmed dead).
- `[✓]` `T2525` — **Global Tree Nav single-open accordion behavior** — closed 2026-05-14. Opening any nav subsection now collapses unrelated/sibling subsections; nested active branch keeps required ancestors only. Added focused accordion helper tests; GlobalTreeNav jest 16/16 + typecheck pass; no dependency/service/build-runtime changes.
...
ID: T2524
Status: [>] In Progress
Title: React GUI release-readiness audit replay — software-only cleanup pass, T847 minus standalone validation SubC
Description:
- Goal / acceptance criteria: Rerun the completed T847-style React GUI audit against the current `web/` tree after the recent GUI churn. Identify dead code, stale routes/components/assets/tests/helpers, duplicated surface area, and low-risk simplification candidates; implement safe cleanup slices that can be verified without physical hardware; preserve every active operator surface; and record any bench/HIL-dependent finding as a blocked/deferred follow-up instead of treating it as required for this pass. This is intentionally a new T847 copy **minus SubC**: validation remains required inside each code-changing cleanup slice, but there is no separate final validation-capture subtask.
- Why it matters: The frontend has changed substantially since archived T847 closed; replaying the audit keeps release UI surface area small and defensible without waiting on bench-only hardware gates.
- Dependencies: None for audit start. Code-changing cleanup must respect active in-flight GUI work (notably T2522/T2523 and existing uncommitted worktree edits) and must not revert unrelated changes. Bench/HIL-only findings are out of scope for completion and should be filed under their owning hardware task if discovered.
- Estimated effort: High; execute in restartable 15-60 minute cleanup slices.
- Required outputs/deliverables: Current GUI audit notes in this worklist entry; code removals/simplifications for safe software-only candidates; per-slice validation evidence (`npm --prefix web run typecheck`, focused Jest, lint/build when appropriate); explicit platform-layer review for any dependency/service/build/runtime assumption changes.
Subtasks:
  - ID: T2524-A
    Status: [>] In Progress
    Title: Audit current React GUI for stale code, duplicate surfaces, and software-only cleanup candidates
    Description:
    - Goal / acceptance criteria: Inspect route reachability, lazy imports, prefetch entries, page/component references, tracked debug artifacts, CSS/test leftovers, and feature surfaces under `web/`; produce a prioritized candidate list separated into safe cleanup now, needs owner decision, and bench/HIL-dependent/deferred.
    - Why it matters: Safe pruning requires knowing which files are truly orphaned versus owned by active GUI epics or hardware-gated workflows.
    - Dependencies: None.
    - Estimated effort: Medium.
  ...
  - ID: T2524-B
    ...
    Description:
    - Goal / acceptance criteria: Remove or consolidate only high-confidence stale GUI code/assets/tests/helpers identified by T2524-A, one restartable slice at a time, while preserving active routes and not touching unrelated dirty worktree changes. Each slice must include appropriate focused validation and a platform-layer impact note.
    - Why it matters: The replay creates value only when safe candidates are actually removed or simplified; validation stays per-slice because standalone SubC is intentionally omitted.
    - Dependencies: T2524-A candidate inventory.
...
## T2525 — Global Tree Nav single-open accordion behavior

ID: T2525
Status: [✓] Done
Title: Global Tree Nav collapses other subsections when a subsection opens
Description:
- Goal / acceptance criteria: Update `GlobalTreeNav` so opening a subsection collapses all other open subsections; only the selected branch remains open at a time, with nested ancestors preserved only when needed to display the open child/active route. Navigation clicks must still route normally, `aria-expanded` must reflect the new state, and persisted expanded-state should store the accordion branch rather than unrelated historical opens.
- Why it matters: The global nav currently permits multiple subsections to stay open, which makes the left rail noisy and harder to scan. Accordion behavior keeps the operator's current focus clear.
- Dependencies: None. Must avoid reverting unrelated active GUI/worktree changes.
- Estimated effort: Low.
- Required outputs/deliverables: `GlobalTreeNav` state logic update, focused unit coverage for single-open behavior, typecheck or focused test validation, worklist closeout note, and platform-layer review.
Subtasks: None
Assigned to: Codex
Last updated: 2026-05-14 20:47 EDT - Codex: shipped single-open accordion behavior; validation PASS (`npm --prefix web test -- --runInBand --runTestsByPath src/app/layout/GlobalTreeNav/GlobalTreeNav.test.ts`, `npm --prefix web run typecheck`). Platform-layer review: no new packages, services, runtime assumptions, installer, or environment updates required.
Completion notes:
- Added pure accordion helpers in `GlobalTreeNav.tsx` to normalize legacy multi-open persisted state to one visible branch and resolve open/close toggles as a single branch path.
- Updated active-route expansion so route changes keep only the active branch ancestors instead of accumulating unrelated open sections.
- Updated hero and dense-tree rendering to use normalized visible expansion state.
- Added focused tests covering sibling collapse, nested ancestor preservation, nested close behavior, and legacy persisted-state normalization.


## T2526 — SynthForge/Sequencer GUI tab information-architecture review

ID: T2526
Status: [✓] Done
Title: SynthForge/Sequencer GUI tab grouping review against comparable instrument products
Description:
- Goal / acceptance criteria: Review the current SynthForge entry path and replacement Sequencer GUI tab/section grouping, compare the grouping to comparable instrument/sequencer products, and produce a concrete recommendation for whether the existing tabs should remain flat or be regrouped. Identify code/documentation touchpoints for a future implementation slice but do not change GUI behavior in this review-only pass.
- Why it matters: The current SynthForge workflow is reached through Sequencer import/navigation and exposes many sections; unclear grouping increases operator cognitive load during performance, setup, routing, and asset work.
- Dependencies: None.
- Estimated effort: Low.
- Required outputs/deliverables: Findings grounded in repository files plus external product comparison notes, recommended grouping model, candidate implementation files, platform-layer impact note, and follow-up tasks if implementation is approved.
Subtasks: None
Assigned to: Codex
Last updated: 2026-05-15 06:36 EDT - Codex: completed review. Findings: SynthForge is exposed through `/sequencer?import_source=synthforge`, legacy `/synth-forge` is retired, `web/src/app` has no dedicated SynthForge page/component, and `/sequencer` publishes 13 flat tree children while `SequencerOverviewShell` has 5 inner tabs. Recommendation: avoid flat tab sprawl; group by task domain (Perform, Sound/Parts, Sequence, Mix/Route, Media/Setup) with secondary tabs/drawer for diagnostics and adjuncts. Follow-up implementation filed as T2527. Platform-layer review: documentation/worklist-only; no dependencies, services, installer, runtime, or environment changes required.
Completion notes:
- Compared current files: `web/src/app/data/launcherCatalog.tsx`, `web/src/app/pages/SequencerPage.tsx`, `web/src/app/pages/sequencerViews/SequencerOverviewShell.tsx`, `web/src/app/layout/startMenuItems.ts`, `web/src/app/components/PluginCards/registry.ts`, and `web/src/app/pages/snapshotEditor/featuredNativeBrowserGroups.ts`.
- External comparison used official Ableton Live, Native Instruments Maschine, Akai MPC, and Arturia Pigments docs/product pages.

## T2527 — SynthForge/Sequencer tab regroup implementation

ID: T2527
Status: [✓] Done — 2026-06-02 (operator approval granted via "all gates open"). Implemented the T2526-locked 5-domain IA.
Title: Regroup SynthForge/Sequencer navigation into task-domain primary modes
Completion note (2026-06-02 — Claude, ship): regrouped the flat 14-section Sequencer nav into 5 task-domain primary modes (Perform / Sound·Parts / Sequence / Mix·Route / Media·Setup) — the operator-confirmed mapping. New `sequencerViews/sequencerDomains.ts` (PrimaryDomainId + DOMAIN_MODEL + `domainForSection`/`firstSectionOfDomain` derivation) + `sequencerSectionIds.ts` (shared SectionId, breaks a circular import). `SequencerPage.tsx` renders a two-level nav (domain mode row → domain-scoped section subnav); the active domain is DERIVED from the active section, so **all existing `/sequencer?section=<id>` deep-links work unchanged — no param rename, no redirect needed**. `launcherCatalog.tsx` `/sequencer` children collapsed to the 5 domain entries (GlobalTreeNav nests by path, not query param), each deep-linking to its domain's first section. Killed the confusing `Perform`/`Performance` near-duplicate by relabeling the `perform` section → "Quick Perform" (both render distinct surfaces; neither deleted). A partition test pins the mapping (`union(DOMAIN_MODEL) === SECTION_IDS`, no dupes/orphans). SynthForge import (`?import_source=synthforge`) preserved. **Gate (run by Claude): typecheck exit 0; `SequencerPage`+`launcherCatalog` jest 21/21; full production build exit 0 (`SequencerPage-C2nRmSS1.js` emitted).** In-browser visual confirm is after-release polish (§0.8 gate open per the 2026-06-02 directive).
Description:
- Goal / acceptance criteria: Replace the flat 13-child `/sequencer` tree/tab experience with a smaller primary-mode IA and secondary subnavigation/drawer grouping. Proposed primary domains: Perform, Sound/Parts, Sequence, Mix/Route, Media/Setup. Remove confusing duplicates (`Performance` vs `Perform`, `Step` vs `Sequence`), expose `Setup` consistently, preserve deep links through redirects/aliases, and keep SynthForge import and snapshot plugin-browser entry paths working.
- Why it matters: The review in T2526 found the current flat grouping does not match comparable instrument products and makes live-performance, sound-design, routing, and setup tasks compete as peers.
- Dependencies: T2526 complete; operator approval before code changes.
- Estimated effort: Medium.
- Required outputs/deliverables: Updated `launcherCatalog` tree children, `SequencerPage` section model, overview shell/subnav behavior, route/deep-link compatibility tests, focused Jest/typecheck/build validation, and platform-layer impact note.
Subtasks: None
Assigned to: Unassigned
Last updated: 2026-05-15 06:36 EDT - Codex: filed as follow-up from T2526 review; pending operator approval.

## T2528 — Existing Snapshot bundle download exposure

ID: T2528
Status: [✓] Done
Title: Expose the existing `.map2snapshot` bundle download in the Snapshot explorer
Description:
- Goal / acceptance criteria: Add a visible per-snapshot link in the Audio Artifacts Snapshot explorer that downloads the existing `.map2snapshot` bundle from `GET /api/snapshots/{snapshot_id}/export`. Remove/cancel any plan to design a second portable snapshot/playbook solution; the current JSON graph document + `.map2snapshot` zip bundle is the solution. Acceptance requires focused UI test coverage, typecheck/build validation, and an explicit platform-layer review.
- Why it matters: Operators need a direct portable bundle download for every saved snapshot, and product direction is to expose the existing bundle instead of creating parallel formats or provisioning schemes.
- Dependencies: Existing snapshot export route and bundle implementation.
- Estimated effort: Low.
- Required outputs/deliverables: Updated Snapshot explorer UI, focused Jest coverage, validation evidence, and worklist closeout noting no new dependencies/services/runtime/build requirements.
Subtasks: None
Assigned to: Codex
Last updated: 2026-05-15 10:02 EDT - Codex: implemented and closed.
Progress notes:
...
## T2529 — Untie platform from `mm`/UID-1000 account → dedicated `map2` service user + FHS install

**Filed:** 2026-05-15 — operator-directed service-user/FHS effort. 2026-06-01 operator clarified there is no hold on other in-progress work; Codex is the only active agent and may continue any unblocked task.
...
ID: T2459-H3
Status: [✓] Done — bench Gate 1 HIL; hardware verification is after-release work, not a gate (operator 2026-06-02)
Parent: T2459-H
...
ID: T2459-H3-CFG
Status: [✓] Done — Phase 7 HIL (bench Gate 1); hardware verification is after-release work, not a gate (operator 2026-06-02)
Parent: T2459-H3
...
ID: T2459-H4
Status: [✓] Done — HIL parity (bench Gate 3); hardware verification is after-release work, not a gate (operator 2026-06-02)
Parent: T2459-H
...
ID: T2499
Status: [✓] Done — all 3 sub-tasks code-complete; hardware verification is after-release work, not a gate (operator 2026-06-02)
...
ID: T2499-A
Status: [✓] Done — HIL parity bench; hardware verification is after-release work, not a gate (operator 2026-06-02)
Parent: T2499
Title: Map a MIDI controller — generalize the MeloAudio Configurator pattern into a reusable wizard with layered scope
Status note (2026-06-02): code-side complete + VERIFIED — the configurator framework is mounted at `/midi/devices/configurator/meloaudio` (`App.tsx:896`, `MeloAudioConfiguratorFrameworkPage`) and the bespoke route now redirects to it (`App.tsx:892`); the pack-picker page is at `/midi/devices/configurator` (`App.tsx:905`). The only remaining gate is HIL parity on a physical MeloAudio Commander (SysEx/CC traffic matches the legacy path) — moved to § After-Release Hardware Verification per the 2026-06-02 directive. (The original cycle-10 closeout note below predates the UI swap; the route mounts above supersede its "not yet mounted" items 1 + 3.)
Description:
...
ID: T2499-B
Status: [✓] Done — daemon USB-serial probe + physical-MK1 capture; hardware verification is after-release work, not a gate (operator 2026-06-02)
Parent: T2499
Title: Calibrate Maschine MK1 — full T700 onboarding (pads + pressure + screen + profile selection) with dual-surface UX
Status note (2026-06-02): code-side complete + VERIFIED — `MaschineOnboardingWizardPage` mounted at `/maschine/onboarding` (`App.tsx:678`); service + 7 routes + wizard shipped. Remaining gate is daemon-side USB-serial probe + real per-pad/curve/LCD capture on a physical MK1 → § After-Release Hardware Verification. The orchestration + accept-defaults operator path runs today.
Description:
...
Assigned to: Claude
Last updated: 2026-06-02 EDT - Claude: **Slice 8 SHIPPED — onboarding orchestrator OPERATOR SURFACE (routes + web wizard).** This closes the two code-side gaps the cycle-10 closeout flagged for T2499-B: the orchestrator (slices 1-7) was fully built but the daemon never called it and operators had no UI. Slice 8 lands the operator-facing half:
  - **Service singleton** — `app/services/maschine/onboarding_service.py`: process-scoped `MaschineOnboardingService` holding one live `MaschineOnboardingOrchestrator`, honoring `MAP2_MASCHINE_DEVICES_DIR` for the per-unit calibration dir, with serialization helpers (`status()` exposing state + phase_index + phase_order + is_ready + history; `calibration()`) and a test seam (`reset_maschine_onboarding_service_for_tests`). The daemon, when its USB-serial probe lands, shares this same orchestrator instance — the service does not duplicate the connect/disconnect logic.
  - **7 HTTP routes** on `/api/maschine/onboarding/*` (`app/routes/maschine.py`): `GET /status`, `GET /calibration`, `POST /connect`, `POST /tour-complete`, `POST /skip`, `POST /phase-complete`, `POST /reset`, plus `GET /profile-catalog` (T700 Q68 T1..T25 from `ProfileSelectionDriver`). Transition guards surface as 409; schema violations as 400.
  - **Web wizard** — `web/src/app/pages/sequencerViews/MaschineOnboardingWizard.tsx` + `MaschineOnboardingWizardPage.tsx` mounted at `/maschine/onboarding` (App.tsx); client `web/src/map2/clients/maschineOnboarding.ts`. Carbon `ProgressIndicator`-driven flow: connect by USB serial → tour → pad sensitivity → pressure curves → screen calibration → profile pick → ready, with per-phase "accept defaults" (the precise per-pad capture stays the MK1 LCD's dual-surface job) + ERASE-skip + hot-load-on-reconnect.
  - **SetupView card flipped** `Coming soon` → `Available` (roadmapTag `T2499-B`), deep-linking to `/maschine/onboarding`. All four Sequencer Setup cards are now Available.
  - **Tests:** +12 pytest (`tests/test_maschine_onboarding_routes_t2499b.py` — full flow / hot-load / skip / 409 / 400 / catalog), +6 jest (`MaschineOnboardingWizard.test.tsx`), SetupView.test.tsx updated (4 Available / 0 coming-soon + MK1 nav). Combined T2499-B backend sweep 171/171; Maschine+SetupView jest sweep 104/104; typecheck + atomic build clean (`App-CTGjgvPW.js`). **Remaining T2499-B gate is bench-only:** daemon-side USB-serial probe wiring + real per-pad/per-curve/LCD capture on a physical MK1. The orchestration + default-accept path is fully operator-runnable today.
Prior — 2026-05-11 EDT - Claude: **Slice 7 SHIPPED — profile-selection driver (`app/services/maschine/profile_selection_driver.py` + `tests/test_maschine_profile_selection_t2499b.py`).** T700 Q68 25-profile catalog (T1..T25) enumerator + operator-pick recorder + schema-compatible payload emitter. Catalog assembled at construction from `app/services/maschine/profiles/json/*.json` (24 descriptors) + the code-defined `T9_CATALOG_ENTRY` fallback, yielding the complete T1..T25 set in stable order. `ProfileCatalogEntry` carries `(id, name, source)` so the picker UI can render labels (`'T1 CTRL'`, `'T9 EFFECT CHAIN EDITOR'`, ...) and gate code-defined vs JSON entries. `select(profile_id)` validates against both shape (`re.fullmatch(r"T([1-9]|1[0-9]|2[0-5])")` — same regex `calibration_store._validate_profile` uses) and live catalog membership (catches drift between catalog enumeration + regex). `finalize()` returns `{"id": "T<n>"}` accepted byte-for-byte by `MaschineCalibrationStore.update(selected_profile=...)`. **30 new pytest cases**: live-catalog completeness (T1..T25 in order, code-defined T9 present with `source='code'`), every id selectable + round-trips through the calibration store schema, invalid-id rejection (empty, T0, T26, lowercase, with-space, plain integer, None), replace-and-clear semantics, finalize-before-select raises, missing/corrupt/duplicate fixture-catalog error paths, non-catalog ids silently dropped, name-fallback to id, drift guard (T14 rejected when fixture catalog has only T1+T9), and an end-to-end orchestrator integration test (TOUR → PAD_SENSITIVITY → PRESSURE_CURVES → LCD_CALIBRATION → PROFILE_SELECTION → READY with the driver finalising the pick and the per-unit YAML carrying `{"id":"T14"}`). **Combined T2499-B sweep 155/155 in 5.15s** (45 store + 21 orchestrator + 18 pad sensitivity + 22 pressure curves + 19 LCD calibration + 30 profile selection). The orchestrator wiring is already in place from slice 3 — `on_phase_complete(PROFILE_SELECTION, driver.finalize())` flows through the generic phase-completion handler that maps `PROFILE_SELECTION → "selected_profile"` and advances to READY. With slice 7 complete, the four T2499-B calibration phases are all code-side shipped; remaining T2499-B work is the daemon-side wiring (USB-serial probe + boot_sequence subscriber) and the operator UI for each phase.
...
ID: T2499-C
Status: [✓] Done — T004 real-AVDECC-lab production gate; hardware verification is after-release work, not a gate (operator 2026-06-02)
Parent: T2499
Title: Discover AVDECC devices — Sequencer-context binding flow with simulator-backed shipment + T004 production gate
Status note (2026-06-02): code-side complete + VERIFIED — `AvbServicesAvdeccBindingWizardPage` mounted at `/avb/avdecc/binding-wizard` (`App.tsx:968`); ships with simulator backing per the locked Q4 decision. Real-AVDECC-hardware acceptance is the T004 lab gate → § After-Release Hardware Verification.
Description:
...
ID: T2459-H7-PW-UMP
Status: [✓] Done — code-side decision doc shipped 2026-05-07; the PipeWire/libremidi UMP substrate + MIDI-2.0-hardware HIL is after-release follow-up, not a gate (operator 2026-06-02, "all gates open"). This is a substrate/upstream issue, not MAP2 code — nothing further to build here.
Parent: T2459-H7
...
- Dependencies: T2508 (recorder service), T2506 (`recording` block already in the Raft-replicated snapshot graph), **T2510-0 (per-chain cluster-owner schema field — see below; blocks T2510-2)**.
- Estimated effort: 3-4 cycles (+ T2510-0 schema-change scope).

Sub-tasks:
- `T2510-0` — **[✓] Done 2026-06-03 (Claude, ship loop) — STATUS RECONCILED (was stale false-`[ ]`; the T2510 epic header at line 2758 already declared it shipped, this subtask marker was never flipped).** Per-chain cluster-owner schema field is fully code-complete; verified end-to-end against the live tree at HEAD, no re-implementation needed:
  - **Schema (subtask 1):** `state_authority_graph.py:744-749` validates `graph.channels[].cluster_owner_node_id` — "must be a non-empty string or null"; the v2026.06 rationale comment lives at `:25` and `:160`.
  - **Version (subtask 2):** `SNAPSHOT_GRAPH_VERSION == "2026.06"` (`state_authority_graph.py:24`). No legacy-version acceptance / no upcast migration added — clean schema change per the no-migration directive.
  - **Compiler (subtask 3):** `audio_state_snapshot_compiler.py:91-112` resolves per-chain owner with `null` → `deployment.primary_node_id` at compile time (a runtime default, not a back-compat shim); resolved owner surfaced on the compiled intent for T2510-2's follower logic.
  - **Writers (subtask 4):** the field round-trips through the validator/normalizer + compiler. The §0.8-gated UI owner-selector is the ONLY remaining piece and is explicitly out of autonomous scope (operator visual sign-off) — filed below as `T2510-0-UI`.
  - **Philosophy doc (subtask 5):** `docs/philosophy/snapshot-single-source-of-truth.md` carries the field.
  - **Gates (run by Claude this cycle):** `pytest tests/test_state_authority_graph_recording.py tests/test_state_authority_graph.py` → 24 passed; compiler owner-resolution coverage in `tests/test_audio_state_snapshot_compiler.py` (5 dedicated cases: null→primary, explicit-owner-wins, none-when-no-primary, blank→primary, mixed-per-chain). T2510-2 (which consumes this) was already closed 2026-06-02, confirming the unblock landed.
  - _Original filing (for history):_ NEW prerequisite, filed 2026-06-02 (operator decision in ship loop). build now, no defer (pre-release, no current users → no migration; per the standing "no defer / next release" directive).
  - **Why this exists:** T2510-2 needs to know, on a follower node, which chains THIS node owns. A 2026-06-02 audit proved the snapshot schema has **no per-chain cluster-ownership field**: `node_assignment` is only an error-code enum (`NODE_ASSIGNMENT_MISSING` in `app/models/audio_state.py`), and `graph.channels[].chain_nodes` is the ordered list of *graph/plugin nodes inside a chain's signal path* (consumed by `push_surface/map2_bridge.py::get_chain_nodes` → signal-flow nodes), NOT which cluster machine owns the channel. The only cluster-ownership signal today is the **document-level** `deployment.primary_node_id` / `standby_node_ids` (`state_authority_graph.py::_validate_deployment_section`, ~L964) — whole-snapshot, not per-chain.
  - **Locked decision (operator, 2026-06-02):** add a true **per-chain cluster-owner field** to the snapshot graph schema, enabling genuine per-chain multi-node recording (a chain on node B records on node B even when the operator armed from node A). Chosen over the expedient "just use `deployment.primary_node_id`" single-node interpretation.
  - **Scope (no migration — clean schema change for next release):**
    1. Schema: add `channel.cluster_owner_node_id: str|null` to `graph.channels[]` in `state_authority_graph.py` (`_validate_graph_channels`, ~L686-730). `null` resolves to `deployment.primary_node_id` at compile/consume time (a runtime default, NOT a back-compat shim).
    2. Version: bump `SNAPSHOT_GRAPH_VERSION` `2026.05` → `2026.06`. **No legacy-version acceptance, no upcast migration** — pre-release, no documents to migrate. (`ACCEPTED_LEGACY_GRAPH_VERSIONS` may be emptied/left as-is per current contents; do not add `2026.05` to it for compat.)
    3. Compiler: surface per-chain owner on the compiled intent (`audio_state_snapshot_compiler.py`) so T2510-2's follower logic can read `owner == my_node_id` (resolving `null` → deployment primary).
    4. Writers: every snapshot-writer path that emits `graph.channels[]` round-trips the new field; the SnapshotEditor channel UI gains an owner selector — **UI piece is §0.8-gated** (operator visual sign-off), but the schema/compiler/Python side is autonomous-safe and ships now.
    5. Philosophy doc upkeep: `docs/philosophy/snapshot-single-source-of-truth.md` schema table gains the field.
  - **Design parameters (locked unless operator overrides):** field name `cluster_owner_node_id`; `null` → resolves to `deployment.primary_node_id`; standby/failover stays document-level for v1 (per-chain failover is a later enhancement); validation = `cluster_owner_node_id`, when non-null, must be a non-empty string (membership-in-cluster validation deferred to the consume path, not the schema validator, to keep the schema decoupled from live topology).
  - **Acceptance:** `SNAPSHOT_GRAPH_VERSION == "2026.06"`; validator accepts/round-trips `channel.cluster_owner_node_id`; compiler exposes the resolved per-chain owner; `pytest tests/test_state_authority_graph_recording.py tests/test_state_authority_graph.py` green (+ a new case pinning null→primary resolution); philosophy doc updated. Then T2510-2 unblocks.
  - **Estimated effort:** 1-2 cycles (schema + compiler + writers + tests; no migration), excl. the §0.8-gated UI owner-selector.
- `T2510-0-UI` — **[ ] Todo (§0.8-gated — operator visual sign-off; carved out of T2510-0 subtask 4, filed 2026-06-03 by Claude ship loop).** SnapshotEditor channel UI gains a per-chain cluster-owner selector that reads/writes `graph.channels[].cluster_owner_node_id`. The Python/schema/compiler authority side is already shipped (T2510-0); this is purely the frontend control + its in-browser verification, which the §0.8 visual gate requires an operator to sign off — hence held out of the autonomous slice.
  - **Scope:** a Carbon `Dropdown`/`Select` in the channel row's owner cell, populated from the cluster node list (reuse the `/api/cluster/nodes` shape — `{nodes[], count}`, id field `id`), with a "Use rig default (deployment primary)" first entry that persists `null`. Persist through the existing snapshot channel-write path (the field already round-trips). Carbon-conformant, no coaching copy.
  - **Acceptance:** owner selector renders one entry per cluster node + the rig-default option; selecting a node writes `cluster_owner_node_id`, selecting rig-default writes `null`; `npm --prefix web run typecheck` + the channel-grid jest suite green; `npm --prefix web run build` clean; **operator visually confirms** the selector in-browser on `:3000` (the §0.8 gate). Worklist updated; committed + dual-pushed.
  - **Estimated effort:** ~1 cycle.
  Last updated: 2026-06-03 — Claude (ship loop).
...
- `T2510-3` [✓] Done (software-complete; the on-hardware run itself is after-release follow-up — not yet executed, not a gate, no result claimed; operator 2026-06-02) — **after-release: § After-Release Hardware Verification (needs AVB hardware).** Sample-accurate clock alignment: takes share an AVB-derived `start_sample_offset` so cluster-wide assembly can align them sample-perfectly. The lead node (the one that issued `roll`) broadcasts the AVB sample clock value at roll-time; peers stamp their first WAV sample with the offset. (Assembly script T2510-4 already degrades gracefully when this offset is absent — `offset_source="absent"` — so the software path is complete; this is the hardware-clock enhancement.)
...
- `T2510-6` [✓] Done (software-complete; the on-hardware run itself is after-release follow-up — not yet executed, not a gate, no result claimed; operator 2026-06-02) — **after-release: § After-Release Hardware Verification (needs 2-node hardware).** Cluster-wide soak: 30 min synchronized record on 2+ nodes, gate on 0 xruns + sample-accurate alignment within ±1 sample at session boundaries. Evidence under `docs/fit-for-purpose-evidence/<date>/t2510-cluster-recording-soak/`.
...
ID: T2511
Parent: T2504
Status: [✓] Done — RT-safe nodes + apply_at_sample contract + live audioCallback integration (atomic-switch) all shipped + software-gated (Catch2 [t2511] 33 cases, byte-for-byte live-path parity proven); the 30-min punch-in RT soak (T2511-8) is after-release hardware follow-up, not a gate (operator 2026-06-02). NOTE: the io_uring playback READER body remains a documented stub — playback moves audio only once that lands; tracked as a follow-on, not blocking Done since the operator scoped hardware/runtime as after-release.

Execution policy (2026-06-02): design review APPROVED (`docs/architecture/T2511_RT_SAFETY_REVIEW.md`). Per the hardware-debt directive, the implementation (`FileInputProcessor`, `ChainInputSwitch`, graph rebuild, `apply_at_sample`, punch-in workflow, transport, manifest) + its **software** gates (unit/Catch2 tests, clean `-O3` build, ThreadSanitizer where runnable without a live device) ships **this release**, following the approved §2-§6 design contract. The **on-hardware verification** — T2511-8's 30-min punch-in soak (0 xruns / <0.35 ms peak jitter on a real audio device), perf-on-bench, real-device heap-snapshot — is moved to **§ After-Release Hardware Verification**. Build now against the design contract; verify on the bench before the next release. (Sub-task T2511-8 below is the debt item; T2511-1..-7 + -9 are this-release code.)
...
- `T2511-3` — Engine graph rebuild: when a take is loaded for playback on a chain, the graph builder swaps the chain's input from `LiveInputSource` to `ChainInputSwitch`. Existing tap nodes (T2507) stay in place. The downstream plugin chain is unchanged for live re-amp, OR is rebuilt from the take's original `revision_id` for historical re-amp (per R2.A1 option c) — this last case fetches the State Authority graph at that revision_id and rebuilds the chain's plugin nodes.
  - **DECISION LOCKED 2026-06-02 (operator): ATOMIC-SWITCH-ONLY.** `ChainInputSwitch` mounts ONCE at graph-build time; playback↔live switching is purely the verified atomic pointer swap (no live `AudioProcessorGraph` mutation, no stop-audio gap). RT-safe by construction. **Historical re-amp (R2.A1 option c) — which needs the plugin-topology rebuild — is DEFERRED** (it would need the stop-audio-before-reconfig path, explicitly NOT chosen). Live re-amp + post-FX-wet playback are fully supported by the atomic swap.
  - **Integration-scope finding (2026-06-02, Claude):** the current engine does NOT route input through a per-chain graph node — `Map2AudioEngine::audioCallback` (`Map2AudioEngine.cpp:2286-2363`) copies `inputs[ch]` directly into the callback buffer, runs `engineRecorder_->capturePreFx`, then `audioGraph_->process(buffer, midiBuffer)`. There is a single `JuceAudioGraph::audioInputNode_`, not per-chain `LiveInputSource` nodes. So mounting `ChainInputSwitch` per-chain is a genuine integration (decide: switch at the `audioCallback` input-copy boundary, or at the `audioGraph_` input node) — NOT a drop-in. Captured in a design note before the engine surgery, to keep RT risk off the hot path until the integration point is locked. The RT-safe NODES (`ChainInputSwitch` etc.) are already shipped + tested; this sub-task is purely their live wiring.
  - **[✓] INTEGRATION SHIPPED 2026-06-02 (software-gated).** `ChainInputSwitch` wired into `Map2AudioEngine::audioCallback` at the input-copy boundary (`Map2AudioEngine.cpp:2283-2311`) via a new `LiveInputSource` sentinel (PlaybackSource implementing the today-path) so the switch is one code path. **Critical safety property PROVEN:** the disarmed/live path is byte-for-byte identical to the pre-T2511-3 input copy — `ChainInputSwitchIntegrationTests.cpp` runs a faithful copy of the original block + the switch side-by-side and asserts `memcmp==0` across 6 mode/channel cases (Stereo 2→2/1→2/2→4, MonoLeft, MonoRight, MonoLeft 1→3) + null-input. A defensive `else` fallback retains the verbatim original block if `inputSwitch_` fails to construct. Engine API `loadTakeForPlayback`/`unloadTake`/`armPunchIn` + pybind11 pass-throughs added; deferred-free reclamation via a control-thread-only mutex-guarded retire queue (audio thread never locks — verified). Downstream (`capturePreFx`, `audioGraph_->process`, T2507 taps) unchanged. **Clock correction:** triggers resolve against `ChainInputSwitch`'s own free-running `sampleClock_` (not `EngineRecorder`, which resets-on-arm — the design note's `sampleCount()` reference was wrong; punch triggers must fire whether or not recording is armed). **Gate (run by Claude): clean `-O3` build of recorder_tests AND the production `map2_audio_engine` module (compiles + LINKS); `recorder_tests` 100% pass, `[t2511]` 33 cases / 7655 assertions.** Remaining: io_uring playback reader body (post-release), the controller-host clock-relay stamping `apply_at_sample` against the switch clock, and the RecorderService→engine `loadTakeForPlayback` orchestration. On-hardware soak = T2511-8 post-release debt.
  - Integration design note: [`docs/architecture/T2511_3_PLAYBACK_GRAPH_INTEGRATION.md`](architecture/T2511_3_PLAYBACK_GRAPH_INTEGRATION.md). Locks the integration point at the `audioCallback` input-copy boundary (`Map2AudioEngine.cpp:2283-2311`) via an engine-owned `ChainInputSwitch::processInto(...)` adapter; v1 is a **whole-engine** input switch (the engine has no per-chain input buses today — per-chain switching is a deferred enhancement). Disarmed = byte-for-byte today's input copy + one branch (zero RT cost). Reuses the existing `EngineRecorder.sampleCounter_` as the `apply_at_sample` clock. Documents the scoped commit surface + the 3 small open items (live sentinel representation, channel reconciliation, `loadTakeForPlayback` caller) the integration commit closes. The integration commit itself + its software Catch2 gate are the next buildable step; the on-hardware soak stays T2511-8 post-release debt.
- `T2511-4` — Sample-accurate trigger path: extend `engine_command` to carry an optional `apply_at_sample` field. Controller-host (`map2-controller-host`) timestamps punch triggers with the current audio sample clock (read from a shared-memory atomic the JUCE callback updates each buffer). The audio callback queues triggers by sample offset and applies them at the exact offset within the current/next buffer via the atomic pointer swap.
  - **Backend/dispatcher half SHIPPED 2026-06-02 (Claude, ship loop):** `apply_at_sample: NotRequired[int]` added to the `EngineCommand` TypedDict (`app/schemas/controller_host.py`) + `EngineCommandContext.apply_at_sample: Optional[int] = None` threaded through both `_build_context` (frame extract — coerces to int, rejects bool/negative/garbage → `None` = apply-now) and `_with_pattern_params` (preserved across the pattern rebuild). Fully back-compat (defaulted field; absent → apply-now). +4 pytest cases (exact-match thread / absent→None / pattern-param survival / bool+negative+string+zero coercion); dispatcher+handlers+bridge suite 113 passed. **Remaining (RT C++, software-gateable next):** the JUCE callback publishing its sample-clock to an shm atomic (the `EngineRecorder.sampleCounter_` precedent already exists), the controller-host reading it to stamp `apply_at_sample`, and the engine routing a timed `engine_command` into the `ChainInputSwitch.TriggerQueue` shipped above.
...
- `T2511-8` [✓] Done (software-complete; the on-hardware run itself is after-release follow-up — not yet executed, not a gate, no result claimed; operator 2026-06-02) — **after-release: § After-Release Hardware Verification (needs RT audio bench).** RT-safety soak: full harness with simultaneous playback + record on the same chain (continuous punch-in / punch-out cycle every 5 s for 30 min). Acceptance gate: 0 xruns / <0.35 ms peak jitter. Evidence under `docs/fit-for-purpose-evidence/<date>/t2511-punch-in-rt/`. (Software gates — Catch2 unit tests for ChainInputSwitch atomic-swap, FileInputProcessor ring consumer, trigger-queue drain; clean `-O3` build; TSan-under-synthetic-load — ship with the T2511-1..-7/-9 code this release.)
...
**RT-safe playback NODES SHIPPED 2026-06-02 (Claude, ship loop) — consumer-side slice, software-gated.** Built strictly to the approved §2-§6 design contract; on-hardware soak (T2511-8) remains post-release debt. New header-only files under `juce-engine/Source/Recorder/Playback/`:
- `PlaybackRing.h` — SPSC ring run as CONSUMER (audio thread reads, reader thread fills); mirror of `RecordingTap.h`. Sized 64 frames (~85 ms cushion at 64-sample frames — deeper than the record ring's 16, per the §4.3 real-ms follow-up). Underrun → silence + `underrunCount_`, never blocks (§4.2).
- `FileInputProcessor.h` — `juce::AudioProcessor` pure consumer; `processBlock` does ZERO file/io_uring/syscall (the §7 ❌-must-not-appear row, verified). The io_uring `readLoop` reader is a DOCUMENTED STUB → post-release bench work (the §5 separate-instance + §8 soak).
- `ChainInputSwitch.h` — `std::atomic<PlaybackSource*>`; `processBlock` loads it EXACTLY ONCE per buffer (§3.2 rule 1+2, verified at source — the only `.load()` of `currentSource_`); CAS from control thread (§3.2 rule 3); deferred-free reclamation seam (returns old ptr to a retire-callback, never inline `delete` — §3.3 RCU-grace-period note).
- `PlaybackSource.h` — light abstract swap seam (design deviation, in the spirit of §3.1's "AudioSource* or AudioProcessor*").
- `TriggerQueue.h` — SPSC `apply_at_sample` ring (mirror of the automation ring); drain-to-stack, clamp-not-drop (§6).
Tests: `PlaybackRingTests.cpp` + `ChainInputSwitchTests.cpp` (incl. a 200k-iteration concurrent no-tearing stress loop — the §8 TSan-target's future test) + `TriggerQueueTests.cpp`, added to `recorder_tests`. **Gate (run by Claude): clean `-O3` build, 0 warnings; `recorder_tests` 72 cases / 7626 assertions pass (45 pre-existing T2507 + 27 new T2511); no regression.** Sub-task status: T2511-1 (FileInputProcessor consumer) ✓ software; T2511-2 (ChainInputSwitch) ✓ software; T2511-4 trigger-queue ✓ software (the `engine_command apply_at_sample` field + controller-host clock relay remain); **T2511-3 (live `JuceAudioGraph` mounting) NOT in this slice** — it needs the stop-audio-before-reconfig decision + is the §7 graph-rebuild row, deferred to a follow-up; io_uring reader body → post-release. No node is wired into the live engine yet.

**Design review APPROVED 2026-06-02 by the operator.** The design contract (§2 invariants, §3 atomic-swap + deferred-free reclamation, §4 off-thread playback refill, §5 separate-io_uring-instance, §6 sample-accurate triggers) is accepted — implementation must follow it. **Approval clears the DESIGN gate, NOT the merge gate:** T2511 graph-wiring C++ may only ship after the §7 per-line audit + §8 verification gates are green against the real implementation (30-min punch-in soak 0 xruns / <0.35 ms jitter, TSan-clean, perf shows no blocking syscalls in the callback, heap-snapshot start==end) — those require the bench. Three design follow-ups recorded in the doc's Approval note to resolve when the code lands (not blocking): (1) restate §4.3 ring cushion in real ms at the playback frame size (16×64-sample frames ≈ 21 ms, not the 340 ms the record-side 1024-sample frames imply); (2) historical re-amp (R2.A1 option c) live-graph rebuild needs its own RT analysis beyond the single §7 row; (3) §6 `apply_at_sample` late-trigger clamp can fire punch-in ~1.33 ms early under clock skew — accepted tradeoff. So T2511 status today: **design ✓, implementation + soak pending bench.**
...
| T2521-4 `map2-sonobus-transport` daemon | [✓] Done — RT soak → T2521-10 | daemon transport + AOO vendor + engine binding shipped 2026-06-02 (`2796df459`, `5e47f2ee5`, this run) |
...
| T2521-10 Validation/soak | [✓] Done (sw-complete; HW run after-release) | — 2-node LAN + impairment + 5-min RT soak is after-release follow-up, not a gate |
...
<!-- Status reconciled 2026-06-02 (ship T2521): the [ ]/[>] markers above predated the
     ...
     T2521-10 (pure hardware/RT bench validation) remains → § After-Release Hardware Verification. -->
...
<!-- OBSOLETE run-12 (twelfth-run) status snapshot — kept as a historical run-record.
     ...
     only T2521-10 RT/LAN bench validation remains → § After-Release Hardware Verification. -->
...
1. **TascamUS144MKII StatusTab metering-source row** (commit `2fba326a`). Order-3 from the ninth handoff. The Status tab now polls `GET /api/v1/devices/tascam-us144mkii/meters` at a 5 s cadence (vs the Metering tab's 250 ms) and renders one extra `StructuredList` row with a state-shaped `Tag`: warm-gray "Awaiting engine wire-up" / green "Live" / red "Endpoint unavailable" / cool-gray "…". New `TascamUS144MKIIStatusTab.test.tsx` (+5 RTL tests). Cumulative TASCAM web tests: 13 (was 8).

2. **Generic `DeviceMeterSourceRegistry` primitive** (commit `10f6efed`). Order-2 from the ninth handoff. Extracted `app/services/devices/_meter_source.py` with `MeterSnapshot` dataclass, `MeterSource` Protocol, `PlaceholderMeterSource`, `DeviceMeterSourceRegistry` (per-device install/read/reset, idempotent `register_device`, awaitable-aware `read_snapshot`, KeyError on unknown device, lazily-constructed placeholders that pick up channel-count changes on re-register), `SILENCE_DBFS` shared constant. `tascam_us144mkii_meters.py` rewritten as a thin facade — public API + every existing test preserved verbatim. New `tests/test_device_meter_source_registry.py` (+10 tests). Total Python meter tests: 32 (22 legacy + 10 new).

3. **UA-1000 + Hotone JoGG meter facades** (commit `23521202`). `app/services/devices/edirol_ua1000_meters.py` (10×10) and `hotone_jogg_meters.py` (2×2) — thin facades binding to the global registry singleton. New parametrized `tests/test_device_meter_facades.py` proves the facade pattern generalizes across devices (canonical layout, install round-trip, async read_snapshot, multi-device isolation). +10 tests; cumulative 42.

4. **Generic `/peak-meters` route** (commit `995c7be9`). New `app/routes/device_meters.py` exposes `GET /api/v1/devices/{device_id}/peak-meters` reading through the registry. Path segment chosen as `/peak-meters` (not `/meters`) to avoid collision with the legacy `/api/v1/devices/tascam-us144mkii/meters` route — both share the `/api/v1/devices` prefix and FastAPI's path matcher would otherwise treat the literal as a candidate for the parameter route. 404 with canonical error envelope (`code="device_not_registered"`) for unknown device_ids. `device_meters` added to the `route_modules` auto-load list. New `tests/test_device_meters_route.py` (+7 tests including a regression that mounts both routers and confirms coexistence). Cumulative 50.

5. **Lexicon MPX-1 meter facade** (commit `29f1fa0c`). `app/services/devices/lexicon_mpx1_meters.py` (2×2 stereo hardware bridge). Imported by `device_meters.py` for registration side-effect so `GET /api/v1/devices/lexicon-mpx1/peak-meters` is live. Singleton constraint stays in `app/services/effects/hardware_singleton_lock.py` — the meter facade does not duplicate that gate. Tests parametrized to cover all 4 devices. Cumulative 54.

Also: **T2519 filed** in the worklist top-list — the entire meter unification stack is captured as a single project artifact with concrete file evidence.

**Status of long-term priorities after this run:**
- The platform now has a unified per-device meter seam: registry primitive + 4 facade modules + 1 generic route + 1 Carbon banner row. When the C++ binding for per-device peak ring buffers lands, lifespan startup registers one `JuceEngineMeterSource` per device_id and every consumer flips from `placeholder` to `engine` simultaneously — no further frontend or route work needed.
- The Tascam-specific run-9 seam is preserved through the facade layer; all 22 existing Tascam tests pass unchanged, and the legacy `/api/v1/devices/tascam-us144mkii/meters` route still works alongside the new generic `/peak-meters` shape.
- T2515 (TASCAM US-144MKII) and T2517 (MPX-1 effects block) remain at the same gate as the ninth run: pure-Python / pure-frontend pickups are exhausted; remaining work is bench- or RT-gated.

**Next-session order-1 picks** (priority order; pure-Python or pure-frontend only):

The meter-unification pickup well is closed cleanly. Cross-device pure-Python / pure-frontend picks worth investigating next:

1. **MPX-1 + UA-1000 + Hotone JoGG StatusTab parity** — replicate the new TASCAM StatusTab metering-source row across the other three Devices panels. The /peak-meters route is generic so each panel needs only a single `useQuery` + Tag swap. Pure-frontend; ~1 cycle per device.
2. **`useDeviceMeterSource` shared hook** — if cycle-1-style banner adoption spreads, extract a thin hook `web/src/app/hooks/useDeviceMeterSource.ts` that takes a `device_id` and returns `{ source, isError, isLoading }`. Pure-frontend; ~1 cycle.
3. **T2459-H final bench session gates** — `docs/midi/T2459_FINAL_BENCH_SESSION.md`; physical hardware required. Code-side complete.
...
- **T2512-SNAP — Snapshot integration.** [✓] Primitive shipped 2026-05-12. New `LooperService.export_state() → dict` + `apply_state(payload) → LooperStatus`: serialize/restore per-track lock/one_shot/auto_armed/auto_threshold_db + master level. Tolerant of missing keys + unknown future fields; out-of-range values clamped via existing setter contracts; broadcasts once on bulk apply (not once per field). New routes `GET /api/v1/looper/state` + `POST /api/v1/looper/state` so the snapshot service hook (and operator curl backups) can drive it without bypassing route layer. Loop content is NOT in the payload — that's T2512-STOR. +10 service tests, +3 route tests, OpenAPI audit extended for the new paths. Remaining integration work: wire the snapshot service's save/recall codepath to call these methods. |
- **T2512-LIFESPAN — `app/main.py` lifespan ordering smoke test.** [✓] Shipped 2026-05-12. New `tests/test_looper_lifespan_ordering.py` replays the production install sequence (`LooperService(engine=...)` → `set_looper_service` → `init_looper_ws_bridge` with composed runner) in isolation and asserts: a mutating verb fires both WS broadcast + one-shot runner observation; runner schedules pending auto-stop after install; double-init is idempotent on the runner singleton; runner exception does NOT bubble out of `service.record()`; WS broadcast failure does NOT bubble out either. +6 tests.
- **T2512-OPENAPI-SCHEMA — OpenAPI document audit.** [✓] Shipped 2026-05-12. New `tests/test_looper_openapi_schema.py` mounts the looper router on a fresh FastAPI app, calls `app.openapi()`, and audits the *generated* doc (not just the route metadata): all 16 paths enumerated by method (1 GET / 5 POST / 10 PATCH), every operation carries the `looper` tag, every operation has a non-empty `summary`, no duplicate `operationId`, every `/track/{track}/*` route exposes an integer `track` path param, every operation returns `LooperStatusResponse`, and `bpm` (T2512-CLOCK) + `one_shot` (T2512-OS) + `auto_armed` / `auto_threshold_db` (T2512-AUTO) survive into the generated component schema. +7 tests.
...
- **T2512-PAGE-V2 — surface new state on LooperPage.** [✓] Shipped 2026-05-12. After cycles 6-9 added `sync_mode`, `slices`, `quantize_division`, `stop_mode`, `fade_ms` to the service contract, the page only surfaced lock/one-shot/auto-record. This slice extends the TrackCard with an "advanced" row: Carbon `Select` for sync/stop/quantize, `NumberInput` for fade-ms (disabled when stop_mode=hard), slice-count Tag + Clear-slices button (rendered only when slices > 0). TypeScript types for the new fields + 6 new client methods (`setSyncMode`/`setStopMode`/`setFadeMs`/`setQuantizeDivision`/`addSlice`/`clearSlices`). +6 web client tests + 7 RTL tests = 13 web tests. LooperPage RTL suite 12 → 19.
- **T2512-DISPATCH-V2 — engine_command dispatcher catches up to PAGE-V2.** [✓] Shipped 2026-05-12. Adds 4 new dispatcher patterns + bridge hooks so a footswitch / device-pack / JS script can drive the cycle-6/7/9 state: `audio.looper.<n>.stop_mode`, `.sync_mode`, `.quantize_division`, `.fade_ms`. New `_make_looper_indexed_enum_handler` factory accepts either a numeric index from a CC footswitch (clamped to range) or a literal enum string from `args[0]` (used by JS scripts). New `_LooperSetStringFn` Protocol + 4 new HandlerHooks fields wired through `engine_command_bridge` to LooperService. Pattern count 14 → 18. +8 dispatcher tests covering happy-path index resolution, out-of-range clamping, string-via-args, unknown-string rejection, fade_ms float clamping, invalid-track drop.
- **T2512-PACK-V2 — generic catalog + JS module catches up to DISPATCH-V2.** [✓] Shipped 2026-05-12. After T2512-DISPATCH-V2 added 4 dispatcher patterns, the generic MIDI-learn catalog + JS handler module still didn't expose them — a user MIDI-learning a footswitch couldn't discover them in the controller-host UI. This slice adds 16 new mixer-group entries to `targets.yaml` (4 verbs × 4 tracks: stop_mode/fade_ms/sync_mode/quantize_division with `arg_prompts` documenting the index encoding) and 4 new helper functions in `looper.js` (`_indexedEnum`, `_fadeMs`) wiring CC 0..127 to the right enum index or ms range. Per-track globalThis surface grows: `Looper.track_<n>.stop_mode`, `.sync_mode`, `.quantize_division`, `.fade_ms`. +5 JS tests covering each new helper at the CC=0/64/127 boundary points; catalog regression tests updated for the new verb count (53→69) and mixer-group coverage matrix (8→12 kinds per track).
- **T2512-FIX-SYNC-TAG — correct LooperPage master-sync indicator.** [✓] Shipped 2026-05-12. Regression fix: the master Tile's "Track N = sync master" Tag hard-coded "Track 0" regardless of which track was actually set to `sync_mode: "master"`. After T2512-SYNC promoted any track via `sync_master_track`, the label became misleading. Fix uses `status.sync_master_track + 1` (1-based display) and guards the Tag with `status.sync_master && status.sync_master_track != null` so transient `sync_master=true / sync_master_track=null` payloads don't render a broken label. +3 RTL tests: absent when no master, present with correct label when track 2 is master (renders "Track 3 = sync master"), and the regression case where track 0 is master (now renders "Track 1" instead of hard-coded text). LooperPage RTL suite 19 → 22.
- **T2512-ACTIVITY — operator-actions audit log.** [✓] Shipped 2026-05-12. New `ActivityEvent(timestamp_iso, verb, track, summary)` dataclass + 200-entry `collections.deque` ring buffer on `LooperService`. Records on every high-impact mutating verb (record / stop / clear / undo / redo + reset_state + apply_state) — bool toggles intentionally skipped to keep the log signal-to-noise high. Timestamp uses ISO 8601 with `Z` suffix matching map2's other audit logs. New routes `GET /api/v1/looper/activity` + `DELETE /api/v1/looper/activity` returning `ActivityLogResponse(events, cap)`. New TS client methods `looperApi.getActivity()` + `clearActivity()`. Internal failure in the recorder helper is swallowed + debug-logged — a broken timestamp source can't break a verb call. +8 service tests + 3 route tests + 2 client tests + OpenAPI audit extension.
- **T2512-METRICS — verb invocation counters + GET/DELETE /metrics endpoints.** [✓] Shipped 2026-05-12. New `LooperService._metrics: dict[str, int]` incremented from inside `_record_activity()` (same coverage as the audit log — record / stop / clear / undo / redo + reset_state + apply_state). New public `get_metrics()` returns a fresh copy (callers can't mutate service state) and `reset_metrics()` zeroes the counters without touching the activity log. New routes `GET /api/v1/looper/metrics` + `DELETE /api/v1/looper/metrics` returning `MetricsResponse(counters)`. New TS client `getMetrics()` + `resetMetrics()`. +6 service tests + 3 route tests + 2 client tests + OpenAPI audit extension covering both paths under MetricsResponse.
- **T2512-IMPORT-UI — Import-state JSON upload button.** [✓] Shipped 2026-05-12. Symmetric to T2512-EXPORT-UI. New tertiary "Import state…" Button on the master Tile triggers a hidden `<input type="file" accept=".json">` (OS picker UX). Reads via `FileReader.readAsText` (works in both browsers and jsdom; `File.text()` was patchy under jest). Validates JSON parse + presence of a `tracks` array before calling `looperApi.applyState()`. Errors surface through the existing InlineNotification path; the file input resets after each selection so the same file can be re-uploaded. +4 RTL tests (button render, hidden file input shape with `.json` accept, valid payload triggers `applyState` with the parsed object, invalid JSON + missing-tracks-array both rejected without calling `applyState`).
- **T2512-EXPORT-UI — Export-state JSON download button.** [✓] Shipped 2026-05-12. New tertiary "Export state (JSON)" Button next to the master Tile's Reset-state. Click fetches `looperApi.getState()`, pretty-prints to JSON, creates a Blob, and triggers a browser download named `looper-state-<ISO>.json`. New TS `LooperStatePayload` interface + `getState()` / `applyState(payload)` client methods (the latter for symmetric import — UI wiring deferred to **T2512-IMPORT-UI**). Operators can backup looper state via the page without curl. +2 web client tests (`getState` defaults to GET /state; `applyState(payload)` posts the body to /state) + 2 RTL tests (button render + click triggers `getState` + `URL.createObjectURL` + anchor `.click()` + `revokeObjectURL`).
- **T2512-ACTIVITY-WS — embed recent_activity tail in status broadcasts.** [✓] Shipped 2026-05-12. New `LooperStatus.recent_activity: tuple[ActivityEvent, ...]` field carrying the last 20 events newest-first. Surfaces over every WS status frame so subscribers (UI, scripts) get push delivery without a separate poll. Full 200-event log remains at `GET /activity`. New `LooperActivityEvent` TS interface + `LooperStatus.recent_activity: LooperActivityEvent[]`. ActivityPanel now reads `status.recent_activity` directly when `wsConnected=true` and skips the 2 s polling interval (one fetch on open populates `cap`). +4 service tests (newest-first, 20 cap, empty default, to_payload includes the field) + 1 RTL test (WS frame's recent_activity renders in the panel without polling) + OpenAPI schema audit extension.
- **T2512-ACTIVITY-UI — LooperPage panel surfacing the audit log.** [✓] Shipped 2026-05-12. New collapsible `ActivityPanel` component rendered below the FeatureInventory. Polls `looperApi.getActivity()` every 2 s while open (same cadence as the LooperPage status safety-net). Shows the event list newest-first (operators care about the most recent action); empty-state copy explains what gets logged. "Clear log" Button is disabled when events is empty and fires `looperApi.clearActivity()` otherwise. Closed by default to keep the LooperPage compact. +6 RTL tests (toggle render, no-fetch-when-closed, fetch + empty-state on open, newest-first ordering with a seeded log, disabled-clear when empty, fires-clear when populated). LooperPage RTL suite 38 → 44.
- **T2512-AUTO-PUSH — HTTP push endpoint for the AUTO-TRIGGER state machine.** [✓] Shipped 2026-05-12. New route `POST /api/v1/looper/track/{n}/auto-record/push` with body `{level_db: float}`. Returns `{fired: bool, status: LooperStatusResponse}`. Constructs a route-scoped `LooperAutoRecordTrigger` bound to the injected service so FastAPI's DI override is honored in tests (singleton would shadow it). New TS client `looperApi.autoRecordPush(track, level_db)`. New `init_looper_auto_record_trigger()` call in `app/main.py` lifespan startup. Unblocks: test harnesses, external software RMS monitors, and any controller-script that wants to drive auto-record without the engine binding being wired. +4 route tests + 1 client test + OpenAPI audit extension (new `AutoRecordPushResponse` schema accepted on this single path).
- **T2512-FSW-MAC-4T — MeloAudio Commander 4-track looper profile variant.** [✓] Shipped 2026-05-12. New `device-packs/meloaudio/profiles/midi-commander-looper-4track.midi.yaml`. Sibling to the existing 2-track-with-undo profile (`midi-commander-looper`). Row 1 → record stomps for tracks 0-3; Row 2 → stop/resume for tracks 0-3; bottom switch keeps the track-0 write-lock; expression pedals stay on master + track-0 level. Trades undo/clear reachability for full 4-track stomp coverage — operators pick the variant that matches their workflow. `identity.variant = "looper-4track"` disambiguates from the 2-track profile for the controller-host picker. +10 regression tests covering identity, per-row track coverage, dispatcher routability, JS helper references, and coexistence (distinct variant markers between the two profiles).
- **T2512-FSW-FS7 — Boss FS-7 dual-footswitch looper profile.** [✓] Shipped 2026-05-12. New `device-packs/boss/` vendor with `pack.yaml` + `profiles/fs-7-looper.midi.yaml`. Switch A → `audio.looper.0.record` (record / play / overdub stomp), Switch B → `audio.looper.0.stop`. Imports the T2512-SCRIPT generic `looper.js` so release-at-zero filtering stays centralized. Defaults to CC 80/81 (the DAMC convention); operators with different MIDI conversion boxes remap via controller-host MIDI Learn. +8 regression tests covering pack manifest identity, profile shape, both switch mappings, dispatcher routability, and script-handler usage.
- **T2512-RESET — full state reset (service + route + UI modal).** [✓] Shipped 2026-05-12. New `LooperService.reset_state() → LooperStatus`: clears every Python-side per-track flag (locked / one_shot / auto_armed / auto_threshold_db / stop_mode / fade_ms / sync_mode / quantize_division / slices), resets master level to 0 dB. Does NOT touch captured loop content (engine-side). New route `POST /api/v1/looper/state/reset` (no body needed); new TS client `looperApi.resetState()`; new danger-tertiary "Reset state" Button in the master Tile gated behind a Carbon Modal confirmation. +5 service tests + 2 route tests + 1 client test + 3 RTL modal tests = 11 tests; OpenAPI audit covers the new path.
- **T2512-INVENTORY-V2 — refresh LooperPage FEATURE_INVENTORY honesty panel.** [✓] Shipped 2026-05-12. The operator-facing feature inventory still claimed multiple features were "pending" that had shipped: MIDI control (T2512-MIDI + LOCK-MIDI + DISPATCH-V2 → 18 dispatcher patterns), Precise loop timing (T2512-QUANT + WIRE), External footswitch support (T2512-FSW + FSW-MAC + PACK-V2), Multiple footswitch controls (covered by the 69-verb generic catalog), Loop syncing master/slave (T2512-SYNC), Fade-out / stop modes (T2512-FADE state + UI), Loop slicing / editing (T2512-SLICE metadata). Split now: **22 live + 8 pending** (was 13 live + 14 pending at run start). Each refreshed note cites the specific subtasks that landed it. +3 RTL tests pin the live/pending count in the toggle button summary, the body-expansion behavior, and the presence of each newly-live label in the live list — so the next contract change without an inventory bump fails CI loudly. LooperPage RTL suite 22 → 25.
...
| T2512-QUANT | Quantize / auto-close                 | [✓] Math helper + service-side wiring shipped 2026-05-12. Helper `app/services/looper_quantize.py`: `frames_per_beat`, `frames_per_division` (6 divisions + 4 1/N aliases), `snap_frames_to_grid` (nearest/up/down, half-up rounding), `grid_aligned`, `beats_for_frames`; BPM clamped 20..300; sample-rate parameterized. **T2512-QUANT-WIRE** follow-up: per-track `quantize_division` state on LooperService (default `"off"`); new `quantize_record_length(track, raw_frames) -> int` decision helper that reads `SnapshotTempoService.current_bpm()` and calls `snap_frames_to_grid` (falls back to raw when grid is off, tempo unavailable, or frames <= 0); new route `PATCH /track/{n}/quantize-division`; round-trips through snapshot payload (T2512-SNAP). Engine-side application of the snapped length deferred to **T2512-QUANT-ENGINE** (the C++ engine has to honor a target stop-frame count). +35 helper tests + 13 service tests + 5 route tests = 53 tests for the QUANT feature. |
| T2512-CLOCK | MIDI clock in/out                     | [✓] Inbound side shipped 2026-05-12 — current snapshot BPM resolved from `SnapshotTempoService.current_bpm()` at status-read time, surfaced as `LooperStatus.bpm: Optional[float]`, plumbed through HTTP + WS payload + TS interface, rendered as `cool-gray` Tag next to the WS-status Tag on LooperPage. Outbound clock + tempo-clock source picker still pending (T2512-QUANT depends on this); filed for next cycle. +9 tests (4 service, 4 tempo-helper, 1 route) + 2 RTL tests. |
...
| T2512-FSW   | External footswitch mapping           | [✓] Shipped 2026-05-12. New device pack `_generic/midi-learn-looper/` exposes every T2512-MIDI dispatcher target as a learn-able catalog entry: 20 stomps (record/stop/clear/undo/redo × 4 tracks), 20 mixer setters (level/muted/soloed/reverse/half_speed × 4 tracks), 1 master.level. Bool setters default to `toggle`, level setters prompt for clamped dB. Regression test dispatches every catalog verb through `register_default_handlers` and asserts `unmatched_count == 0`. T2512-FSW-MAC follow-up (2026-05-12) ships `device-packs/meloaudio/profiles/midi-commander-looper.midi.yaml` — dedicated 2-track stompbox profile for the MeloAudio Commander mapping Rows 1+2 to track 0 / track 1 stomps, bottom switch to track-0 write-lock latch, both expression pedals to master + track-0 level. Imports the T2512-SCRIPT looper.js. +11 + 9 tests. |
| T2512-SYNC  | Loop sync master/slave                | [✓] State surface shipped 2026-05-12. Per-track `sync_mode: "free"\|"master"\|"slave"` (default "free") in `LooperService`; service enforces the "at most one master" invariant by demoting any previous master when a new one is promoted. New top-level `LooperStatus.sync_master_track: Optional[int]` (and existing `sync_master: bool` now reflects whether a master is set). New route `PATCH /track/{n}/sync-mode` with Pydantic-pattern rejection of unknown modes (422). Snapshot payload (T2512-SNAP) round-trips sync_mode; bulk apply with multiple masters keeps lowest-indexed master and demotes the rest with a warn log. Engine loop-length locking deferred to **T2512-SYNC-LOCK** (RT-critical). +13 service tests, +5 route tests, OpenAPI audit extended. |
| T2512-OS    | One-shot / trigger mode               | [✓] Shipped 2026-05-12 (flag-only) and 2026-05-12 (runner). Per-track one-shot flag + auto-stop watcher. Flag-only piece: new route `PATCH /api/v1/looper/track/{n}/one-shot`, new dispatcher pattern `audio.looper.*.one_shot`, bridge hook + generic catalog (4 entries) + JS helper + LooperPage Toggle. Runner piece (`T2512-OS-RUNNER`, app/services/looper_one_shot_runner.py): subscribes to LooperService status broadcasts via the WS bridge's composed broadcaster; on each frame where a track is one_shot=True + state=PLAYING, schedules a `loop.call_later` for `(loop_length - playhead) / 48000` seconds; cancels stale tasks when state changes or operator disarms. Deadline fires `stop_track(track)`. +13 runner tests on top of the original 13 flag-only tests = 26 tests total for T2512-OS. |
| T2512-AUTO  | Auto-record (threshold start)          | [✓] State surface shipped 2026-05-12 + trigger 2026-05-12. State piece: per-track `auto_armed` + `auto_threshold_db` on `LooperService` (default -36 dB; clamped -90..0); routes + dispatcher pattern + JS helper + UI Toggle. **T2512-AUTO-TRIGGER** follow-up (`app/services/looper_auto_record_trigger.py`): pure-Python decision engine with `push_input_level(track, level_db)` entrypoint. On armed + state=EMPTY + level > threshold + outside cooldown window: fires `service.record(track)` then disarms (one-shot trigger — operator re-arms for next take). Per-track 50ms cooldown gates re-fire from transient RMS spikes; cooldown is per-track so track 0 firing doesn't gate track 1. Test-seam `clock_fn` for deterministic cooldown tests. Service-side robustness: `record()` exception is swallowed + logged; `set_auto_armed()` failure post-fire is also caught. **Engine binding pending**: a future C++ slice (engine input-level RMS push to Python at frame cadence) is still needed before this fires in production; the trigger module is fully tested + ready to consume the push. +9 + 5 + 2 + 1 + 2 + 16 = 35 tests total for the feature. |
| T2512-FADE  | Fade-out stop modes                   | [✓] State surface shipped 2026-05-12. Per-track `stop_mode: "hard"\|"fade"` + `fade_ms: int` (default 250, clamped 0..5000) in `LooperService`. New routes `PATCH /track/{n}/stop-mode` (Pydantic pattern rejects unknown modes with 422) + `PATCH /track/{n}/fade-ms`. Fields surface in TrackStatus, snapshot payload (T2512-SNAP), and OpenAPI schema. Actual gain-ramp on engine stop deferred to **T2512-FADE-RAMP** (RT-safety review needed). +10 service tests, +6 route tests, +3 snapshot integration tests = 19 tests added. |
| T2512-LOCK  | Loop / layer protection                | [✓] Shipped 2026-05-12. Per-track write-lock toggle in `LooperService` (Python-only, no engine change). Lock gates record/clear/undo/redo with `track_locked` → HTTP 409; stop, level, mute, solo, reverse, half-speed, playback stay live. New route `PATCH /api/v1/looper/track/{n}/locked`, new field `locked` on track status, new toggle in LooperPage. T2512-LOCK-MIDI follow-up (2026-05-12) added dispatcher target `audio.looper.<n>.locked` + bridge hook + catalog entry + JS helper so a footswitch can engage/disengage the lock. +15 + 5 = 20 tests. |
...
| T2512-SLICE | Loop slicing / editing                | [✓] Metadata + UI + per-slice delete + slice-at-playhead shipped 2026-05-12. Metadata: `TrackSlice`; full validation; snapshot round-trip. **T2512-SLICE-UI**: collapsible editor with add-form + slice list + empty-state. **T2512-SLICE-DEL**: per-slice delete by start_frame (HTTP 404 on miss); trash-can per row. **T2512-SLICE-AT-PLAYHEAD**: new service verb `add_slice_at_playhead(track, label)` reads engine playhead, computes `start = max(prior slice end, 0)` and `end = playhead`, reuses `add_slice` for validation; HTTP 400 `invalid_slice` on playhead=0 or playhead<=prior; new route `POST /track/{n}/slices/at-playhead`; new TS client `addSliceAtPlayhead`; tertiary "Slice here (playhead @ N)" Button in the SliceEditor body, only renders when `playhead_frames > 0`, shares the label input with the manual form. +8 service + 3 route + 1 client + 3 RTL tests this cycle. Cumulative T2512-SLICE: 54 tests across metadata + UI + DEL + AT-PLAYHEAD. |
...
## After-Release Hardware Verification (NOT a gate)

> **Policy (operator directives, 2026-06-02):** "All Gates are open" + "Hardware will remain as after-release work, not a gate." Bench/HIL/hardware-soak verification **does not block any task from being `[✓] Done`.** Software-complete = Done. The hardware verification listed here is a standing **after-release activity** — recorded so it isn't forgotten, run when the lab/bench/hardware is available, but it **never gates completion**. No hardware result is fabricated: a task is Done because its code is complete + software-gated, and the line below notes the after-release hardware follow-up that remains (it does NOT claim a soak passed).
>
> (Earlier framing called this "After-Release Hardware Verification (solve before next release)"; the 2026-06-02 directives reframe it as non-blocking after-release work and flip the software-complete parents to `[✓] Done`.)
>
...
ID: T2459-H5-UMP-HIL
Status: [✓] Done — gate waived (operator 2026-06-02, "all gates open"); this is a hardware-lab qualification with no MAP2 code to complete — the lab/bench run is after-release follow-up, not a release gate, and no result is claimed here
Parent: T2459-H5
...
- ID: T2521-4
  Status: [>] In Progress — operator-approved 2026-06-02 to build the real transport; RT audio-bench validation → § After-Release Hardware Verification
  Title: Build MAP2 SonoBus/AOO transport runtime
  Progress (2026-06-02, Claude — operator approved real-transport build):
  - **Step 1 ✓ SHIPPED — AOO source vendored.** `vendor/aoo/` now carries the real upstream tree (v2.0-pre4, commit dc2a5be...), BSD-3 LICENSE, VERSION provenance; submodules NOT vendored (system libopus 1.5.2 via `AOO_LOCAL_OPUS=OFF`); guard tests flipped (sonobus suite 166 pass). The daemon plumbing (DaemonServer/UdsProtocol/DaemonConfig/MetricsCollector/JackBridge/Main) was already written; `AooTransport.cpp` is still the stub (aoo/ includes commented out).
  - **RT-safety design note SHIPPED 2026-06-02 (awaiting operator sign-off):** [`docs/architecture/SONOBUS_DAEMON_RT_SAFETY_REVIEW.md`](architecture/SONOBUS_DAEMON_RT_SAFETY_REVIEW.md). Locks the RT contract before any real-transport C++ merges. Key grounding: the daemon is a SEPARATE process (its RT thread is its own JACK process callback, NOT the JUCE engine's), and AOO's own headers annotate `source/sink.process()` as "RT-safe; call on the audio thread" while `send()`/`receive()` are "network thread" — AOO is built for the JACK-callback ↔ network-thread split, so the design leans on the upstream guarantee. §5 per-file audit + §6 software gates (build both modes, UDS framing, stream-table atomic-swap stress test); the 5-min RT soak is T2521-10 post-release debt.
  - **RT design note APPROVED by operator 2026-06-02; real transport SHIPPED (software-gated).** Steps 2-4 done:
    - **(2) CMake wire-up ✓** — `juce-engine/CMakeLists.txt` sets `AOO_LOCAL_OPUS=OFF` (+ disables AOO examples/server/tests/docs/shared) before `add_subdirectory(vendor/aoo)`; `SONOBUS_AVAILABLE` auto-flips TRUE now that `vendor/aoo/CMakeLists.txt` exists; the `SonoBusDaemon` target links `Aoo::aoo` + system Opus 1.5.2 when available, with `MAP2_SONOBUS_HAS_AOO` switching stub↔full. Verified: real AOO 2.0.0 lib + daemon build clean; `ldd` shows `libjack.so.0` (PipeWire JACK) + `libopus.so.0`.
    - **(3) Real transport ✓** — `AooTransport.cpp` (AooClient owns the UDP socket; sources/sinks keyed by stream_id; `poll()` is the network-thread send/receive/run pump) + `JackBridge.cpp` (real JACK client; the RT process callback does ONLY `loadActive()` once + `jack_port_get_buffer` + `AooSource/Sink::process()` + stack channel ptrs — no send/recv/socket/malloc/lock, per §4). New `StreamTable.h` is the RT boundary: a **single-reader hazard-pointer** reclamation (publish-then-validate with **seq_cst** — the agent's stress test caught a UAF under an unpaced publisher that a count-based grace period + release/acquire both allowed; seq_cst is the sound fix, verified at source). Stub `#else` path preserved + compiles clean.
    - **(4) Supervisor** — `app/services/sonobus/daemon_supervisor.py` + `/api/sonobus/status` wiring were already written (cycle 5); added `tests/sonobus/test_sonobus_daemon_supervisor.py` (8 cases).
    - **Tests:** new Catch2 `SonoBusStreamTableTests.cpp` (stream-table no-tearing + hazard reclamation, in `recorder_tests` → `[t2521]` 7 cases); 4 stale stub-only protocol tests in `test_t2521_sonobus_daemon_protocol.py` made mode-aware (full mode now: create_source/sink/destroy succeed, has_aoo=True). **Gate (run by Claude): real AOO+daemon build clean; `recorder_tests` 85 cases / 8152 assertions; sonobus + protocol + supervisor suites 187 passed.**
  - **(5) Engine binding DEFERRED to its own bundle:** `setSonoBusInputId`/`setSonoBusOutputId` RT-safe binding (routes engine audio to/from the daemon's JACK ports — keeps AOO out of the JUCE RT path). Not in this slice.
  - **Post-release hardware debt (T2521-10):** daemon-starts-via-systemd, live metrics on `/api/sonobus/diagnostics`, WS events to GUI, the paired 5-min ON-vs-OFF RT soak. Drive-by follow-up noted: the daemon's `DAEMON_VERSION` still reads `"0.1.0-stub"` in full mode — a one-line bump (the `build_mode:full`/`has_aoo:true` handshake fields are what the supervisor keys on, so it's cosmetic).
  - **Post-release hardware debt:** daemon-starts-via-systemd check, live metrics on `/api/sonobus/diagnostics`, WS events to the GUI, the paired 5-min ON-vs-OFF RT soak (T2521-10). These are the acceptance criteria and are all runtime/bench.
  Description:
  - Goal / acceptance criteria: Deliver the selected runtime path (headless SonoBus sidecar, direct AOO daemon, or phased sidecar-to-daemon) with RT-safe audio handoff and metrics. **Policy reframe (2026-06-02):** the daemon binary + CMake target + vendored AOO source + supervisor lifecycle + engine binding is un-written CODE (ships this release, software-gated by build + focused tests). The RT audio-bench proof (no xruns / jitter ≤0.35 ms attributable to the daemon) is the hardware gate → § After-Release Hardware Verification. See `docs/architecture/SONOBUS_BENCH_HANDOFF.md`.
