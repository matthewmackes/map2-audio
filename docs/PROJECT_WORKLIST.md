...
- ID: T2521-4
  ...
  - **Remaining (this-release code, software-gated):** (2) wire `vendor/aoo` into the engine CMake so `SONOBUS_AVAILABLE=TRUE` (currently both AVAILABLE+STUB are FALSE) + the `SonoBusDaemon` target links the core `aoo/` lib + system Opus; (3) implement the real `aoo_source`/`aoo_sink` path in `AooTransport.cpp` (RT boundary: JACK callback ↔ AOO over UDP — its own focused build, ideally with an RT-safety design note like T2511 since it's audio-path C++); (4) `app/services/sonobus/daemon_supervisor.py` (mirror the maschine daemon supervisor); (5) engine `setSonoBusInputId`/`setSonoBusOutputId` RT-safe binding.
