...
Assigned to: TBD
Last updated: 2026-05-04 EDT — Slices 2-6 SHIPPED (cycles 53-57 of autonomous-loop session). 15 useQuery calls consolidated into `useSnapshotEditorReadQueries.ts` across 5 typed hook entry points: `useSnapshotEditorCatalogReadQueries` (chains/plugins/presets), `useSnapshotEditorMidiReadQueries` (midi status/learn/mappings), `useSnapshotEditorAudioReadQueries` (7 audio-engine queries), `useSnapshotEditorAssignmentReadQueries` (cluster nodes + chain analysis), `useSnapshotEditorSnapshotConfigQueries` (snapshots summary + noise-gate + IO defaults). **Cache-key bit-identity verified** by 8 cache-key parity tests in `useSnapshotEditorReadQueries.test.tsx` (all queryKey arrays inspected via `QueryClient.getQueryCache().getAll()` after `renderHook` mount). Integration test `SnapshotEditorPage.integration.test.tsx` green; full SnapshotEditor test suite 52/52 passing post-refactor. Monolith size: 6807 → 6695 (-112 LoC). **Conservatively deferred** for a focused operator session with React Query Devtools cache-inspection: `authoritySnapshotDetailQuery`, `snapshotRevisionsQuery`, `heroPublishReadinessQuery` (queryKeys depend on `useMemo`-derived state that flows through other consumer state — extracting risks moving consumer state out of order). Mutations untouched (31 sites) — Phase 2 of T2472. Status remains `[>] In Progress` because the read consolidation isn't complete; the deferred 3 read queries + the 31 mutations are session-2 work.
