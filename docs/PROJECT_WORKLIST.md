...
| T2512-SLICE | Loop slicing / editing                | [✓] Metadata + UI + per-slice delete shipped 2026-05-12. Metadata: `TrackSlice` dataclass; per-track ordered list; `add_slice`/`clear_slices` verbs; full validation; snapshot round-trip. **T2512-SLICE-UI** follow-up: collapsible `SliceEditor` on the LooperPage track card with add-form + sorted slice list + empty-state. **T2512-SLICE-DEL** follow-up: new service verb `delete_slice(track, start_frame) → LooperStatus` (raises `slice_not_found` → HTTP 404 when no match); new route `DELETE /api/v1/looper/track/{n}/slices/{start_frame}` keyed on the slice's start_frame (unique within a track); new TS client method `deleteSlice(track, start_frame)`; trash-can Button per slice row in the SliceEditor list with `iconDescription` + `tooltipPosition="left"` for accessibility. +16 + 8 + 2 + 6 + 6 service tests + 3 route tests + 1 client test + 1 RTL test = 43 tests for the SLICE feature. |
