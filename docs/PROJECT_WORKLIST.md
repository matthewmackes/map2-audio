...
| T2512-AUTO  | Auto-record (threshold start)          | [✓] State surface shipped 2026-05-12. Per-track `auto_armed: bool` + `auto_threshold_db: float` fields in `LooperService` (default -36 dB; clamped -90..0). New routes `PATCH /track/{n}/auto-armed` + `PATCH /track/{n}/auto-threshold`, new dispatcher pattern `audio.looper.*.auto_armed`, new bridge hook + generic catalog entries (4) + JS module helper, new LooperPage Toggle showing the live threshold value. Actual trigger ("input RMS above threshold → fire record()") deferred to **T2512-AUTO-TRIGGER** once engine input-level RMS is exposed to Python. +9 service tests, +5 route tests, +2 dispatcher tests, +1 JS test, +2 web client tests = 19 tests added. |
