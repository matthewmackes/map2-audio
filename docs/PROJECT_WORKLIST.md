...
- `[>]` `T2459-H7-PW-UMP` — Path 4 substrate detection LANDED (2026-05-08; Python detection probe `controller_host_pipewire_substrate.py` + 12 unit tests + decision doc + MIDI_BACKEND.md §10 + evidence dir; **operator-driven C++ slice remains**: `juce-engine/Source/ControllerHost/main.cpp` must consume `MAP2_MIDI_BACKEND_FORCE` via `getenv()` before `Map2MidiBackend::probe()` to wire the detection end-to-end)
...
- `T2459-H7-PW-UMP` — Path 4 LANDED 2026-05-08 (operator picked ALSA-seq direct subscription, contrary to the Path 3 recommendation in the decision doc). Python detection probe + tests + docs all on master. Remaining: one-line C++ consumer in `juce-engine/Source/ControllerHost/main.cpp` to read `MAP2_MIDI_BACKEND_FORCE` and call `Map2MidiBackend::forceSelect()` before `probe()`. Until that lands, the Python detection is a no-op end-to-end. Tracked in §6 of the decision doc + gate G3 of [`docs/fit-for-purpose-evidence/20260508/t2459h7-pw-ump-path4/README.md`](fit-for-purpose-evidence/20260508/t2459h7-pw-ump-path4/README.md).
