...
| T2512-SLICE | Loop slicing / editing                | [✓] Metadata model shipped 2026-05-12. New `TrackSlice` frozen dataclass (`start_frame`, `end_frame`, `label`); per-track ordered list on `LooperService`. New verbs `add_slice(track, start, end, label)` + `clear_slices(track)`. Service-side validation: integer frame range; `0 <= start < end`; no overlaps within a track (HTTP 409 `slice_overlap`); per-track cap of 64 slices (HTTP 409 `slice_limit`); label trimmed + truncated to 64 chars. New routes `POST /track/{n}/slices` + `DELETE /track/{n}/slices`. TrackStatus + snapshot payload (T2512-SNAP) round-trip slices; bulk apply defensively drops overlapping slices. Region-editor UI deferred to **T2512-SLICE-UI**. +16 service tests, +8 route tests, +2 snapshot integration tests = 26 tests; OpenAPI audit extended for the 2 new paths + new TrackSliceResponse component schema. |
