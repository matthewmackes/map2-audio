...
| T2512-SLICE | Loop slicing / editing                | [✓] Metadata model + UI shipped 2026-05-12. Metadata piece: new `TrackSlice` dataclass (`start_frame`, `end_frame`, `label`); per-track ordered list; `add_slice`/`clear_slices` verbs; full validation (integer range, no overlaps, 64/track cap, 64-char label cap); HTTP 409 on slice_overlap + slice_limit; snapshot round-trip with defensive overlap-drop. **T2512-SLICE-UI** follow-up: new collapsible `SliceEditor` block on the LooperPage track card. Operator opens the toggle, sees a sorted list of existing slices as `start–end (label)` rows, fills an inline form (NumberInput start/end + text label, max 64 chars), clicks Add. Empty-state copy explains the frame math (1 s @ 48 kHz = 48000 frames). +16 service tests + 8 route tests + 2 snapshot tests + 6 RTL tests = 32 tests; OpenAPI audit covers the routes + TrackSliceResponse component schema. |
