...
| T2512-OS    | One-shot / trigger mode               | [✓] Shipped 2026-05-12 (flag-only). Per-track one-shot flag in `LooperService` (Python-only). New route `PATCH /api/v1/looper/track/{n}/one-shot`, new dispatcher pattern `audio.looper.*.one_shot`, new bridge hook + generic catalog entries (4) + JS module helper, new LooperPage Toggle. Flag is persistent across record/clear/undo/redo. Auto-stop scheduling deferred to **T2512-OS-RUNNER** follow-up (async runner that reads `playhead_frames` and fires `stop_track` after one pass). +6 service tests, +4 route tests, +2 dispatcher tests, +1 JS test = 13 tests added. |
