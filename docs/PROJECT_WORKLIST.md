...
- Completion notes:
  ...
  - Hardened `app/routes/plugins.py` so `GET /api/plugins/{uri}/parameters` now accepts `instance_id` and `plugin_position`, resolves the live target instance through `engine.resolve_instance_id()` when available, and rejects unknown scoped positions instead of silently reading the wrong duplicate instance.
  - Added backend regression coverage in `tests/test_plugin_parameter_route_identity.py` for position-scoped reads, stale `instance_id` recovery through `plugin_position`, and 404 handling when a requested duplicate-plugin position no longer exists. This completes the remaining backend half of `T453-subJ`.
  ...
  - Added scoped NAM status helpers in `app/services/juce_engine_service.py` for `model_loaded`, `loading`, `bypass`, `normalize`, `input_gain`, `output_gain`, `input_level`, and `output_level`, plus `get_nam_status_instance()`, all sourced from the duplicate-safe `get_nam_model_info_instance()` payload instead of the global singleton status path.
  - Expanded `tests/test_juce_engine_service_instance_resolution.py` with service-level regression coverage for instance-scoped NAM status reads and safe defaults when no engine is available.
  - Updated `.github/copilot-instructions.md` with the rule that duplicate-safe NAM status consumers should use instance-scoped helpers at the service layer instead of reusing global NAM status methods.
  - Licensing review: touched backend/test/worklist/instructions files remain MAP2-owned AGPL-covered repository artifacts with no third-party override in scope; reran `rg -n "AGPL|GNU Affero|license|LICENSE|THIRD_PARTY_NOTICES|SPDX|non-commercial|source-available|Proprietary|MIT" README.md LICENSE docs .github/copilot-instructions.md app web/src tests systemd scripts ReadMe-Make_New_Node.txt` and `rg --files -g 'LICENSE*' -g '*COPYING*' -g '*NOTICE*'`, and found no new notice or ownership gaps requiring follow-up work.
  - Validation: `pytest -q tests/test_juce_engine_service_instance_resolution.py tests/test_nam_ir_instance_routes.py` -> PASS (`15 passed`).
...
Subtasks: None
Assigned to: Codex
Last updated: 2026-03-27 08:45 EDT - Codex
- Completion notes:
  - Traced the forced-kill stop path through two concrete root causes. First, `systemctl show map2-web-prod.service -p ExecStart` showed the live host was still running `/etc/systemd/system/map2-web-prod.service` with `npm run preview`, so the earlier cycle was still stopping the stale npm-wrapper process (`3800199`). Second, after switching to a direct node unit, the next full rebuild still timed out because `scripts/serve_web_dist.mjs` handled `SIGTERM` with `server.close()` only and left persistent browser sockets alive.
  - Updated the repo-controlled production runtime contract so `systemd/map2-web-prod.service`, the installer-generated frontend unit in `scripts/install-node.sh`, and the manual fallback in `scripts/build/deploy` all exec `/usr/bin/node .../scripts/serve_web_dist.mjs --host 0.0.0.0 --port 3000` directly instead of using `npm` as the long-lived service wrapper.
  - Hardened `scripts/serve_web_dist.mjs` to track live client/proxy sockets, destroy them during shutdown, and use a short forced-exit timer after `SIGTERM`, which prevents keep-alive or upgraded connections from holding the production web service open indefinitely while systemd is trying to stop it.
  - Expanded `tests/test_serve_web_dist.py` so the production server now has focused coverage for both plain `SIGTERM` exit and the previously failing keep-alive case where an open client connection must be torn down for the process to exit promptly (`3 passed`).
  - Synced the live host with `sudo install -m 0644 systemd/map2-web-prod.service /etc/systemd/system/map2-web-prod.service` followed by `sudo systemctl daemon-reload`. The first post-sync restart and the first post-script restart necessarily killed still-running pre-fix processes, but the patched runtime then proved clean shutdown on the next restart: `sudo journalctl -u map2-web-prod.service --since '2026-03-27 08:42:30' --no-pager` shows `08:44:40 EDT` logging `Received SIGTERM; shutting down MAP2 production web server` followed immediately by `map2-web-prod.service: Deactivated successfully.` before relaunching PID `3817174`.
  - Validation: `bash -n scripts/build/deploy` -> pass; `node --check scripts/serve_web_dist.mjs` -> pass; `pytest -q tests/test_serve_web_dist.py` -> pass (`3 passed in 1.86s`); `systemd-analyze verify systemd/map2-web-prod.service` -> pass; repeated `./scripts/build/deploy --skip-build` restarts now stop cleanly once the patched runtime is live; `systemctl show -p ExecStart -p MainPID map2-web-prod.service` confirms `/usr/bin/node /home/mm/map2-audio/scripts/serve_web_dist.mjs --host 0.0.0.0 --port 3000`; `curl -I --max-time 10 http://127.0.0.1:3000/` -> `HTTP/1.1 200 OK`.
...
- T454-subH: [✓] Done - Toolbar — routing dropdown + inline mode controls, preset dropdown, undo/redo, automation transport, search, column picker, batch actions
- T454-subI: [✓] Done - MIDI columns — CC#, Channel, Curve, Min, Max editable inline per plugin row
