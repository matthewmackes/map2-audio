...
- `[>]` `T2528` — **Expose existing Snapshot bundle downloads** — operator cancelled any plan to construct another portable-snapshot solution; use the existing `.map2snapshot` bundle route only. Implement a per-snapshot download link in the Snapshot explorer and document that no installer/environment changes are required.
