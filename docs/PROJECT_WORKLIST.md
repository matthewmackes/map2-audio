...
| T2512-QUANT | Quantize / auto-close                 | [✓] Math helper shipped 2026-05-12. New `app/services/looper_quantize.py` — pure-Python module exposing `frames_per_beat`, `frames_per_division` (whole/half/quarter/eighth/sixteenth/thirty-second + 1/N aliases), `snap_frames_to_grid(frames, bpm, division, sample_rate, mode)` (nearest/up/down, half-up rounding, returns int), `grid_aligned` (within tolerance_frames), `beats_for_frames`. BPM clamped 20..300 to match SnapshotTempoService; sample_rate parameterized. No service dependencies — pure math. Unblocks future callers (auto-close on grid boundary, quantized overdub stop, UI preview). Service-side wiring + UI deferred to **T2512-QUANT-WIRE**. +35 tests. |
