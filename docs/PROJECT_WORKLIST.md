...
| T2512-FSW   | External footswitch mapping           | [✓] Shipped 2026-05-12. New device pack `_generic/midi-learn-looper/` exposes every T2512-MIDI dispatcher target as a learn-able catalog entry: 20 stomps (record/stop/clear/undo/redo × 4 tracks), 20 mixer setters (level/muted/soloed/reverse/half_speed × 4 tracks), 1 master.level. Bool setters default to `toggle`, level setters prompt for clamped dB. Regression test dispatches every catalog verb through `register_default_handlers` and asserts `unmatched_count == 0`. +11 tests. |
