...
- ID: T2521-4
  ...
  - **Remaining (this-release code, software-gated; gated on the design-note sign-off for the RT pieces):** (2) wire `vendor/aoo` into the engine CMake so `SONOBUS_AVAILABLE=TRUE` (currently both AVAILABLE+STUB are FALSE) + the `SonoBusDaemon` target links the core `aoo/` lib + system Opus; (3) implement the real `aoo_source`/`aoo_sink` path in `AooTransport.cpp` + the JACK callback in `JackBridge.cpp` per the design note; (4) `app/services/sonobus/daemon_supervisor.py` (mirror the maschine daemon supervisor); (5) engine `setSonoBusInputId`/`setSonoBusOutputId` RT-safe binding (routes engine audio to/from JACK ports — keeps AOO out of the JUCE RT path).
