...
- `[>]` `T2528` — **Portable Snapshot/playbook architecture spike** — evaluate whether MAP2 Snapshots should become portable declarative rig intent bundles, with Ansible-like apply/plan semantics for compatible hosts while keeping provisioning/install concerns out of runtime snapshot authority. 2026-05-15 discovery: Ansible is not required; current storage is JSON graph document `snapshots.document` validated by local JSON Schema `schemas/snapshot-graph-v1.schema.json` v2026.05, exported as `.map2snapshot` zip with `snapshot.json` manifest + assets.
...
## T2528 — Portable Snapshot/playbook architecture spike

ID: T2528
Status: [>] In Progress
Title: Evaluate portable Snapshot/playbook semantics for cross-host rig restore
Description:
- Goal / acceptance criteria: Define whether a MAP2 Snapshot should be exportable as a portable declarative rig-intent bundle with Ansible-like `plan`, `check`, `apply`, and compatibility-report behavior. Acceptance requires a documented boundary between snapshot-owned musical/runtime intent and host provisioning, a capability-resolution model for missing plugins/devices/interfaces/controllers, a package/export shape, and route/service/UI implications.
- Why it matters: Portable snapshots could let an operator move a rig between MAP2 hosts without treating cached live state or machine-local config as authoritative, but the design must not blur installer/environment responsibilities with snapshot activation.
- Dependencies: Snapshot-first authority rules; State Authority downstream contract; Configuration Authority Model; no code changes until operator approves the architecture direction.
- Estimated effort: Medium.
- Required outputs/deliverables: Architecture note or ADR, compatibility matrix, proposed schema/export fields, installer/environment impact review, and follow-up implementation tasks if approved.
Subtasks: None
Assigned to: Unassigned
Last updated: 2026-05-15 09:47 EDT - Codex: filed from operator question about whether snapshots could be portable like Ansible playbooks; worklist-only, no dependency/service/build/runtime changes.
Progress notes:
