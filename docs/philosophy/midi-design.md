...
- **Sequencer reframing** — `SequencerPage` (renamed from `PerformanceBrainPage` per T_RENAME 2026-05-02) carries the same banner with bespoke "Sequencer inputs are MIDI Services consumers" copy + link to `/midi/bindings?consumer_type=brain_slot`. The `brain_slot` consumer-type literal is preserved on the binding model since renaming it would invalidate every saved binding row; the surface label flipped, the storage contract did not.
