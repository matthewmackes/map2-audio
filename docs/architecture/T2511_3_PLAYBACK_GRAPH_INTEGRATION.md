...
4. Drain the `TriggerQueue` at block start (passing `engineRecorder_->sampleCount()` as the current clock) so a sample-accurate `apply_at_sample` switch flips `currentSource_` at the exact intra-block offset (§6). The trigger queue + sub-span walk are already implemented in `ChainInputSwitch`.
