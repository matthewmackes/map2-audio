...
from app.routes import sequencer as sequencer_routes
...
class _FakeSequencerAuthoritySyncService:
    def __init__(self, service: SequencerService):
        self.calls = []
...
def make_client(tmp_path: Path) -> tuple[TestClient, _FakeWsManager, _FakeSequencerAuthoritySyncService]:
    service = SequencerService(root_path=tmp_path / "brain-routes")
    ws_manager = _FakeWsManager()
    authority_sync = _FakeSequencerAuthoritySyncService(service)
    app = FastAPI()
    app.include_router(sequencer_routes.router)
    sequencer_routes.get_sequencer_service = lambda: service
    sequencer_routes._brain_authority_service = lambda: authority_sync
    sequencer_routes.get_drum_machine_service = lambda: _FakeDrumMachineService()
    sequencer_routes.get_drum_sequencer_service = lambda: _FakeDrumSequencerService()
    sequencer_routes.get_drum_kit_service = lambda: _FakeDrumKitService()
    sequencer_routes.get_audio_engine = lambda: _FakeSynthForgeEngine()
    sequencer_routes.ws_manager = ws_manager
    return TestClient(app), ws_manager, authority_sync


def test_sequencer_routes_scope_state_by_instance_id(tmp_path):
    client, _, _ = make_client(tmp_path)

    update_response = client.post("/api/engine/sequencer/state?instance_id=17", json={"set_name": "Card Instance", "active_slot": 4})
    assert update_response.status_code == 200
    assert update_response.json()["set_name"] == "Card Instance"

    get_response = client.get("/api/engine/sequencer/state?instance_id=17")
...
def test_sequencer_routes_scope_duplicate_instance_ids_by_plugin_position(tmp_path):
    client, _, _ = make_client(tmp_path)

    first_update = client.post(
        "/api/engine/sequencer/state?instance_id=17&plugin_position=0",
        json={"set_name": "Position Zero", "active_slot": 2},
    )
    second_update = client.post(
        "/api/engine/sequencer/state?instance_id=17&plugin_position=1",
    ...
    first_payload = client.get("/api/engine/sequencer/state?instance_id=17&plugin_position=0").json()
    second_payload = client.get("/api/engine/sequencer/state?instance_id=17&plugin_position=1").json()
...
def test_sequencer_routes_restore_scoped_state_from_authority_before_reads(tmp_path):
    client, _, authority_sync = make_client(tmp_path)
    seed_state = SequencerService(root_path=tmp_path / "brain-authority-seed").get_state()
    ...
    response = client.get("/api/engine/sequencer/state?instance_id=17&plugin_position=3")
...
def test_sequencer_routes_accept_session_media_active_section(tmp_path):
    client, _, _ = make_client(tmp_path)
...
def test_sequencer_routes_accept_practice_coach_active_section(tmp_path):
    client, _, _ = make_client(tmp_path)
...
def test_sequencer_routes_expose_scoped_controller_qualification(tmp_path):
    client, _, authority_sync = make_client(tmp_path)

    update_response = client.post(
        "/api/engine/sequencer/inputs?instance_id=17&plugin_position=3",
        json={
            "inputs": {
                "keyboard_zones": [],
                "trigger_profiles": [],
    ...
    response = client.get("/api/engine/sequencer/diagnostics?instance_id=17&plugin_position=3")
    assert response.status_code == 200
    payload = response.json()
    ...
    other_scope = client.get("/api/engine/sequencer/diagnostics?instance_id=17&plugin_position=4").json()
...
def test_sequencer_routes_import_drum_machine_state(tmp_path):
    client, _, authority_sync = make_client(tmp_path)

    response = client.post("/api/engine/sequencer/import/drums?instance_id=23")
    assert response.status_code == 200
    payload = response.json()
    ...
    assert payload["set_name"] == "Arena Kit Sequencer Import"
...
def test_sequencer_routes_import_synthforge_state(tmp_path):
    client, _, authority_sync = make_client(tmp_path)

    response = client.post("/api/engine/sequencer/import/synthforge?instance_id=42")
    assert response.status_code == 200
    payload = response.json()
...
def test_sequencer_routes_broadcast_scoped_runtime_updates(tmp_path):
    client, ws_manager, authority_sync = make_client(tmp_path)

    response = client.post(
        "/api/engine/sequencer/transport?instance_id=17&plugin_position=3",
