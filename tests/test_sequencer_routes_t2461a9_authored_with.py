"""T2461-A9 — Sequencer library asset overlay (`authored_with_devices`).

Covers the two routes added in this tranche:
- POST /api/engine/sequencer/library/assets/{asset_id}/authored-with
- GET  /api/engine/sequencer/library/by-device/{profile_key}

Plus the service-level overlay round-trip so the field actually surfaces
on subsequent /api/engine/sequencer/library reads.
"""
...
def _make_client(tmp_path: Path) -> tuple[TestClient, SequencerService]:
    service = SequencerService(root_path=tmp_path / "brain-a9")
    app = FastAPI()
    ...
    sequencer_routes._brain_authority_service = lambda: _NoopAuthoritySync()
    return TestClient(app), service


def _first_asset_id(client: TestClient) -> str:
    resp = client.get("/api/engine/sequencer/library")
...
def test_set_authored_with_persists_and_surfaces_on_library_read(tmp_path):
    ...
    resp = client.post(
        f"/api/engine/sequencer/library/assets/{asset_id}/authored-with",
    ...
    library = client.get("/api/engine/sequencer/library").json()
    found = False
...
def test_set_authored_with_unknown_asset_404(tmp_path):
    ...
    resp = client.post(
        "/api/engine/sequencer/library/assets/asset-that-does-not-exist/authored-with",
        json={"profile_keys": ["edirol-ua/ua-1000.audio"]},
    )
...
def test_assets_for_device_returns_count_and_ids(tmp_path):
    ...
    client.post(
        f"/api/engine/sequencer/library/assets/{asset_id_a}/authored-with",
        json={"profile_keys": ["edirol-ua/ua-1000.audio"]},
    )
    client.post(
        f"/api/engine/sequencer/library/assets/{asset_id_b}/authored-with",
        json={"profile_keys": ["edirol-ua/ua-1000.audio", "hotone/jogg.audio"]},
    )

    # UA-1000 should be tagged on both.
    resp = client.get("/api/engine/sequencer/library/by-device/edirol-ua%2Fua-1000.audio")
    assert resp.status_code == 200
    body = resp.json()
    ...
    resp = client.get("/api/engine/sequencer/library/by-device/hotone%2Fjogg.audio")
    assert resp.status_code == 200
    body = resp.json()
    ...
    resp = client.get("/api/engine/sequencer/library/by-device/never-pinned%2Fmodel.midi")
    assert resp.status_code == 200
    assert resp.json() == {
...
def test_overlay_survives_service_restart(tmp_path):
    ...
    SequencerService._instances.clear()  # type: ignore[attr-defined]
    fresh_service = SequencerService(root_path=service._root_path)
