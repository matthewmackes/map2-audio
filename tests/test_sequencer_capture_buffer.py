"""T2461-A6 — Sequencer capture buffer tests."""
...
@pytest.fixture
def buffer() -> SequencerCaptureBuffer:
...
def test_session_gc_drops_after_retention(buffer, monkeypatch):
    ...
    import app.services.sequencer.sequencer_capture_buffer as mod
...
def test_capture_start_then_stop_round_trip(client):
    r = client.post("/api/engine/sequencer/capture/start",
    ...
    r = client.post("/api/engine/sequencer/capture/stop")
    assert r.status_code == 200
    assert r.json()["finalised"] is True


def test_capture_get_returns_payload(client):
    client.post("/api/engine/sequencer/capture/start",
                json={"slot_id": 0, "duration_s": 2.0})
    sid = client.post("/api/engine/sequencer/capture/start",
                      json={"slot_id": 7, "duration_s": 2.0}).json()["session_id"]
    client.post("/api/engine/sequencer/capture/stop")

    r = client.get(f"/api/engine/sequencer/capture/{sid}")
    body = r.json()
...
def test_capture_get_unknown_session_returns_not_found(client):
    r = client.get("/api/engine/sequencer/capture/does-not-exist")
