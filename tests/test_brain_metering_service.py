"""Tests for the per-slot Performance Brain ConsoleView metering service.
...
fader and mute state. It ships through the WebSocket `brain_metering` topic
at 30 fps and is also exposed for polling fallback at
GET /api/engine/brain/metering.
"""
...
from app.services.brain_metering_service import (
    BrainMeteringService,
    SlotMeterReading,
    _MIN_DB,
)
...
@pytest.fixture
def service(monkeypatch: pytest.MonkeyPatch) -> BrainMeteringService:
    svc = BrainMeteringService()
    # Patch the brain-service slot lookup so tests don't need a fully
    # initialised PerformanceBrainService. The default 16 drum slots map
    # MIDI notes 36..51 to slots 0..15.
    fake_slots = [_slot(i) for i in range(16)]
    monkeypatch.setattr(
        "app.services.brain_metering_service.get_performance_brain_service",
        lambda: mock.Mock(get_slots=lambda: fake_slots),
    )
    return svc


def test_idle_slots_report_minimum_db(service: BrainMeteringService) -> None:
...
def test_note_on_excites_only_the_matching_slot(service: BrainMeteringService) -> None:
...
def test_excitation_decays_to_floor(service: BrainMeteringService) -> None:
...
def test_velocity_zero_is_treated_as_note_off(service: BrainMeteringService) -> None:
    service._on_midi(_midi(bytes([0x90, 36, 0])))
    readings = service.read_slot_meters()
    assert readings[0].peak_db == _MIN_DB


def test_muted_slot_reads_silent_even_after_excitation(service: BrainMeteringService, monkeypatch: pytest.MonkeyPatch) -> None:
    service._on_midi(_midi(bytes([0x90, 36, 127])))
...
def test_fader_level_scales_peak(service: BrainMeteringService, monkeypatch: pytest.MonkeyPatch) -> None:
    service._on_midi(_midi(bytes([0x90, 36, 127])))
...
def test_clipping_flag_fires_at_zero_dbfs(service: BrainMeteringService) -> None:
    service._on_midi(_midi(bytes([0x90, 36, 127])))
...
def test_read_payload_shape(service: BrainMeteringService) -> None:
...
def test_short_or_non_note_on_messages_are_ignored(service: BrainMeteringService) -> None:
