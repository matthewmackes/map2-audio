...
def make_service(tmp_path: Path) -> SequencerService:
    return SequencerService(root_path=tmp_path / "brain-service")


def test_sequencer_service_isolates_instance_state(tmp_path):
    service = make_service(tmp_path)

    service.update_state(SequencerStateUpdateModel(set_name="Stage Rig", active_slot=3), instance_id="101")
    service.update_transport(SequencerTransportUpdateModel(bpm=132, pattern=7), instance_id="101")
...
def test_sequencer_service_isolates_duplicate_instance_ids_by_plugin_position(tmp_path):
    service = make_service(tmp_path)

    service.update_state(
        SequencerStateUpdateModel(set_name="Rig A", active_slot=1),
        instance_id="101",
        plugin_position=0,
    )
    service.update_state(
        SequencerStateUpdateModel(set_name="Rig B", active_slot=7),
        instance_id="101",
        plugin_position=1,
    )
...
def test_sequencer_service_builds_scoped_runtime_events(tmp_path):
    service = make_service(tmp_path)

    service.update_transport(
        SequencerTransportUpdateModel(bpm=131, pattern=4),
        instance_id="101",
        plugin_position=3,
    )
...
def test_sequencer_service_derives_controller_qualification_from_scoped_state(tmp_path):
    service = make_service(tmp_path)
...
def test_sequencer_service_update_slot_rejects_out_of_range_slot_id(tmp_path):
    service = make_service(tmp_path)

    with pytest.raises(IndexError, match="slot_id 99 out of range"):
        service.update_slot(99, SequencerSlotUpdateModel(name="Invalid"))


def test_sequencer_service_get_library_is_cached_and_read_only(tmp_path, monkeypatch):
    service = make_service(tmp_path)
...
def test_sequencer_service_keeps_controller_qualification_scope_specific(tmp_path):
    service = make_service(tmp_path)
...
def test_sequencer_service_imports_drum_machine_payloads(tmp_path):
    service = make_service(tmp_path)
    ...
    assert imported["set_name"] == "Arena Kit Sequencer Import"
...
def test_sequencer_service_imports_synthforge_payloads(tmp_path):
    service = make_service(tmp_path)
