...
from app.services.sequencer_service import SequencerStateUpdateModel, SequencerService
...
async def test_sync_instance_serializes_brain_state_into_authority_extensions(tmp_path):
    sequencer_service = SequencerService(root_path=tmp_path / "brain-authority")
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Authority Brain", active_slot=4),
        instance_id="17",
        plugin_position=3,
    )
    authority_service = _FakeAuthorityService(_build_committed_envelope())
    sync_service = SequencerAuthoritySyncService(
    ...
    desired_projection = authority_service.desired_updates[-1].extensions["sequencer"]["instances"]["instance-17__position-3"]
    committed_projection = result.value.extensions["sequencer"]["instances"]["instance-17__position-3"]
    observed_projection = authority_service.observations[-1].extensions["sequencer"]["instances"]["instance-17__position-3"]
    ...
    assert authority_service.observations[-1].runtime_metrics["sequencer"]["instance_count"] == 1
...
async def test_sync_instance_preserves_existing_brain_extensions_in_observation(tmp_path):
    sequencer_service = SequencerService(root_path=tmp_path / "brain-authority")
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Authority Sequencer A"),
        instance_id="17",
        plugin_position=3,
    )
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Authority Sequencer B"),
        instance_id="22",
        plugin_position=5,
    )
    authority_service = _FakeAuthorityService(_build_committed_envelope())
    ...
    observed_instances = authority_service.observations[-1].extensions["sequencer"]["instances"]

    assert set(observed_instances) == {"instance-17__position-3", "instance-22__position-5"}
    assert observed_instances["instance-17__position-3"]["state"]["set_name"] == "Authority Sequencer A"
    assert observed_instances["instance-22__position-5"]["state"]["set_name"] == "Authority Sequencer B"
    assert authority_service.observations[-1].runtime_metrics["sequencer"]["instance_count"] == 2
...
async def test_restore_instance_hydrates_local_brain_state_from_committed_projection(tmp_path):
    sequencer_service = SequencerService(root_path=tmp_path / "brain-authority")
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Stale Local Brain", active_slot=2),
        instance_id="17",
        plugin_position=3,
    )
    ...
    synced_projection = await SequencerAuthoritySyncService(
        authority_service=authority_service,
        sequencer_service=sequencer_service,
        node_id="node-local",
    ...
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Locally Drifted Again", active_slot=7),
        instance_id="17",
        plugin_position=3,
    )
    authority_service.committed = synced_projection

    restored = await SequencerAuthoritySyncService(
        authority_service=authority_service,
        sequencer_service=sequencer_service,
        node_id="node-local",
    ...
    assert sequencer_service.get_state(instance_id="17", plugin_position=3)["set_name"] == "Stale Local Brain"
...
async def test_restore_instance_falls_back_to_local_state_without_projection(tmp_path):
    sequencer_service = SequencerService(root_path=tmp_path / "brain-authority")
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Only Local Brain", active_slot=5),
        instance_id="99",
        plugin_position=1,
    )
    authority_service = _FakeAuthorityService(_build_committed_envelope())
...
async def test_restore_instance_prefers_desired_projection_when_committed_lags(tmp_path):
    sequencer_service = SequencerService(root_path=tmp_path / "brain-authority")
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Locally Drifted Brain", active_slot=1),
        instance_id="17",
        plugin_position=3,
    )
    committed_envelope = _build_committed_envelope()
    ...
    assert sequencer_service.get_state(instance_id="17", plugin_position=3)["set_name"] == "Desired Snapshot Brain"
...
async def test_reconcile_runtime_with_extensions_restores_snapshot_state_and_resets_missing_instances(tmp_path):
    sequencer_service = SequencerService(root_path=tmp_path / "brain-authority")
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Local Sequencer A", active_slot=2),
        instance_id="17",
        plugin_position=3,
    )
    sequencer_service.update_state(
        SequencerStateUpdateModel(set_name="Local Sequencer B", active_slot=5),
        instance_id="22",
        plugin_position=5,
    )
            ...
            "sequencer": {
                "instances": {
                    "instance-17__position-3": {
                        ...
                        "instance_id": "17",
                        "plugin_position": 3,
                        "state": sequencer_service.get_state(instance_id="17", plugin_position=3),
                    },
    ...
    assert sequencer_service.get_state(instance_id="17", plugin_position=3)["set_name"] == "Snapshot Sequencer A"
