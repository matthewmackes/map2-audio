/**
 * useSequencerChannelMeters — per-slot Sequencer ConsoleView meters.
 *
 ...
 * GET /api/engine/sequencer/metering for the warm-up window before the first WS
...
export interface SequencerChannelMeter {
  slotId: number
  peakDb: number
  rmsDb: number
  clipping: boolean
...
export interface SequencerChannelMetersState {
  meters: SequencerChannelMeter[]
  running: boolean
  isConnected: boolean
}

interface SequencerMeteringPayload {
  running: boolean
  slots: Array<{
    slot_id: number
    peak_db: number
...
function emptyMeters(): SequencerChannelMeter[] {
...
function applyPayload(
  payload: SequencerMeteringPayload,
  prev: SequencerChannelMeter[],
  holds: Map<number, { value: number; timestamp: number }>,
  now: number,
): SequencerChannelMeter[] {
...
interface UseSequencerChannelMetersOptions {
...
export function useSequencerChannelMeters(
  options: UseSequencerChannelMetersOptions = {},
): SequencerChannelMetersState {
  const { useWebSocket = true, pollingIntervalMs = 1000 } = options
  const [meters, setMeters] = useState<SequencerChannelMeter[]>(() => emptyMeters())
  const [isConnected, setIsConnected] = useState(false)
  const [running, setRunning] = useState(false)
  const holdsRef = useRef<Map<number, { value: number; timestamp: number }>>(new Map())
  const pendingPayloadRef = useRef<SequencerMeteringPayload | null>(null)
  ...
  const enqueue = (payload: SequencerMeteringPayload) => {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      if (disposed) return
      try {
        const message = JSON.parse(event.data) as { type?: string; data?: SequencerMeteringPayload }
        if (message.type === 'brain_metering_update' && message.data) {
          enqueue(message.data)
        }
      } catch (err) {
        console.error('useSequencerChannelMeters: bad WS payload', err)
      }
    }
