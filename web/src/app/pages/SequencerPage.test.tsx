...
jest.mock('@/app/pages/sequencerViews/SequencerOverviewShell', () => {
  const React = require('react')
  return {
    SequencerOverviewShell: () =>
      React.createElement('div', { 'data-testid': 'brain-overview-shell' }, 'SequencerOverviewShell'),
  }
})
...
function renderPage(initialEntry = '/sequencer') {
  return render(
    <MemoryRouter
...
describe('SequencerPage', () => {
  ...
  it('renders the routed diagnostics section without the legacy page rail', async () => {
    renderPage('/sequencer?instance_id=42&plugin_position=9&section=diagnostics')
    ...
    expect(screen.queryByLabelText('Sequencer section navigation')).not.toBeInTheDocument()
    expect(screen.getByTestId('location-probe')).toHaveTextContent('/sequencer?instance_id=42&plugin_position=9&section=diagnostics')
  })

  it('normalizes the section query param from backend state when the route is missing it', async () => {
    primeApi({ activeSection: 'layers' })

    renderPage('/sequencer?instance_id=7&plugin_position=2')

    await waitFor(() => expect(screen.getByText('Layer map')).toBeInTheDocument())
    await waitFor(() => expect(screen.getByTestId('location-probe')).toHaveTextContent('/sequencer?instance_id=7&plugin_position=2&section=layers'))
    expect(screen.queryByLabelText('Sequencer section navigation')).not.toBeInTheDocument()
    expect(mockUpdateState).not.toHaveBeenCalled()
  })

  it('runs the drum importer from the library section', async () => {
    renderPage('/sequencer?section=library')
  ...
  it('renders the SequencerOverviewShell on every Overview subview section', async () => {
  ...
  it('shows scoped qualification details on the inputs section', async () => {
    ...
    renderPage('/sequencer?instance_id=42&plugin_position=9&section=inputs')
  ...
  it('renders session media as a backing-track adjunct and routes transport mutations', async () => {
    ...
    renderPage('/sequencer?section=session_media')

    await waitFor(() => expect(screen.getByText('Session media adjunct')).toBeInTheDocument())
    expect(screen.getByText(/Backing tracks stay outside the core Sequencer transport/)).toBeInTheDocument()
  ...
  it('renders practice coaching as an adjunct and routes rehearsal mutations through drum state', async () => {
    ...
    renderPage('/sequencer?section=practice_coach')

    await waitFor(() => expect(screen.getByText('Practice and coaching adjunct')).toBeInTheDocument())
    expect(screen.getByText(/Practice packs and coaching cues stay outside the core Sequencer transport/)).toBeInTheDocument()
  ...
  it('auto-runs a scoped drum handoff import and clears the handoff flag from the route', async () => {
    renderPage('/sequencer?instance_id=42&plugin_position=9&section=performance&import_source=drums')
  ...
  it('auto-runs a scoped SynthForge handoff import while preserving the selected section', async () => {
    renderPage('/sequencer?instance_id=7&plugin_position=2&section=library&import_source=synthforge')
