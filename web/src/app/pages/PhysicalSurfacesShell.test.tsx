...
describe('PhysicalSurfacesShell', () => {
  ...
  it('renders the overview workspace with the shared stack name and unit cards', async () => {
    ...
    expect(screen.getAllByRole('treeitem', { name: 'Open Native Instruments Maschine MK1' }).length).toBeGreaterThan(0)
    expect(screen.getByRole('treeitem', { name: 'Open Ableton Push' })).toBeInTheDocument()
    expect(screen.getAllByRole('treeitem', { name: 'Open Mackie MCU Pro' }).length).toBeGreaterThan(0)
    expect(screen.getByRole('treeitem', { name: 'Open Novation Launch Control Family' })).toBeInTheDocument()
    expect(screen.getByRole('treeitem', { name: 'Open MeloAudio MIDI Commander' })).toBeInTheDocument()
