...
function DevicesAndUtilitiesTab() {
  ...
  // Backend has shipped two response shapes for /v2/midi/devices over the years:
    ...
    <div style={{ display: 'grid', gap: '1rem' }}>
      <Tile>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '0.5rem' }}>
          <div>
            <h4>Input</h4>
            <p style={{ opacity: 0.7 }}>Active: {currentInput ?? '—'}</p>
            {inputDeviceNames.map((d) => (
          ...
          <div>
            <h4>Output</h4>
            <p style={{ opacity: 0.7 }}>Active: {currentOutput ?? '—'}</p>
            {outputDeviceNames.map((d) => (
              <div key={d} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.25rem 0' }}>
                <span>{d}</span>
...
export function LegacyMidiAssignments() {
  ...
  // Soft-fail every query so a single broken backend endpoint can't blow up the page.
  // (e.g. /api/v2/midi/routing-rules currently returns 500 and /chain-configs is 404 —
  //  pre-existing backend bugs we surface with a banner instead of crashing.)
  const mappingsQuery = useQuery({ queryKey: ['midi', 'mappings'], queryFn: () => midiApiV2.getMappings(), retry: false })
  const commandsQuery = useQuery({ queryKey: ['midi', 'commands'], queryFn: () => midiApiV2.getCommands(), retry: false })
  const routingRulesQuery = useQuery({ queryKey: ['midi', 'routing-rules'], queryFn: () => midiApiV2.getRoutingRules(), retry: false })
  const groupsQuery = useQuery({ queryKey: ['midi', 'groups'], queryFn: () => midiApiV2.getGroups(), retry: false })
  const chainConfigsQuery = useQuery({ queryKey: ['midi', 'chain-configs'], queryFn: () => midiApiV2.getChainConfigs(), retry: false })
  const chainsQuery = useQuery({ queryKey: ['chains'], queryFn: () => chainsApi.list(), retry: false })
  const pluginsQuery = useQuery({ queryKey: ['plugins', 'all'], queryFn: () => pluginsApi.getAll(), retry: false })
  const snapshotsQuery = useQuery({ queryKey: ['snapshots', 'list'], queryFn: () => snapshotsApi.list(), retry: false })

  const learnStatusQuery = useQuery({ queryKey: ['midi', 'learn-status'], queryFn: () => midiApiV2.getLearnStatus(), refetchInterval: 1000, retry: false })
  const stopLearnMutation = useMutation({ mutationFn: () => midiApiV2.stopLearn() })

  const queryErrors: Array<{ name: string; error: unknown }> = [
    { name: 'mappings', error: mappingsQuery.error },
    { name: 'commands', error: commandsQuery.error },
    { name: 'routing-rules', error: routingRulesQuery.error },
    { name: 'groups', error: groupsQuery.error },
    { name: 'chain-configs', error: chainConfigsQuery.error },
    { name: 'chains', error: chainsQuery.error },
    { name: 'plugins', error: pluginsQuery.error },
    { name: 'snapshots', error: snapshotsQuery.error },
  ...
  return (
    <div style={{ padding: '1.5rem', display: 'grid', gap: '1rem', minHeight: '100vh' }}>
      ...
      {queryErrors.length > 0 && (
        <InlineNotification
          kind="warning"
          title={`${queryErrors.length} backend endpoint${queryErrors.length === 1 ? '' : 's'} unreachable`}
          subtitle={queryErrors.map((entry) => {
            const err = entry.error
            const msg = err instanceof Error ? err.message : String(err)
            return `${entry.name}: ${msg}`
          }).join(' · ')}
          hideCloseButton
          lowContrast
        />
      ...
      <Tabs>
        ...
        <TabPanels>
          <TabPanel>
            <ErrorBoundary title="Parameters tab crashed"><ParameterMappingsTab mappings={mappings} groups={groups} chains={chains} plugins={plugins} snapshotIdFilter={snapshotIdFilter} /></ErrorBoundary>
          </TabPanel>
          <TabPanel>
            <ErrorBoundary title="Snapshot triggers tab crashed"><CommandsTab commands={commands} chains={chains} plugins={plugins} snapshots={snapshots} chainConfigs={chainConfigs} /></ErrorBoundary>
          </TabPanel>
          <TabPanel>
            <ErrorBoundary title="Routing rules tab crashed"><RoutingRulesTab rules={routingRules} chains={chains} /></ErrorBoundary>
          </TabPanel>
          <TabPanel>
            <ErrorBoundary title="Expression pedals tab crashed"><ExpressionTab snapshotIdFilter={snapshotIdFilter} /></ErrorBoundary>
          </TabPanel>
          <TabPanel>
            <ErrorBoundary title="Devices &amp; utilities tab crashed"><DevicesAndUtilitiesTab /></ErrorBoundary>
          </TabPanel>
        </TabPanels>
      </Tabs>
    </div>
...
export default LegacyMidiAssignments
