...
import type {
  SequencerControllerQualification,
  ...
  SequencerInputsState,
} from '@/map2/api'

export type SequencerViewId = 'performance' | 'console' | 'step' | 'split' | 'setup'

export const SEQUENCER_VIEW_IDS: readonly SequencerViewId[] = ['performance', 'console', 'step', 'split', 'setup'] as const

export interface SequencerOverviewSharedProps {
  ...
  onTransportPatch: (patch: Partial<SequencerTransportState>) => void
  onSlotSelect: (slotId: number) => void
  onSlotUpdate: (slotId: number, patch: Partial<SequencerSlot>) => void
}

export function computeWarnCount(diagnostics: SequencerDiagnostics): number {
  return (diagnostics.controller_qualification.issues.length ?? 0) + (diagnostics.warnings.length ?? 0)
}

export function qualificationReadyCount(qualification: SequencerControllerQualification): number {
...
export function slotColor(_slot: SequencerSlot | undefined, index: number): string {
...
export function formatPosition(transport: SequencerTransportState): string {
