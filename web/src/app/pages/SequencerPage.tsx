...
import {
  sequencerApi,
  drumsApi,
  type SequencerControllerQualification,
  type SequencerDiagnostics,
  type SequencerLayer,
  type SequencerSlot,
  type SequencerState,
  type SequencerTransportState,
...
function summaryForSlot(slot: SequencerSlot | undefined): string {
  if (!slot) {
...
export function SequencerPage() {
  ...
  const importSource = useMemo(() => parseSequencerImportSource(searchParams.get('import_source')), [searchParams])
  ...
  // model via POST /api/engine/sequencer/library/assets/{id}/authored-with.
  ...
  const saveLibrarySnapshotMutation = useMutation({
    mutationFn: async ({ assetId, keys }: { assetId: string; keys: string[] }) => {
      return sequencerApi.setAssetAuthoredWith(assetId, keys)
    },
  ...
  if (
    ...
    return (
      <section className="brain-page brain-page--loading">
        <InlineLoading status="active" description="Loading Sequencer workspace" />
      </section>
    )
  }
            ...
            <div className="brain-page__section-grid">
              <Tile className="brain-page__panel">
                ...
                <table className="brain-page__table">
                  ...
                  <tbody>
                    {layers.map((layer: SequencerLayer) => (
          ...
          {activeSection === 'session_media' ? (
            <div className="brain-page__section-grid">
              <Tile className="brain-page__panel">
                ...
                <p className="brain-page__panel-copy">
                  Backing tracks stay outside the core Sequencer transport and sequence identity.
          ...
          {activeSection === 'practice_coach' ? (
            <div className="brain-page__section-grid">
              <Tile className="brain-page__panel">
                ...
                <p className="brain-page__panel-copy">
                  Practice packs and coaching cues stay outside the core Sequencer transport,
