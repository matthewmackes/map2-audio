                  ...
                  {clusterRows.map((row) => (
                    <TableRow key={row.node.nodeId}>
                      ...
                      <TableCell className="chains-page__table-cell--actions">
                        <Button kind="tertiary" size="sm" onClick={() => setActiveNode(row.node.nodeId)}>
                          Inspect
                        </Button>
                      </TableCell>
                    </TableRow>
  ...
  return (
    <div className="chains-page">
      <Layer className="chains-page__hero">
        <div className="chains-page__header-row">
          <div className="chains-page__title-block">
            <Flow size={32} aria-hidden="true" className="chains-page__title-icon" />
            <div>
              <h1 className="chains-page__title">{remoteSelected ? `Chains - ${selectedNode?.hostname ?? activeNodeId}` : 'Chains'}</h1>
        ...
        {isClusterMode ? (
          <div className={`chains-page__scope-card ${remoteSelected ? 'chains-page__scope-card--remote' : 'chains-page__scope-card--local'}`}>
            <div className="chains-page__scope-label">Chain scope</div>
            <strong className="chains-page__scope-title">{remoteSelected ? selectedNode?.hostname ?? activeNodeId : 'Local node'}</strong>
            <p className="chains-page__scope-copy">
              {remoteSelected
                ? `Chain actions are proxied to ${selectedNode?.hostname ?? activeNodeId}${selectedNode?.latencyMs == null ? '' : ` with peer latency ${selectedNode.latencyMs.toFixed(1)} ms`}.`
      ...
      <Modal
        ...
        size={isMobile ? 'lg' : 'sm'}
        modalLabel={remoteSelected ? `Target ${selectedNode?.hostname ?? activeNodeId}` : 'Local node'}
