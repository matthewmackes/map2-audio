...
import { recorderApi } from '../../map2/clients/recorder'
...
const DISCOVER_TAB_BY_CATEGORY: Record<ArtifactCategory, 'plugin-packs' | 'nam' | 'cabinet-irs' | 'reverb-irs' | 'soundfonts'> = {
  ...
  // Recordings have no off-platform discovery target — operators
  // generate them via the recorder; the Discover landing stays on
  // plugin-packs.
  recordings: 'plugin-packs',
}
...
const CATEGORIES: CategoryMeta[] = [
  ...
  {
    // T2509-2 (phase 5 of T2504 Multi-Track Recorder). Recorded WAV
    // takes appear here once the T2507 engine taps + recorder
    // service register them in the artifact registry. Until then
    // the list is empty — real, not stub. Operators delete via the
    // `Delete` row action; per-recording detail (waveform +
    // transport) lands under T2509-3 (RecordingDetailPanel).
    id: 'recordings',
    label: 'Recordings',
    shortLabel: 'Takes',
    icon: Microphone,
    description: 'Captured multi-track WAV takes — generated by the recorder service, registered in the artifact registry, replayable via the per-recording detail panel.',
    primaryAction: 'Inspect Recording',
    statusLabel: (item) => String(item.status ?? 'Ready'),
    statusType: (item) => item.statusRaw === 'ready' ? 'green' : 'warm-gray',
    columns: [
      { key: 'name', header: 'File Name' },
      { key: 'size', header: 'Size' },
      { key: 'createdAt', header: 'Captured' },
      { key: 'assetHash', header: 'Asset Hash' },
      { key: 'node', header: 'Node' },
      { key: 'status', header: 'Status' },
    ],
  },
...
export function AudioArtifactsPage({
  ...
  // T2509-2 — recordings registry. Reads /api/recordings (T2508-5)
  // and surfaces takes captured by the recorder service. The list
  // is empty until T2507 engine taps register real WAV files.
  const recordingsQuery = useQuery({
    queryKey: ['recordings', 'list', selectedNodeId],
    queryFn: () => recorderApi.listRecordings(),
    enabled: activeCategory === 'recordings',
    staleTime: 15000,
    refetchInterval: artifactsInventoryCadence,
  })
  ...
  const allRows: ArtifactRow[] = useMemo(() => {
    ...
    if (activeCategory === 'recordings') {
      // T2509-2 — surface registry rows from /api/recordings. Each
      // row carries its asset_hash so subsequent actions (stream
      // WAV, fetch metadata, DELETE) can address it directly.
      return (recordingsQuery.data?.recordings ?? []).map((rec) => ({
        id: rec.asset_hash,
        name: rec.file_name,
        size: formatBytes(rec.size_bytes),
        createdAt: rec.created_at || '—',
        assetHash: rec.asset_hash,
        node: nodeName,
        status: 'Ready',
        statusRaw: 'ready',
      }))
  ...
  const isLoading =
    ...
    (activeCategory === 'recordings' && recordingsQuery.isLoading)
