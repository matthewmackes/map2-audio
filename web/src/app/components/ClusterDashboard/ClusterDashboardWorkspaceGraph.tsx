import { useMemo } from 'react'

import { Handle, Position } from 'reactflow'
import type { Node, NodeProps } from 'reactflow'

import { SignalFlowGraph } from '../shared/SignalFlowGraph'
import type {
  ClusterDashboardWorkspaceGraphModel,
  ClusterDashboardWorkspaceGraphSelection,
  ClusterDashboardWorkspaceNodeData,
} from './clusterDashboardWorkspaceGraph'

type RenderNodeData = ClusterDashboardWorkspaceNodeData & {
  onSelect: (selection: ClusterDashboardWorkspaceGraphSelection) => void
}

function toneBorder(tone: ClusterDashboardWorkspaceNodeData['tone']) {
  switch (tone) {
    case 'aligned':
      return 'var(--cds-support-success)'
    case 'warning':
      return 'var(--cds-support-warning)'
    case 'critical':
      return 'var(--cds-support-error)'
    case 'neutral':
    default:
      return 'var(--cds-border-strong-01)'
  }
}

function toneLabel(tone: ClusterDashboardWorkspaceNodeData['tone']) {
  switch (tone) {
    case 'aligned':
      return 'Stable'
    case 'warning':
      return 'Watch'
    case 'critical':
      return 'Risk'
    case 'neutral':
    default:
      return 'Info'
  }
}

function cardWidth(kind: ClusterDashboardWorkspaceNodeData['kind']) {
  switch (kind) {
    case 'fabric':
      return 300
    case 'node':
    default:
      return 252
  }
}

export function ClusterDashboardWorkspaceNodeCard({ data }: NodeProps<RenderNodeData>) {
  return (
    <button
      type="button"
      style={{
        // T2474 B8: Removed borderRadius:10 (Q1=A flat corners) and the
        // box-shadow drop+halo (was Tailwind blue rgba + accent-tinted
        // selection halo — Q1=A retires both). Border carries selection
        // state instead via accentColor; inline styles here only carry
        // dynamic per-card values (width, accent color) that legitimately
        // come from data, not literals.
        width: cardWidth(data.kind),
        minHeight: data.kind === 'fabric' ? 124 : 120,
        border: `1px solid ${data.selected ? data.accentColor : toneBorder(data.tone)}`,
        borderInlineStart: `4px solid ${data.accentColor}`,
        background: data.selected ? 'var(--cds-layer-selected)' : 'var(--cds-layer)',
        color: 'var(--cds-text-primary)',
        padding: '0.875rem',
        textAlign: 'left',
        cursor: 'pointer',
      }}
      onClick={() => data.onSelect({
        anchorId: data.anchorId,
        recordId: data.recordId,
        contextNodeId: data.contextNodeId,
      })}
      aria-label={`Jump to ${data.label}`}
    >
      <Handle type="target" position={Position.Top} style={{ background: data.accentColor }} />
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.5rem', marginBottom: '0.5rem' }}>
        <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--cds-text-secondary)', letterSpacing: '0.02em' }}>
          {data.eyebrow}
        </span>
        <span style={{ fontSize: '0.72rem', fontWeight: 600, color: data.selected ? data.accentColor : 'var(--cds-text-secondary)' }}>
          {data.kind === 'node' ? data.statusLabel : toneLabel(data.tone)}
        </span>
      </div>
      <div style={{ fontSize: '0.95rem', fontWeight: 600, lineHeight: 1.25 }}>{data.label}</div>
      <div style={{ fontSize: '0.76rem', color: 'var(--cds-text-secondary)', marginTop: '0.375rem', lineHeight: 1.35 }}>
        {data.caption}
      </div>
      <div style={{ fontSize: '0.76rem', color: data.accentColor, marginTop: '0.75rem', fontWeight: 600 }}>
        {data.metric}
      </div>
      {data.kind === 'node' && data.healthPercent != null ? (
        <div style={{ marginTop: '0.55rem', fontSize: '0.72rem', color: 'var(--cds-text-secondary)' }}>
          {data.presenceLabel} context · health {Math.round(data.healthPercent)}%
        </div>
      ) : null}
      <Handle type="source" position={Position.Bottom} style={{ background: data.accentColor }} />
    </button>
  )
}

const nodeTypes = { clusterDashboardWorkspaceNode: ClusterDashboardWorkspaceNodeCard }

export function ClusterDashboardWorkspaceGraph({
  model,
  onSelect,
}: {
  model: ClusterDashboardWorkspaceGraphModel
  onSelect: (selection: ClusterDashboardWorkspaceGraphSelection) => void
}) {
  const graphNodes = useMemo<Array<Node<RenderNodeData>>>(() => (
    model.nodes.map((node) => ({
      ...node,
      data: {
        ...node.data,
        onSelect,
      },
    }))
  ), [model.nodes, onSelect])

  return (
    <SignalFlowGraph<RenderNodeData>
      nodes={graphNodes}
      edges={model.edges}
      nodeTypes={nodeTypes}
      wrapperClassName="cluster-dashboard-workspace__graph"
      emptyState={
        <div className="cluster-dashboard-workspace__graph-empty">
          No cluster topology nodes are currently available for this workspace.
        </div>
      }
    />
  )
}

export default ClusterDashboardWorkspaceGraph
