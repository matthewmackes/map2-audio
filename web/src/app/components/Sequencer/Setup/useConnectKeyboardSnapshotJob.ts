...
//   2. PATCH /api/engine/sequencer/slots/0 with the picked asset (asset_type +
...
export interface JobResult {
  asset: SequencerLibraryAssetModel | null
...
export function useConnectKeyboardSnapshotJob(): UseConnectKeyboardSnapshotJobResult {
  ...
  const start = useCallback(
    async ({ portName, deviceId }: RunArgs) => {
      ...
      setStage('library_scan', { status: 'running', detail: 'Querying /api/engine/sequencer/library' })
      ...
      if (pickedAsset !== null) {
        setStage('wire_slot', {
          status: 'running',
          detail: `PATCH /api/engine/sequencer/slots/0 (${pickedAsset.asset_type})`,
        })
        try {
          const patch: SequencerSlotUpdateModel = {
