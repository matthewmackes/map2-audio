...
function hidTagType(status: MaschineDaemonStatus | null): 'green' | 'red' | 'warm-gray' {
  if (!status) return 'warm-gray'
  return status.hid_device && Object.keys(status.hid_device).length > 0 ? 'green' : 'red'
}
  ...
  return (
    <Layer className="maschine-page__panel" data-testid="maschine-connection-panel">
      ...
      <p className="maschine-page__panel-copy">
        HID availability, daemon registration, and ALSA virtual-port state are refreshed every two seconds and reinforced by the dedicated Maschine websocket.
      </p>
      <div className="maschine-page__tag-row">
        <Tag type={hidTagType(status)}>HID {status?.hid_device && Object.keys(status.hid_device).length > 0 ? 'online' : 'offline'}</Tag>
