"""
Chowdhury DSP VST3 Scraper

Scrapes VST3 plugins from Chowdhury DSP GitHub repositories.
Includes ChowTape, ChowMatrix, ChowCentaur, and other plugins.
"""
...
class ChowDSPScraper(VST3ScraperBase):
    """Scraper for Chowdhury DSP plugins.

    Chowdhury DSP plugins include:
    - ChowTape: Analog tape machine simulation
    - ChowMatrix: Delay effect with feedback matrix
    - ChowCentaur: Klon Centaur pedal emulation
    - ChowPhaser: Phaser effect
    """

    BASE_URL = "https://github.com"
    API_URL = "https://api.github.com"

    # Multiple repositories to scrape
    REPOS = [
        {
            "repo": "jatinchowdhury18/AnalogTapeModel",
            "name": "ChowTape",
            "category": "Distortion",
            "description": "Analog tape machine simulation with hysteresis modeling",
            "tags": ["tape", "saturation", "analog", "vintage", "free"],
        },
        {
            "repo": "Chowdhury-DSP/ChowMatrix",
            "name": "ChowMatrix",
            "category": "Delay",
            "description": "Delay effect with infinite feedback matrix routing",
            "tags": ["delay", "feedback", "matrix", "creative", "free"],
        },
        {
            "repo": "jatinchowdhury18/KlonCentaur",
            "name": "ChowCentaur",
            "category": "Distortion",
            "description": "Klon Centaur overdrive pedal emulation",
            "tags": ["overdrive", "distortion", "pedal", "guitar", "free"],
        },
    ]

    def __init__(self):
        """Initialize ChowDSP scraper."""
        super().__init__(
            library_name="chowdsp",
    ...
    async def discover_plugins(self) -> List[VST3PluginInfo]:
        """Discover plugins from Chowdhury DSP GitHub repositories.
                        ...
                        if "linux" in name.lower():
                            if name.endswith(('.tar.gz', '.zip')) or 'x86_64' in name.lower():
                                plugin_info = VST3PluginInfo(
                                    ...
                                    platform="linux",
                                    homepage=f"https://chowdsp.com/"
                                )
                                self.discovered_plugins.append(plugin_info)
                                break  # One per repo

            except Exception as e:
                logger.warning(f"Could not fetch GitHub releases for {repo_info['name']}: {e}")
