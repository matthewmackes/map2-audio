...
Scrapes VST3 plugins from the Odin 2 synthesizer GitHub repository.
Odin 2 is a powerful semi-modular polyphonic synthesizer.
"""
...
class OdinScraper(VST3ScraperBase):
    """Scraper for Odin 2 semi-modular synthesizer.
    ...
    REPO = "TheWaveWarden/odin2"

    def __init__(self):
        """Initialize Odin scraper."""
        super().__init__(
            library_name="odin2",
    ...
    async def discover_plugins(self) -> List[VST3PluginInfo]:
        """Discover Odin 2 from GitHub releases.
        ...
        try:
                    ...
                    if "linux" in name.lower():
                        if name.endswith(('.tar.gz', '.zip', '.deb')) or 'x86_64' in name.lower():
                            plugin_info = VST3PluginInfo(
                                ...
                                version=version,
                                license="GPL-3.0",
                                description="24-voice semi-modular polyphonic synthesizer with 11 oscillator types",
                                tags=["synth", "polyphonic", "modular", "oscillator", "free"],
            ...
            logger.warning(f"Could not fetch GitHub releases for Odin 2: {e}")
