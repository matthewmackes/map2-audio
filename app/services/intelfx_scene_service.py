"""
IntelFX Scene, Setlist, and Morph service.
...
Data persisted atomically at ~/.map2/intelfx_scenes.json
"""
...
_DEFAULT_SCENES_PATH = Path.home() / ".map2" / "intelfx_scenes.json"
...
class IntelFXScene:
    ...
    # Chain integration: position in effects chain and audio routing params
...
@dataclass
class IntelFXSetlist:
    id: str
    name: str
    songs: List[IntelFXSong] = field(default_factory=list)
...
def _apply_curve(t: float, curve: str) -> float:
    """Map t in [0,1] through a named easing curve."""
    t = max(0.0, min(1.0, t))
    if curve == "ease_in":
        return t * t
    if curve == "ease_out":
...
class IntelFXSceneService:
    """Manages IntelFX scene snapshots, morphing, and setlists."""

    def __init__(self, scenes_path: Optional[Path] = None) -> None:
        self._path: Path = scenes_path or _DEFAULT_SCENES_PATH
        self._scenes: Dict[str, IntelFXScene] = {}
        self._setlists: Dict[str, IntelFXSetlist] = {}
        # Current morph job
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            for raw in data.get("setlists", []):
                songs = [IntelFXSong(**sg) for sg in raw.get("songs", [])]
                sl = IntelFXSetlist(id=raw["id"], name=raw["name"], songs=songs)
    ...
    def capture_scene(self, name: str, tags: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # Capture chain integration state
    ...
    def get_scene(self, scene_id: str) -> IntelFXScene:
        try:
    ...
    async def recall_scene(self, scene_id: str) -> Dict[str, Any]:
        ...
        # Restore chain integration state
    ...
    def create_setlist(self, name: str) -> Dict[str, Any]:
        sl = IntelFXSetlist(id=str(uuid.uuid4()), name=name)
        self._setlists[sl.id] = sl
        self._save()
    ...
    def get_setlist(self, setlist_id: str) -> IntelFXSetlist:
        try:
    ...
    def update_setlist(self, setlist_id: str, name: Optional[str] = None, songs: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        ...
        if songs is not None:
            sl.songs = [IntelFXSong(**sg) for sg in songs]
        self._save()
        return _setlist_to_dict(sl)
...
def _scene_to_dict(s: IntelFXScene) -> Dict[str, Any]:
    return {
        "id": s.id,
        "name": s.name,
        "program": s.program,
...
def _setlist_to_dict(sl: IntelFXSetlist) -> Dict[str, Any]:
    return {
        "id": sl.id,
        "name": sl.name,
        "songs": [asdict(sg) for sg in sl.songs],
...
_intelfx_scene_service: Optional[IntelFXSceneService] = None


def get_intelfx_scene_service(scenes_path: Optional[Path] = None) -> IntelFXSceneService:
    global _intelfx_scene_service
    if _intelfx_scene_service is None:
        _intelfx_scene_service = IntelFXSceneService(scenes_path)
    return _intelfx_scene_service
