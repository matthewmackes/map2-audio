...
@dataclass
class McuReconnectNotification:
    severity: str
    title: str
    subtitle: str
    emitted_at: str
...
class McuSurfaceDaemon:
    def __init__(
        ...
        *,
        ...
        self._status: dict[str, Any] = {
            ...
            "last_destination_ports": [],
            "last_transport_owner": None,
            "notification": None,
        }
    ...
    async def ensure_started(self) -> None:
        ...
        self._task = loop.create_task(self._run(), name="mcu-surface-daemon")
    ...
    async def _publish_state(
        ...
        *,
        ...
        notification: McuReconnectNotification | None = None,
        error: str | None = None,
    ) -> None:
        ...
        await _maybe_await(self._emit("mcu_surface:daemon", self.snapshot()))
    ...
    async def _run(self) -> None:
        while True:
            try:
                ...
                logger.warning("MCU surface daemon poll failed: %s", exc, exc_info=True)
    ...
    async def tick(self) -> None:
        async with self._lock:
            ...
            if available and not self._available:
                ...
                reconnect_note = McuReconnectNotification(
                    severity="info",
                    title="MCU surface reconnected",
                    subtitle="Re-pushing the focused plugin bank, motor faders, scribble strip, and transport state.",
                    emitted_at=now,
                )
                await self._publish_state(
                    state="repushing",
                ...
                self._status["last_destination_ports"] = list(repush_result.get("destination_ports") or [])
                transport = repush_result.get("transport") if isinstance(repush_result.get("transport"), dict) else {}
                self._status["last_transport_owner"] = transport.get("active_owner")
                done_note = McuReconnectNotification(
                    severity="info",
                    title="MCU surface state restored",
                    subtitle=str(repush_result.get("status_label") or "Focused plugin bank and transport state re-pushed."),
            ...
            if not available and self._available:
                self._available = False
                note = McuReconnectNotification(
                    severity="warning",
                    title="MCU surface disconnected",
                    subtitle="Waiting for the controller to reconnect so MAP2 can re-push the focused bank and transport state.",
                    emitted_at=now,
                )
                await self._publish_state(
                    state="reconnecting",
