...
from app.services.sequencer_service import SequencerService, get_sequencer_service
from app.services.snapshot_runtime_state_service import resolve_local_node_id

_SEQUENCER_EXTENSION_KEY = "sequencer"
...
def _upsert_brain_projection(extensions: dict[str, Any] | None, projection: dict[str, Any]) -> dict[str, Any]:
    ...
    brain_extension = next_extensions.get(_SEQUENCER_EXTENSION_KEY)
    ...
    next_extensions[_SEQUENCER_EXTENSION_KEY] = brain_extension
...
class SequencerAuthoritySyncService:
    def __init__(
        self,
        authority_service: AudioStateAuthorityService | None = None,
        sequencer_service: SequencerService | None = None,
        node_id: str | None = None,
    ) -> None:
        self.authority_service = authority_service or AudioStateAuthorityService()
        self.sequencer_service = sequencer_service or get_sequencer_service()
    ...
    async def sync_instance(
        ...
        *,
        ...
        brain_state = self.sequencer_service.get_state(instance_id=instance_id, plugin_position=plugin_position)
        ...
        observation = AudioStateObservation(
            ...
            runtime_metrics={
                "sequencer": {
                    ...
                    "instance_count": len(
                        (
                            committed_result.value.extensions.get(_SEQUENCER_EXTENSION_KEY, {})
    ...
    async def restore_instance(
        ...
        *,
        ...
        runtime_instance_id = self.sequencer_service.resolve_runtime_instance_id(
            instance_id=instance_id,
            plugin_position=plugin_position,
        )
        try:
            ...
            if "No committed authoritative audio state exists" in str(exc):
                return self.sequencer_service.get_state(instance_id=instance_id, plugin_position=plugin_position)
            raise
        ...
        return self.sequencer_service.replace_state(
            projected_state,
            instance_id=instance_id,
            plugin_position=plugin_position,
        )
    ...
    def reconcile_runtime_with_extensions(
        ...
        *,
        ...
        if not isinstance(next_extensions, dict) or _SEQUENCER_EXTENSION_KEY not in next_extensions:
            return {
                "reconciled": False,
        ...
        for runtime_instance_id in sorted(next_instances):
            ...
            restored_state = self.sequencer_service.replace_state(
                projected_state,
                instance_id=instance_id,
                plugin_position=plugin_position,
            )
        ...
        for runtime_instance_id in sorted(set(current_instances) - set(next_instances)):
            ...
            if not isinstance(projection, dict):
                continue
            instance_id, plugin_position = _projection_scope(projection)
            reset_state = self.sequencer_service.reset_state(
                instance_id=instance_id,
                plugin_position=plugin_position,
            )
            reset.append(
