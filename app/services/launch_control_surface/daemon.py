...
@dataclass
class LaunchControlReconnectNotification:
    severity: str
    title: str
    subtitle: str
    emitted_at: str
...
class LaunchControlSurfaceDaemon:
    def __init__(
        self,
        *,
    ...
    async def ensure_started(self) -> None:
        ...
        self._task = loop.create_task(self._run(), name="launch-control-surface-daemon")
    ...
    async def _publish_state(
        ...
        *,
        ...
        notification: LaunchControlReconnectNotification | None = None,
        error: str | None = None,
    ) -> None:
        ...
        await _maybe_await(self._emit("launch_control_surface:daemon", self.snapshot()))
    ...
    async def _run(self) -> None:
        while True:
            try:
                ...
                logger.warning("Launch Control daemon poll failed: %s", exc, exc_info=True)
    ...
    async def tick(self) -> None:
        async with self._lock:
            ...
            if available and not self._available:
                ...
                reconnect_note = LaunchControlReconnectNotification(
                    severity="info",
                    title="Launch Control reconnected",
                    subtitle="Re-pushing the MAP2 template, current snapshot mappings, and LED state.",
                    emitted_at=now,
                )
                await self._publish_state(
                    state="repushing",
                ...
                done_note = LaunchControlReconnectNotification(
                    severity="info",
                    title="Launch Control state restored",
                    subtitle=str(repush_result.get("status_label") or "Live snapshot mappings and LED state re-pushed."),
            ...
            if not available and self._available:
                self._available = False
                note = LaunchControlReconnectNotification(
                    severity="warning",
                    title="Launch Control disconnected",
                    subtitle="Waiting for the controller to reconnect so MAP2 can re-push the current snapshot state.",
                    emitted_at=now,
                )
                await self._publish_state(
                    state="reconnecting",
