"""Brain capture buffer — T2461-A6 foundation.

Process-singleton ring buffer that the Brain Step view writes to and
the MIDI Assignments wizard's Calibrate step reads back. The buffer
holds the most recent N seconds of Brain output frames (slot id +
peak dB + RMS dB + clip flag + timestamp) so the wizard can render
the Brain response alongside the MIDI source stream during a
calibrate session.
...
Lifecycle:
  ...
  - ``record_frame(slot_id, peak_db, rms_db, clipping, ts)`` is called
    by the Brain meter pipeline each WS tick to append one frame for
...
testing; in production the existing `BrainMeteringService` calls
...
_singleton: BrainCaptureBuffer | None = None
_singleton_lock = threading.Lock()


def get_brain_capture_buffer() -> BrainCaptureBuffer:
    global _singleton
    with _singleton_lock:
        if _singleton is None:
            _singleton = BrainCaptureBuffer()
        return _singleton


def reset_brain_capture_buffer_for_tests() -> BrainCaptureBuffer:
    global _singleton
    with _singleton_lock:
        _singleton = BrainCaptureBuffer()
        return _singleton
