...
Provides:
- IntelFX parameter registry loading
- Shadow state persistence (~/.map2/intelfx_shadow.json)
- MIDI map persistence (~/.map2/intelfx_midi_maps.json)
...
- WebSocket event fan-out for /api/intelfx/ws and global websocket topics
"""
...
# ---------------------------------------------------------------------------
# Optional software simulator (set INTELFX_SIMULATOR=1 in env to activate)
# ---------------------------------------------------------------------------
...
class IntelFXService:
    """Stateful IntelFX bridge for REST + WebSocket control."""

    # Rocktron manufacturer ID header used consistently for local encode/decode round-trips.
    ...
    def __init__(
        ...
        self.registry_path = registry_path or (repo_root / "app" / "data" / "intelfx_params.json")

        state_dir = Path.home() / ".map2"
        state_dir.mkdir(parents=True, exist_ok=True)
        self.shadow_path = shadow_path or (state_dir / "intelfx_shadow.json")
        self.library_path = library_path or (state_dir / "intelfx_library.json")
        self.midi_maps_path = midi_maps_path or (state_dir / "intelfx_midi_maps.json")
        ...
        # MidiHub bridge ports for IntelFX consumer migration.
        self._hub = None
        self._hub_subscriber_id = f"intelfx_service:{id(self)}"
        self._hub_input_port_id = "consumer:intelfx_in"
        self._hub_output_port_id = "consumer:intelfx_out"
    ...
    def _init_midi_hub_bridge(self) -> None:
        """Best-effort registration of IntelFX endpoints with MidiHub."""
        ...
        try:
            ...
            logger.debug("IntelFX MidiHub bridge unavailable: %s", exc)
    ...
    def _load_registry(self) -> None:
        if not self.registry_path.exists():
            raise FileNotFoundError(f"IntelFX registry not found: {self.registry_path}")
    ...
    def _load_shadow_state(self) -> None:
        ...
        try:
            ...
            logger.warning("Failed to load IntelFX shadow state, using defaults: %s", exc)
            self.shadow_state = defaults
    ...
    def _load_library(self) -> None:
        if self.library_path.exists():
            try:
                ...
                logger.warning("Failed to parse IntelFX library file, resetting: %s", exc)
    ...
    def _load_midi_maps(self) -> None:
        if self.midi_maps_path.exists():
            try:
                ...
                logger.warning("Failed to parse IntelFX MIDI maps file, resetting: %s", exc)
    ...
    def encode_param_sysex(self, param_id: str, value: float) -> List[int]:
        ...
        if param is None:
            raise KeyError(f"Unknown IntelFX param id: {param_id}")
    ...
    def decode_param_sysex(self, message: List[int]) -> Optional[Dict[str, Any]]:
        # Accept canonical MAP2 frames plus common Rocktron hardware header variants.
        # Rocktron SysEx: F0 00 01 56 [device_id] [command] [addr x4] [lo hi] F7
        if len(message) < 10:
            return None
        if int(message[0]) & 0xFF != 0xF0:
            return None
        # Verify Rocktron manufacturer bytes at positions [1,2,3]
        if (int(message[1]) & 0x7F) != 0x00:
            return None
        if (int(message[2]) & 0x7F) != 0x01:
            return None
        if (int(message[3]) & 0x7F) != 0x56:
            return None
        if int(message[-1]) & 0xFF != self._SYSEX_SUFFIX:
            return None

        # Rocktron format: device_id at [4], command at [5], address bytes at [6..9], value at [10..11]
        # Most common frames:
        #   F0 00 01 56 dd cc [addr x4] [lo hi] F7  -> offset 6
        # Some units may use a shorter header -> offset 5
        candidate_offsets: List[int] = [6, 5]
    ...
    def decode_extended_sysex(self, message: List[int]) -> Optional[Dict[str, Any]]:
        """Decode known long-form Rocktron IntelFX status/report frames."""
        if len(message) < 8:
            return None
        if int(message[0]) & 0xFF != 0xF0:
            return None
        # Verify Rocktron manufacturer bytes
        if (int(message[1]) & 0x7F) != 0x00:
            return None
        if (int(message[2]) & 0x7F) != 0x01:
            return None
        ...
        # F0 00 01 56 dd 12 00 12 01 00 F7
        ...
        # Example observed frame (program status):
        # F0 00 01 56 dd 09 00 01 02 ... <program_lsn> <program_msn> ... F7
        if len(message) >= 14 and (int(message[7]) & 0x7F, int(message[8]) & 0x7F) == (0x01, 0x02):
            program_lsn = int(message[12]) & 0x0F
            program_msn = int(message[13]) & 0x0F
            return {
        ...
        # Example observed frame (panel status):
        # F0 00 01 56 dd 09 00 01 01 ... <value_lsn> <value_msn> ... F7
        if len(message) >= 14 and (int(message[7]) & 0x7F, int(message[8]) & 0x7F) == (0x01, 0x01):
            value_lsn = int(message[12]) & 0x0F
            value_msn = int(message[13]) & 0x0F
            return {
    ...
    async def get_midi_ports(self) -> Dict[str, Any]:
        if not RTMIDI_AVAILABLE:
            return {
                "rtmidi_available": False,
                "inputs": [{"index": 0, "name": "Virtual IntelFX Input", "connected": False}],
                "outputs": [{"index": 0, "name": "Virtual IntelFX Output", "connected": False}],
        ...
        try:
            ...
            for index in range(midi_in.get_port_count()):
                ...
                lowered = name.lower()
                if recommended_in is None and ("intelfx" in lowered or "intellifex" in lowered or "rocktron" in lowered):
            ...
            logger.warning("IntelFX MIDI input probe failed: %s", exc)
        finally:
        ...
        try:
            ...
            for index in range(midi_out.get_port_count()):
                ...
                lowered = name.lower()
                if recommended_out is None and ("intelfx" in lowered or "intellifex" in lowered or "rocktron" in lowered):
            ...
            logger.warning("IntelFX MIDI output probe failed: %s", exc)
        finally:
    ...
    async def connect_midi(
        ...
        hint = (name_hint or "intelfx").lower()

        if input_port_index is None:
            for item in ports["inputs"]:
                if hint in item["name"].lower() or "rocktron" in item["name"].lower():
        ...
        if input_port_index is None or output_port_index is None:
            return {"connected": False, "detail": "No IntelFX-like MIDI ports found"}
        ...
        if _SIMULATOR_ACTIVE:
            ...
            if self._hub is not None:
                try:
                    ...
                    logger.debug("Failed to subscribe IntelFX hub bridge: %s", exc)
            self._midi_poll_task = asyncio.create_task(self._midi_poll_loop(), name="intelfx_midi_poll")
        ...
        try:
            ...
            logger.error("Failed to connect IntelFX MIDI ports: %s", exc)
            await self.disconnect_midi()
    ...
    def _on_hub_message(self, message: MidiMessage) -> None:
        """Receive MIDI routed to IntelFX via MidiHub and process it locally."""
        ...
        try:
            ...
            logger.debug("IntelFX hub dispatch scheduling failed: %s", exc)
    ...
    def _emit_hub_message(self, payload: List[int], *, direction: str, kind: str) -> None:
        ...
        if self._hub is None:
            return
        try:
            destination = self._hub_input_port_id if direction == "in" else "intelfx:device"
            self._hub.send(
                ...
                metadata={"bridge": "intelfx_service", "direction": direction, "kind": kind},
            )
        except Exception:
            return
    ...
    async def _check_drift(self) -> None:
        ...
        if current_checksum == self._expected_checksum:
            if self._drift_status != "clean":
                self._drift_status = "clean"
                await self._publish_event("intelfx:drift_status", {"status": "clean"})
            return
        ...
        if mismatch_count >= self._drift_mismatch_threshold:
            logger.warning("IntelFX drift detected (mismatch bits=%d)", mismatch_count)
            self._drift_status = "resyncing"
            await self._publish_event(
                "intelfx:drift_detected",
    ...
    async def release_write_lock(self, client_id: str) -> Dict[str, Any]:
        ...
        self._writer_client_id = None
        await self._publish_event("intelfx:writer_changed", {"client_id": None})
    ...
    async def _midi_poll_loop(self) -> None:
        while self._running and self._midi_in is not None:
            try:
                ...
                logger.debug("IntelFX MIDI poll loop error: %s", exc)
    ...
    async def handle_incoming_sysex(self, message: List[int]) -> Optional[Dict[str, Any]]:
        ...
        if extended is not None:
            ...
            if frame_type == "program_status":
                ...
                # Some IntelFX front panels emit only long-form 01 02 status frames during
            ...
            if frame_type == "heartbeat":
                ...
                await self._publish_event("intelfx:heartbeat", {})
                return extended
    ...
    async def handle_incoming_cc(self, channel: int, cc: int, value: int) -> Dict[str, Any]:
        ...
        await self._publish_event("intelfx:midi_cc", payload)
    ...
    async def _dispatch_param_update(self, param_id: str, value: float, source: str) -> None:
        ...
        if self._midi_out is not None:
            try:
                ...
                logger.debug("IntelFX send_message failed for %s: %s", param_id, exc)
    ...
    async def set_program(self, program: int) -> Dict[str, Any]:
        max_slots = int(self.registry.get("program_management", {}).get("program_slots", 256))
        ...
        if self._midi_out is not None:
            try:
                ...
                logger.debug("IntelFX program change send failed: %s", exc)
    ...
    async def activate_midi_map(self, map_id: str) -> Dict[str, Any]:
        ...
        if not any(str(item.get("id")) == str(map_id) for item in maps):
            raise ValueError(f"Unknown IntelFX MIDI map id: {map_id}")

        payload["active_map_id"] = str(map_id)
        self._write_midi_maps(payload)
        await self._publish_event(
            "intelfx:midi_map_activated",
            {"map_id": map_id},
        )
    ...
    async def start_dump_all(self) -> Dict[str, Any]:
        ...
        asyncio.create_task(self._run_dump_job(job_id), name=f"intelfx_dump_{job_id}")
        ...
        for index, prog in enumerate(programs):
            try:
                ...
                if program_number is None:
                    ...
                    program_number = next(
                        (n for n in range(256) if n not in used), len(entries)
                    )
    ...
    async def audition_program(self, program: int) -> Dict[str, Any]:
        ...
        self._audition_revert_task = asyncio.create_task(
            ...
            name="intelfx_audition_revert",
        )
        await self._publish_event(
            "intelfx:audition_started",
            {
                "program": program,
    ...
    async def audition_revert(self) -> Dict[str, Any]:
        ...
        await self._publish_event("intelfx:audition_reverted", {"program": prev})
        return result
    ...
    async def audition_confirm(self) -> Dict[str, Any]:
        ...
        if hasattr(self, "_audition_revert_task") and self._audition_revert_task:
            try:
                ...
                pass
            self._audition_revert_task = None
        await self._publish_event("intelfx:audition_confirmed", {"program": self.current_program})
    ...
    async def _publish_event(self, event_type: str, data: Dict[str, Any]) -> None:
        ...
        try:
            await ws_manager.broadcast_json(message, topic="intelfx")
        except Exception as exc:
            logger.debug("IntelFX ws_manager broadcast failed: %s", exc)

        # Direct IntelFX websocket subscribers
        stale: List[str] = []
