    ...
    async def _update_node(self, node_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                try:
                    client.execute_command("lvcreate -L5G -s -n pre_update_snapshot /dev/vg/root", timeout=30)
                    job.snapshot_id = "pre_update_snapshot"
                except:
                    self.logger.warning(f"Could not create snapshot on {node_id}")
            ...
            if not await self._validate_node_post_update(node_id):
                ...
                # Try rollback
                if not self.dry_run:
                    await self._rollback_node(node_id)
    ...
    async def _validate_node_post_update(self, node_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                if hasattr(self, 'health_aggregator'):
                    health_score = self.health_aggregator.get_node_health(node_id)
