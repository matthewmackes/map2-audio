"""Performance Sequencer model definitions."""
...
SequencerRuntimeResource = Literal[
    "state",
    "transport",
    "slot",
    "layers",
...
class SequencerTransportStateModel(BaseModel):
...
class SequencerSlotModel(BaseModel):
...
class SequencerLayerModel(BaseModel):
    layer_id: str
    name: str
...
class SequencerPatternSummaryModel(BaseModel):
...
class SequencerSequenceLaneSummaryModel(BaseModel):
    slot_id: int = Field(..., ge=0, le=15)
    name: str
    length: int = Field(16, ge=1, le=64)
    swing: int = Field(0, ge=0, le=100)
...
class SequencerSequenceModel(BaseModel):
    ...
    patterns: list[SequencerPatternSummaryModel] = Field(default_factory=list)
    lanes: list[SequencerSequenceLaneSummaryModel] = Field(default_factory=list)
    fill_mode: str = "manual"
    song_entry_count: int = Field(0, ge=0)


class SequencerSongEntryModel(BaseModel):
    pattern_id: int = Field(..., ge=0, le=127)
    variation: int = Field(0, ge=0, le=10)
...
class SequencerSongStateModel(BaseModel):
    entries: list[SequencerSongEntryModel] = Field(default_factory=list)
    loop: bool = False


class SequencerMixerBusModel(BaseModel):
...
class SequencerMasterSectionModel(BaseModel):
...
class SequencerMixerStateModel(BaseModel):
    buses: list[SequencerMixerBusModel] = Field(default_factory=list)
    master: SequencerMasterSectionModel = Field(default_factory=SequencerMasterSectionModel)


class SequencerKeyboardZoneModel(BaseModel):
    zone_id: str
    name: str
...
class SequencerTriggerProfileModel(BaseModel):
    profile_id: str
    name: str
...
class SequencerControllerAssignmentModel(BaseModel):
    source: str
    target: str
    mode: str = "absolute"
    enabled: bool = True
...
class SequencerInputsStateModel(BaseModel):
    keyboard_zones: list[SequencerKeyboardZoneModel] = Field(default_factory=list)
    trigger_profiles: list[SequencerTriggerProfileModel] = Field(default_factory=list)
    controller_assignments: list[SequencerControllerAssignmentModel] = Field(default_factory=list)


class SequencerLibraryAssetModel(BaseModel):
    asset_id: str
    name: str
    ...
    # surfaces these as device-pin Tags + a "Used in N Sequencer entries"
    # reverse-link on the matching Hardware Store cards.
    authored_with_devices: list[str] = Field(default_factory=list)


class SequencerLibraryCollectionModel(BaseModel):
    collection_id: str
    label: str
    asset_count: int = Field(0, ge=0)
    assets: list[SequencerLibraryAssetModel] = Field(default_factory=list)


class SequencerLibraryStateModel(BaseModel):
    collections: list[SequencerLibraryCollectionModel] = Field(default_factory=list)
    featured_assets: list[str] = Field(default_factory=list)
    last_scan_iso: str = ""


class SequencerSampleEditorStateModel(BaseModel):
...
class SequencerSnapshotIntegrationModel(BaseModel):
...
class SequencerKeyboardQualificationModel(BaseModel):
    ready: bool = False
...
class SequencerTriggerQualificationModel(BaseModel):
    ready: bool = False
...
class SequencerSequenceQualificationModel(BaseModel):
    ready: bool = False
...
class SequencerRoutingQualificationModel(BaseModel):
    ready: bool = False
...
class SequencerControllerQualificationModel(BaseModel):
    ...
    keyboard: SequencerKeyboardQualificationModel = Field(default_factory=SequencerKeyboardQualificationModel)
    triggers: SequencerTriggerQualificationModel = Field(default_factory=SequencerTriggerQualificationModel)
    sequence: SequencerSequenceQualificationModel = Field(default_factory=SequencerSequenceQualificationModel)
    routing: SequencerRoutingQualificationModel = Field(default_factory=SequencerRoutingQualificationModel)
    summary: str = ""
    issues: list[str] = Field(default_factory=list)


class SequencerDiagnosticsModel(BaseModel):
    ...
    controller_qualification: SequencerControllerQualificationModel = Field(default_factory=SequencerControllerQualificationModel)
    updated_at_iso: str = Field(default_factory=utcnow_iso)


class SequencerStateModel(BaseModel):
    instance_id: str
    product_name: str = "Sequencer"
    set_name: str = "Init Sequencer"
    active_slot: int = Field(0, ge=0, le=15)
    active_layer_id: str = "main-stack"
    # T2442: Sequencer Overview tabs (performance/console/step/split) are now first-class
    ...
    transport: SequencerTransportStateModel = Field(default_factory=SequencerTransportStateModel)
    slots: list[SequencerSlotModel] = Field(default_factory=list, min_length=16, max_length=16)
    layers: list[SequencerLayerModel] = Field(default_factory=list)
    sequence: SequencerSequenceModel = Field(default_factory=SequencerSequenceModel)
    song: SequencerSongStateModel = Field(default_factory=SequencerSongStateModel)
    mixer: SequencerMixerStateModel = Field(default_factory=SequencerMixerStateModel)
    inputs: SequencerInputsStateModel = Field(default_factory=SequencerInputsStateModel)
    library: SequencerLibraryStateModel = Field(default_factory=SequencerLibraryStateModel)
    sample_editor: SequencerSampleEditorStateModel = Field(default_factory=SequencerSampleEditorStateModel)
    diagnostics: SequencerDiagnosticsModel = Field(default_factory=SequencerDiagnosticsModel)
    snapshot_integration: SequencerSnapshotIntegrationModel = Field(default_factory=SequencerSnapshotIntegrationModel)


class SequencerRuntimeScopeModel(BaseModel):
    runtime_instance_id: str
    instance_id: str | None = None
    plugin_position: int | None = Field(default=None, ge=0)


class SequencerRuntimeEventModel(BaseModel):
    resource: SequencerRuntimeResource
    scope: SequencerRuntimeScopeModel
    state: SequencerStateModel


class SequencerStateUpdateModel(BaseModel):
...
class SequencerTransportUpdateModel(BaseModel):
...
class SequencerSlotUpdateModel(BaseModel):
...
class SequencerLayersUpdateModel(BaseModel):
    layers: list[SequencerLayerModel] = Field(default_factory=list)


class SequencerSequenceUpdateModel(BaseModel):
    sequence: SequencerSequenceModel


class SequencerSongUpdateModel(BaseModel):
    song: SequencerSongStateModel


class SequencerMixerUpdateModel(BaseModel):
    mixer: SequencerMixerStateModel


class SequencerInputsUpdateModel(BaseModel):
    inputs: SequencerInputsStateModel


class SequencerSampleEditorUpdateModel(BaseModel):
    slot_id: int = Field(..., ge=0, le=15)
...
__all__ = [
    ...
    "SequencerRuntimeEventModel",
    "SequencerRuntimeResource",
    "SequencerRuntimeScopeModel",
