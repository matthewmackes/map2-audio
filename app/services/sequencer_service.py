"""
Performance Sequencer state service.
...
_DEFAULT_BRAIN_ROOT = Path(
    os.environ.get("MAP2_SEQUENCER_ROOT", Path.home() / ".map2" / "sequencer")
)
...
class SequencerService(Singleton):
    def __init__(self, root_path: Path | None = None) -> None:
        ...
        self._instances: dict[str, SequencerStateModel] = {}
        self._lock = threading.RLock()
        self._library_cache: SequencerLibraryStateModel | None = None
    ...
    def _default_slots(self) -> list[SequencerSlotModel]:
        slots: list[SequencerSlotModel] = []
    ...
    def _default_layers(self) -> list[SequencerLayerModel]:
        return [
            SequencerLayerModel(
                layer_id="main-stack",
                name="Main Stack",
    ...
    def _default_sequence(self, slots: list[SequencerSlotModel]) -> SequencerSequenceModel:
        patterns = [
            SequencerPatternSummaryModel(
                pattern_id=index,
    ...
    def _default_song(self) -> SequencerSongStateModel:
        return SequencerSongStateModel(
            entries=[
                SequencerSongEntryModel(pattern_id=0, variation=0, repeat_count=2, label="Intro"),
                SequencerSongEntryModel(pattern_id=1, variation=1, repeat_count=4, label="Main"),
            ],
            loop=False,
        )

    def _default_mixer(self) -> SequencerMixerStateModel:
        return SequencerMixerStateModel(
            buses=[
                SequencerMixerBusModel(
                    bus_id=bus_id,
                    name=f"Bus {bus_id + 1}",
            ...
            master=SequencerMasterSectionModel(),
        )

    def _default_inputs(self) -> SequencerInputsStateModel:
        return SequencerInputsStateModel(
            keyboard_zones=[
                SequencerKeyboardZoneModel(
            ...
            controller_assignments=[
                SequencerControllerAssignmentModel(source="modwheel", target="layer:main-stack:blend"),
                SequencerControllerAssignmentModel(source="aftertouch", target="slot:8:filter"),
                SequencerControllerAssignmentModel(source="pedal_1", target="transport:start-stop"),
            ],
        )

    def _scan_soundfont_assets(self) -> list[SequencerLibraryAssetModel]:
        soundfonts: list[SequencerLibraryAssetModel] = []
        for root in (
    ...
    def _scan_sfz_assets(self) -> list[SequencerLibraryAssetModel]:
        sfz_assets: list[SequencerLibraryAssetModel] = []
        for root in (
    ...
    def _scan_sample_assets(self) -> list[SequencerLibraryAssetModel]:
        sample_assets: list[SequencerLibraryAssetModel] = []
        for root in (
    ...
    def _scan_kit_assets(self) -> list[SequencerLibraryAssetModel]:
        kits: list[SequencerLibraryAssetModel] = []
    ...
    def _scan_library_state(self) -> SequencerLibraryStateModel:
        ...
        return SequencerLibraryStateModel(
            collections=collections,
            featured_assets=featured_assets,
            last_scan_iso=_utcnow_iso(),
        )

    def _build_library_state(self) -> SequencerLibraryStateModel:
        now = time.monotonic()
        if (
    ...
    def _build_runtime_library(
        self,
        existing_library: SequencerLibraryStateModel | None = None,
    ) -> SequencerLibraryStateModel:
    ...
    def _apply_asset_overlays(self, library: SequencerLibraryStateModel) -> SequencerLibraryStateModel:
        ...
        if asset_path and asset_path.lower().endswith(".wav"):
            ...
            if path.exists():
                try:
                    ...
                    waveform_available = False
        return SequencerSampleEditorStateModel(
            slot_id=slot_id,
    ...
    def _build_keyboard_qualification(self, state: SequencerStateModel) -> SequencerKeyboardQualificationModel:
        ...
        return SequencerKeyboardQualificationModel(
            ready=ready,
    ...
    def _build_trigger_qualification(self, state: SequencerStateModel) -> SequencerTriggerQualificationModel:
        ...
        return SequencerTriggerQualificationModel(
            ready=ready,
    ...
    def _build_sequence_qualification(self, state: SequencerStateModel) -> SequencerSequenceQualificationModel:
        ...
        return SequencerSequenceQualificationModel(
            ready=ready,
    ...
    def _build_routing_qualification(self, state: SequencerStateModel) -> SequencerRoutingQualificationModel:
        ...
        if controller_assignment_count == 0:
            issues.append("No controller assignments are available to drive routed Sequencer actions.")

        ready = not issues
        summary = f"{len(used_buses)} buses · {len(output_pairs)} outputs · {controller_assignment_count} assignments"
        return SequencerRoutingQualificationModel(
            ready=ready,
    ...
    def _build_controller_qualification(self, state: SequencerStateModel) -> SequencerControllerQualificationModel:
        ...
        if not scope_binding_ready:
            issues.append("Authority state IDs are not bound to the scoped Sequencer instance.")
        if not tier_a_runtime_locked:
            issues.append("Tier A runtime locks are not currently satisfied for this scoped Sequencer instance.")
        ...
        return SequencerControllerQualificationModel(
    ...
    def _default_state(self, instance_key: str) -> SequencerStateModel:
        ...
        return SequencerStateModel(
            ...
            diagnostics=SequencerDiagnosticsModel(updated_at_iso=_utcnow_iso()),
            snapshot_integration=SequencerSnapshotIntegrationModel(
    ...
    def _load_instance(self, instance_key: str) -> SequencerStateModel:
        ...
        if path.exists():
            ...
            # four Sequencer Overview subviews are now first-class. Default migrated
            ...
            state = SequencerStateModel.model_validate(payload)
        else:
    ...
    def _persist_state(self, state: SequencerStateModel) -> None:
    ...
    def _refresh_derived_state(self, state: SequencerStateModel, slot_id: int | None = None) -> SequencerStateModel:
    ...
    def replace_state(
        self,
        state: SequencerStateModel | dict[str, Any],
        *,
        instance_id: str | int | None = None,
        plugin_position: int | None = None,
    ) -> dict[str, Any]:
        with self._lock:
            ...
            next_state = SequencerStateModel.model_validate(state).model_copy(deep=True)
    ...
    def get_runtime_event(
        ...
        with self._lock:
            state = self._load_instance(self._build_instance_key(instance_id, plugin_position))
            self._refresh_derived_state(state)
            return SequencerRuntimeEventModel(
                resource=resource,
    ...
    def _build_drum_import_sequence(
        ...
        *,
        ...
        pattern_summaries: list[SequencerPatternSummaryModel] = []
        active_pattern = None
        ...
        lane_summaries: list[SequencerSequenceLaneSummaryModel] = []
    ...
    def import_from_drums(
        ...
        *,
        ...
        with self._lock:
            ...
            state.song = SequencerSongStateModel(
                entries=[
                    SequencerSongEntryModel(
            ...
            state.mixer = SequencerMixerStateModel(
                buses=[
                    SequencerMixerBusModel(
            ...
            if any(
                ...
                logger.info(
                    "Imported Drum Machine transport bridge assignments into Sequencer.",
                    extra={
            ...
            state.set_name = f"{imported_kit_name} Sequencer Import"
            ...
            if hybrid_slots:
                state.diagnostics.warnings.append(
                    f"Pads {', '.join(hybrid_slots[:6])} import hybrid Drum Machine sources into Sequencer slot state."
                )
            cv_gate_slots = [
                str(index + 1)
    ...
    def import_from_synthforge(
        ...
        *,
        ...
        with self._lock:
            ...
            keyboard_zones: list[SequencerKeyboardZoneModel] = []
            controller_assignments: list[SequencerControllerAssignmentModel] = []
            warnings: list[str] = []
...
def get_sequencer_service() -> SequencerService:
    return SequencerService.get_instance()
