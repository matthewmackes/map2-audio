...
class LaunchControlMappingPatchRequest(BaseModel):
...
def _build_projection(
    *,
    ...
    if isinstance(live_snapshot_payload, dict):
        for entry in service._iter_live_input_mappings(live_snapshot_payload):
            controls.append(
                {
                    **dict(entry),
                    "assignment_summary": _assignment_summary(entry),
                }
            )
    return {
        "snapshot": (
            {
                "id": int(live_snapshot_payload.get("id") or 0),
                "name": str(live_snapshot_payload.get("name") or ""),
            }
            if isinstance(live_snapshot_payload, dict)
            else None
        ),
        "controls": controls,
        "template_state_by_port": service_state.get("template_state_by_port", {}),
...
@router.get("/status")
async def get_launch_control_status() -> dict[str, Any]:
    service = get_launch_control_surface_service()
    ...
    return {
        ...
        "state": {
            ...
            "template_state_by_port": state.get("template_state_by_port", {}),
...
async def get_launch_control_projection() -> dict[str, Any]:
    service = get_launch_control_surface_service()
...
@router.post("/mapping")
async def patch_live_launch_control_mapping(request: LaunchControlMappingPatchRequest) -> dict[str, Any]:
    ...
    async with get_session() as session:
        ...
        launch_control = extensions.setdefault("launch_control", {})
        if not isinstance(launch_control, dict):
            extensions["launch_control"] = {}
            launch_control = extensions["launch_control"]
        mappings = launch_control.get("mappings")
        if not isinstance(mappings, list):
            mappings = []
            launch_control["mappings"] = mappings

        target = next(
            (
                entry
                for entry in mappings
                if isinstance(entry, dict) and str(entry.get("control_id") or entry.get("id") or "").strip() == control_id
            ),
            None,
        )
        if target is None:
        ...
        if not isinstance(updated, dict):
            raise HTTPException(status_code=500, detail="Failed to update live snapshot Launch Control mapping")
        extension_payload = (
            updated.get("extensions", {}).get("launch_control")
            if isinstance(updated.get("extensions"), dict)
            else None
        )
