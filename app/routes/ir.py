...
try:
    ...
    async def _load_ir_to_target(ir_name: str, ir_type: str, instance_id: Optional[int]) -> Dict[str, Any]:
        ...
        if isinstance(instance_id, int) and instance_id > 0:
            engine = get_audio_engine()
            if ir_type == "cabinet":
                success = await engine.load_cabinet_ir_instance(instance_id, ir_path)
            else:
                success = await engine.load_reverb_ir_instance(instance_id, ir_path)
            if not success:
    ...
    async def _set_ir_mix(ir_type: str, mix: float, instance_id: Optional[int]) -> Dict[str, Any]:
        if not 0 <= mix <= 100:
            raise HTTPException(status_code=400, detail="Mix must be between 0 and 100")

        if isinstance(instance_id, int) and instance_id > 0:
            engine = get_audio_engine()
            updated = await engine.set_ir_mix_instance(instance_id, mix)
            if not updated:
                raise HTTPException(status_code=404, detail=f"IR instance not found: {instance_id}")
    ...
    async def _set_ir_bypass(ir_type: str, bypass: bool, instance_id: Optional[int]) -> Dict[str, Any]:
        if isinstance(instance_id, int) and instance_id > 0:
            engine = get_audio_engine()
            updated = await engine.set_ir_bypass_instance(instance_id, bypass)
            if not updated:
                raise HTTPException(status_code=404, detail=f"IR instance not found: {instance_id}")
        elif ir_type == "cabinet":
    ...
    async def _navigate_ir(ir_type: str, direction: str, instance_id: Optional[int]) -> Dict[str, Any]:
        irs = _scan_irs(ir_type)
        if not irs:
        ...
        if isinstance(instance_id, int) and instance_id > 0:
            info = await get_audio_engine().get_ir_info_instance(instance_id)
        ...
        return await _load_ir_to_target(next_name, ir_type, instance_id)
    ...
    @router.get("/status")
    async def get_ir_status(type: str = Query("cabinet"), instance_id: Optional[int] = Query(None)):
        ...
        if isinstance(instance_id, int) and instance_id > 0:
            return await _get_instance_ir_status(instance_id, type)
    ...
    async def load_cabinet_ir(ir_name: str, instance_id: Optional[int] = Query(None)):
        """Load a cabinet IR."""
        return await _load_ir_to_target(ir_name, "cabinet", instance_id)

    @router.post("/set-cabinet/{ir_name}")
    async def set_cabinet_ir(ir_name: str, instance_id: Optional[int] = Query(None)):
        """Set/load a cabinet IR (alias for frontend compatibility)."""
        payload = await _load_ir_to_target(ir_name, "cabinet", instance_id)
        payload["status"] = "ok"
        return payload

    @router.post("/reverbs/{ir_name}/load")
    async def load_reverb_ir(ir_name: str, instance_id: Optional[int] = Query(None)):
        """Load a reverb IR."""
        return await _load_ir_to_target(ir_name, "reverb", instance_id)

    @router.post("/set-reverb/{ir_name}")
    async def set_reverb_ir(ir_name: str, instance_id: Optional[int] = Query(None)):
        """Set/load a reverb IR (alias for frontend compatibility)."""
        payload = await _load_ir_to_target(ir_name, "reverb", instance_id)
        payload["status"] = "ok"
        return payload

    @router.post("/set-cabinet-mix/{mix}")
    async def set_cabinet_mix(mix: float, instance_id: Optional[int] = Query(None)):
        """Set cabinet IR wet/dry mix."""
        return await _set_ir_mix("cabinet", mix, instance_id)

    @router.post("/set-reverb-mix/{mix}")
    async def set_reverb_mix(mix: float, instance_id: Optional[int] = Query(None)):
        """Set reverb IR wet/dry mix."""
        return await _set_ir_mix("reverb", mix, instance_id)

    @router.post("/set-cabinet-bypass/{bypass}")
    async def set_cabinet_bypass(bypass: bool, instance_id: Optional[int] = Query(None)):
        """Set cabinet IR bypass state."""
        return await _set_ir_bypass("cabinet", bypass, instance_id)

    @router.post("/set-reverb-bypass/{bypass}")
    async def set_reverb_bypass(bypass: bool, instance_id: Optional[int] = Query(None)):
        """Set reverb IR bypass state."""
        return await _set_ir_bypass("reverb", bypass, instance_id)

    @router.post("/navigate-cabinet/{direction}")
    async def navigate_cabinet_ir(direction: str, instance_id: Optional[int] = Query(None)):
        """Navigate to the next/previous cabinet IR."""
        return await _navigate_ir("cabinet", direction, instance_id)

    @router.post("/navigate-reverb/{direction}")
    async def navigate_reverb_ir(direction: str, instance_id: Optional[int] = Query(None)):
        """Navigate to the next/previous reverb IR."""
        return await _navigate_ir("reverb", direction, instance_id)

    @router.post("/unload")
    async def unload_ir(type: str = Query("cabinet"), instance_id: Optional[int] = Query(None)):
        """Unload the current cabinet or reverb IR."""
        if type not in {"cabinet", "reverb"}:
            raise HTTPException(status_code=400, detail="type must be 'cabinet' or 'reverb'")

        if isinstance(instance_id, int) and instance_id > 0:
            unloaded = await get_audio_engine().unload_ir_instance(instance_id)
            if not unloaded:
                raise HTTPException(status_code=404, detail=f"IR instance not found: {instance_id}")
        elif type == "cabinet":
