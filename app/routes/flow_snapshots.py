...
class UpdateSnapshotRequest(BaseModel):
    ...
    snapshot_data: Optional[SnapshotData] = None
...
class PreviewSnapshotRequest(BaseModel):
    """Request to apply snapshot data without changing the active snapshot."""
    snapshot_data: SnapshotData
...
async def _prepare_snapshot_runtime(
    session,
    ...
    for old_chain_id_str, chain_data in snapshot_chains.items():
        ...
        if old_chain_id in existing_chain_ids:
            ...
            remapped_chains[str(old_chain_id)] = chain_data
            continue

        chain_name = chain_data.get("name", f"Chain {old_chain_id}")
        new_chain = await chain_service.create_chain(chain_name)
        if not new_chain:
            logger.warning("Failed to create missing chain %s during snapshot apply", chain_name)
            continue
    ...
    for slot in flow_slots:
        ...
        if old_id is not None and old_id in chain_id_mapping:
            slot["chainId"] = chain_id_mapping[old_id]

    snapshot_data["chains"] = remapped_chains
    return snapshot_data, chains_created


async def _apply_snapshot_to_engine(snapshot_data: Dict[str, Any]) -> tuple[int, int]:
...
async def create_snapshot(request: CreateSnapshotRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        async with get_session() as session:
            ...
            snapshot_data = await _enrich_snapshot_data(snapshot_data)
...
async def update_snapshot(
    ...
    """
    Update snapshot metadata and optionally replace snapshot_data
    ...
    - snapshot_data: Replacement snapshot payload (optional)
    ...
    try:
        ...
        async with get_session() as session:
            ...
            if request.snapshot_data is not None:
                snapshot.snapshot_data = json.dumps(
                    await _enrich_snapshot_data(request.snapshot_data.model_dump())
                )
...
async def load_snapshot(snapshot_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        async with get_session() as session:
            ...
            snapshot_data = json.loads(snapshot.snapshot_data)
            snapshot_data, chains_created = await _prepare_snapshot_runtime(session, snapshot_data)
            params_applied, bypass_applied = await _apply_snapshot_to_engine(snapshot_data)
...
async def preview_snapshot(request: PreviewSnapshotRequest) -> Dict[str, Any]:
    """
    Apply snapshot data without changing the active snapshot record.

    Request body:
    - snapshot_data: Complete flow state

    Returns:
        Normalized snapshot payload plus apply statistics
    """
    try:
        ...
        logger.error(f"Error previewing flow snapshot: {e}")
        raise HTTPException(status_code=500, detail=str(e))
