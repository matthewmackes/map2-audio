...
async def lifespan(app):
    ...
    try:
        ...
        # in single-node ALL-IN-ONE mode and meant the Sequencer Setup task's
        ...
        # the engine_command bridge above or the inbound traffic bridge.
        try:
            from app.services.midi_visualization_bridge import (
                install_midi_visualization_bridge,
            )
            install_midi_visualization_bridge()
        except Exception as exc:
            logger.warning(
                "MidiVisualizationProducerBridge not installed: %s", exc
            )
