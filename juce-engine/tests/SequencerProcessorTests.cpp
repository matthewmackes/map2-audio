...
#include "../Source/Sequencer/SequencerProcessor.h"
...
TEST_CASE("SequencerProcessor seeds 16 mixed workflow slots", "[brain][processor]") {
    SequencerProcessor processor;
    ...
    REQUIRE(firstSlot.mode == SequencerProcessor::SlotMode::Drum);
    REQUIRE(firstSlot.triggerNote == 36);
    REQUIRE(keyboardSlot.mode == SequencerProcessor::SlotMode::Chromatic);
    REQUIRE(hybridSlot.mode == SequencerProcessor::SlotMode::Hybrid);
}

TEST_CASE("SequencerProcessor keeps per-instance transport state isolated", "[brain][processor]") {
    SequencerProcessor left;
    SequencerProcessor right;
...
TEST_CASE("SequencerProcessor renders note-driven output and tracks voices", "[brain][processor]") {
    SequencerProcessor processor;
...
TEST_CASE("SequencerProcessor tracks keyboard polyphony and note-off release", "[brain][processor]") {
    SequencerProcessor processor;
    processor.prepareToPlay(48000.0, 128);
...
TEST_CASE("SequencerProcessor keeps trigger notes out of sustained keyboard polyphony", "[brain][processor]") {
    SequencerProcessor processor;
    processor.prepareToPlay(48000.0, 128);
...
TEST_CASE("SequencerProcessor clears active notes when transport stops", "[brain][processor]") {
    SequencerProcessor processor;
    processor.prepareToPlay(48000.0, 128);
